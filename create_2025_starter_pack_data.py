...
class StarterPack2025DataGenerator:
        ...
        with cfbd.ApiClient(self.cfbd_config) as api_client:
            ...
            for week in range(1, self.current_week + 1):
                ...
                games = self.rate_limit_wrapper(
                    games_api.get_games, year=2025, week=week, seasonType="regular"
                )
    ...
    def get_2025_advanced_game_stats(self, games):
        ...
        for game in games:
            ...
            home_row = {
                ...
                **{
                    f"defense_{k}": v for k, v in away_stats.items()
                },  # Defense stats = opponent offense
            }
            ...
            away_row = {
                ...
                **{
                    f"defense_{k}": v for k, v in home_stats.items()
                },  # Defense stats = opponent offense
            }
    ...
    def _generate_mock_advanced_stats(self, team, opponent, team_score, opponent_score):
        ...
        performance_multiplier = 1 + (
            score_diff * 0.05
        )  # Better performance when scoring more

        return {
            "passingPlays_explosiveness": base_explosiveness * performance_multiplier,
            "passingPlays_successRate": np.clip(
                base_success_rate * performance_multiplier, 0, 1
            ),
            "passingPlays_totalPPA": base_ppa
            * performance_multiplier
            * 50,  # Scale up for total
            "passingPlays_ppa": base_ppa * performance_multiplier,
            "rushingPlays_explosiveness": base_explosiveness
            * 0.9
            * performance_multiplier,
            "rushingPlays_successRate": np.clip(
                base_success_rate * 0.95 * performance_multiplier, 0, 1
            ),
            ...
            "openFieldYardsTotal": max(
                0, int(np.random.normal(120, 40) * performance_multiplier)
            ),
            "openFieldYards": max(
                0, int(np.random.normal(15, 5) * performance_multiplier)
            ),
            "secondLevelYardsTotal": max(
                0, int(np.random.normal(80, 30) * performance_multiplier)
            ),
            "secondLevelYards": max(
                0, int(np.random.normal(10, 3) * performance_multiplier)
            ),
            "lineYardsTotal": max(
                0, int(np.random.normal(60, 20) * performance_multiplier)
            ),
    ...
    def _generate_mock_drives(self, game):
        ...
        for drive_num in range(1, num_drives + 1):
            for is_home in [True, False]:
                ...
                drive = {
                    "offense": offense,
                    "offenseConference": getattr(
                        game,
                        "home_conference" if is_home else "away_conference",
                        "Unknown",
                    ),
                    "defense": defense,
                    "defenseConference": getattr(
                        game,
                        "away_conference" if is_home else "home_conference",
                        "Unknown",
                    ),
                    "gameId": game.id,
    ...
    def _generate_mock_game_stats(self, game, is_home):
        ...
        pass_completions = int(
            pass_attempts * np.clip(np.random.normal(0.6, 0.1), 0.3, 0.8)
        )
        ...
        return {
            ...
            "team": team,
            "conference": getattr(
                game, "home_conference" if is_home else "away_conference", "Unknown"
            ),
            "opponent_id": getattr(game, "away_id" if is_home else "home_id", None),
            "opponent": opponent,
            "opponent_conference": getattr(
                game, "away_conference" if is_home else "home_conference", "Unknown"
            ),
    ...
    def _generate_mock_plays(self, game):
        ...
        total_score = (
            game.home_points + game.away_scores
            if hasattr(game, "away_scores")
            else game.home_points + game.away_points
        )
        ...
        for play_num in range(total_plays):
            ...
            period = (
                1
                if play_num < total_plays * 0.25
                else (
                    2
                    if play_num < total_plays * 0.6
                    else (3 if play_num < total_plays * 0.85 else 4)
                )
            )
            ...
            yards_gained = (
                int(np.random.normal(3, 8)) if play_type in ["Pass", "Run"] else 0
            )
            ...
            play = {
                ...
                "offenseConference": getattr(
                    game,
                    "home_conference" if is_home_offense else "away_conference",
                    "Unknown",
                ),
                ...
                "defenseConference": getattr(
                    game,
                    "away_conference" if is_home_offense else "home_conference",
                    "Unknown",
                ),
    ...
    def _generate_mock_season_stats(self, team):
        ...
        return {
            ...
            "firstDownsOpponent": max(
                50, int(np.random.normal(200, 50) * games_played / 12)
            ),
            ...
            "interceptionsOpponent": max(
                1, int(np.random.normal(8, 4) * games_played / 12)
            ),
            ...
            "kickReturnYards": max(
                0, int(np.random.normal(500, 200) * games_played / 12)
            ),
            ...
            "kickReturnsOpponent": max(
                1, int(np.random.normal(25, 10) * games_played / 12)
            ),
            "netPassingYards": max(0, int(pass_yards_pg * games_played)),
            "netPassingYardsOpponent": max(
                0, int(np.random.normal(250, 80) * games_played)
            ),
            ...
            "passCompletionsOpponent": max(
                0, int(np.random.normal(35 * 0.6, 5) * games_played)
            ),
            ...
            "passingTDsOpponent": max(
                1, int(np.random.normal(20, 8) * games_played / 12)
            ),
            "penalties": max(5, int(np.random.normal(60, 20) * games_played / 12)),
            "penaltiesOpponent": max(
                5, int(np.random.normal(60, 20) * games_played / 12)
            ),
            "penaltyYards": max(
                20, int(np.random.normal(500, 150) * games_played / 12)
            ),
            ...
            "puntReturnTDs": max(
                0, int(np.random.normal(0.5, 0.8) * games_played / 12)
            ),
            "puntReturnTDsOpponent": max(
                0, int(np.random.normal(0.5, 0.8) * games_played / 12)
            ),
            "puntReturnYards": max(
                0, int(np.random.normal(200, 100) * games_played / 12)
            ),
            ...
            "tacklesForLossOpponent": max(
                2, int(np.random.normal(60, 20) * games_played / 12)
            ),
            "thirdDownConversions": max(
                5, int(np.random.normal(40, 15) * games_played / 12)
            ),
    ...
    def write_csv_files(self, data_dict):
        ...
        if "plays" in data_dict:
            ...
            for week in range(1, self.current_week + 1):
                week_plays = (
                    plays_df[plays_df["week"] == week]
                    if "week" in plays_df.columns
                    else plays_df.sample(frac=1 / 13)
                )
    ...
    def validate_output_files(self):
        ...
        if plays_dir.exists():
            ...
            if plays_files:
                ...
                logger.info(
                    f"    Sample plays file ({sample_file.name}): {len(df.columns)} columns"
                )
