...
class DataValidator:
    ...
    def validate_file_exists(self, file_path, description=""):
        ...
        self.log(
            f"{description}: {status} ({file_path})", "SUCCESS" if exists else "ERROR"
        )
        ...
        try:
            ...
            for col, description in expected_cols.items():
                if col in df.columns:
                    ...
                    if null_count == 0:
                        ...
                        self.log(
                            f"  ⚠️  {col:20s} - {description} ({null_count} null, {pct:.1f}%)",
                            "WARNING",
                        )
                        if pct > 50:
                            validation_result["status"] = "MISSING_COLUMNS"
                            self.report["issues_found"].append(
                                f"Column '{col}' has {null_count} null values ({pct:.1f}%)"
                            )
                else:
            ...
            if "season" in df.columns:
                ...
                if len(df_2025) > 500:
                    ...
                    self.report["issues_found"].append(
                        "2025 data may be mock/incomplete"
                    )
                ...
                if "week" in df_2025.columns:
                    ...
                    if 12 in df_2025["week"].values:
                        ...
                        # Ohio State vs UCLA
                        if (
                            "home_team" in df_week12.columns
                            and "away_team" in df_week12.columns
                        ):
                            osu_ucla = df_week12[
                                (
                                    (
                                        df_week12["home_team"].str.contains(
                                            "Ohio|OSU", case=False, na=False
                                        )
                                    )
                                    & (
                                        df_week12["away_team"].str.contains(
                            ...
                            if len(osu_ucla) > 0:
                                self.log(
                                    "✅ Ohio State vs UCLA FOUND in Week 12", "SUCCESS"
                                )
                                validation_result["osu_ucla_week12"] = True
                                game = osu_ucla.iloc[0]
                                self.log(f"   Game ID: {game.get('id', 'N/A')}", "INFO")
                                self.log(
                                    f"   Date: {game.get('start_date', 'N/A')}", "INFO"
                                )
                            else:
                                self.log(
                                    "❌ Ohio State vs UCLA NOT in Week 12", "ERROR"
                                )
                                validation_result["osu_ucla_week12"] = False
                                self.report["issues_found"].append(
                                    "Ohio State vs UCLA missing from Week 12"
                                )
                    else:
            ...
            if len(nulls_with_values) > 0:
                self.log(
                    f"Found {len(nulls_with_values)} columns with null values:",
                    "WARNING",
                )
            ...
            traceback.print_exc()
            self.report["validations"]["training_data"] = {
                "status": "ERROR",
                "error": str(e),
            }
            return False
        ...
        for name, rel_path in models.items():
            ...
            if full_path.exists():
                ...
                try:
                    ...
                    all_ok = False
                    self.report["issues_found"].append(
                        f"Model '{name}' exists but cannot be loaded: {str(e)[:50]}"
                    )
            else:
    ...
    def validate_week12_data(self):
        ...
        try:
            ...
            if len(df_week12) >= 45:
                ...
                self.report["issues_found"].append(
                    f"Week 12 has only {len(df_week12)} games (expected 45+)"
                )
    ...
    def generate_report(self):
        ...
        passed_checks = sum(
            1
            for v in self.report["validations"].values()
        ...
        # Save report
        report_file = (
            self.project_root / "project_management" / "DATA_VALIDATION_REPORT.json"
        )
