...
class DataRemediator:
    ...
    def fix_missing_columns(self, df):
        ...
        if "game_key" not in df.columns or df["game_key"].isnull().all():
            ...
            self.log(
                f"✅ Filled {missing_mask.sum()} missing game_key values", "SUCCESS"
            )
        else:
        ...
        if "conference_game" not in df.columns or df["conference_game"].isnull().all():
            ...
            df.loc[missing_mask, "conference_game"] = (
                ...
                & (
                    df.loc[missing_mask, "home_conference"]
                    == df.loc[missing_mask, "away_conference"]
                )
            )
            fixes_applied.append(
                f"Filled {missing_mask.sum()} missing conference_game values"
            )
            self.log(
                f"✅ Filled {missing_mask.sum()} missing conference_game values",
                "SUCCESS",
    ...
    def validate_team_names(self, df):
        ...
        teams_2025 = set(df_2025["home_team"].unique()) | set(
            df_2025["away_team"].unique()
        )
    ...
    def validate_conferences(self, df):
        ...
        if home_conf_missing == 0 and away_conf_missing == 0:
            ...
            self.log(
                f"⚠️  Missing conference data: Home={home_conf_missing}, Away={away_conf_missing}",
                "WARNING",
            )
    ...
    def run_remediation(self):
        ...
        try:
            ...
            print(
                f"  • Opponent-adjusted features: {self.report['summary'].get('opponent_adj_features', 'N/A')}"
            )
            print("=" * 80)
