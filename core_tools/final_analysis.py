...
def run_final_analysis():
    ...
    for team in pd.concat([df["home_team"], df["away_team"]]).unique():
        ...
        avg_elo = pd.concat([home_games["home_elo"], away_games["away_elo"]]).mean()

        avg_talent = pd.concat(
            [home_games["home_talent"], away_games["away_talent"]]
        ).mean()
...
if __name__ == "__main__":
    ...
    print(f"\n📊 Key Results:")
    print(
        f"Top Team: {result['power_rankings'][0]['team']} (Elo: {result['power_rankings'][0]['avg_elo']:.0f})"
    )
