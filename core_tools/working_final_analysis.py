...
def run_working_analysis():
    ...
    for team in pd.concat([df["home_team"], df["away_team"]]).unique():
        ...
        avg_elo = np.mean(
            [*home_games["home_elo"].fillna(0), *away_games["away_elo"].fillna(0)]
        )

        avg_talent = np.mean(
            [*home_games["home_talent"].fillna(0), *away_games["away_talent"].fillna(0)]
        )
    ...
    ranked_teams = sorted(
        team_strength.items(), key=lambda x: x[1]["combined_strength"], reverse=True
    )
    power_rankings = []
    ...
    for team in power_rankings[:10]:
        print(
            f"{team['rank']}. {team['team']} - Elo: {team['avg_elo']:.0f}, Talent: {team['avg_talent']:.0f}"
        )
    ...
    conference_strength = {}
    for conference in pd.concat(
        [df["home_conference"], df["away_conference"]]
    ).unique():
        conf_df = df[
            (df["home_conference"] == conference)
            | (df["away_conference"] == conference)
        ]
        ...
        conf_elo = np.mean(
            [team_strength[t]["avg_elo"] for t in conf_teams if t in team_strength]
        )
        conf_talent = np.mean(
            [team_strength[t]["avg_talent"] for t in conf_teams if t in team_strength]
        )
        conf_epa = np.mean(
            [team_strength[t]["avg_epa"] for t in conf_teams if t in team_strength]
        )
    ...
    ranked_conferences = sorted(
        conference_strength.items(), key=lambda x: x[1]["avg_elo"], reverse=True
    )
    ...
    # Analyze EPA trends
    high_epa_teams = sorted(
        team_strength.items(), key=lambda x: x[1]["avg_epa"], reverse=True
    )[:5]
    high_elo_teams = sorted(
        team_strength.items(), key=lambda x: x[1]["avg_elo"], reverse=True
    )[:5]
    high_talent_teams = sorted(
        team_strength.items(), key=lambda x: x[1]["avg_talent"], reverse=True
    )[:5]
    ...
    results["performance_trends"] = {
        ...
        "high_talent_teams": [
            {"team": t, "talent": s["avg_talent"]} for t, s in high_talent_teams
        ],
    }

    print(
        f"✅ EPA Leaders: {[t['team'] for t in results['performance_trends']['high_epa_teams']]}"
    )
    print(
        f"✅ Elo Leaders: {[t['team'] for t in results['performance_trends']['high_elo_teams']]}"
    )
    print(
        f"✅ Talent Leaders: {[t['team'] for t in results['performance_trends']['high_talent_teams']]}"
    )
...
"""
    for team_info in results["performance_trends"]["high_talent_teams"]:
        markdown_content += (
            f"- **{team_info['team']}**: {team_info['talent']:.0f} Talent\n"
        )
