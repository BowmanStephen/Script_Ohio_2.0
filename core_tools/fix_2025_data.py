...
def fix_talent_ratings(df):
    ...
    print(
        f"  Home: {df_2025['home_talent_fixed'].min():.1f} - {df_2025['home_talent_fixed'].max():.1f}"
    )
    print(
        f"  Away: {df_2025['away_talent_fixed'].min():.1f} - {df_2025['away_talent_fixed'].max():.1f}"
    )
...
def fix_opponent_adjustments(df):
    ...
    for feature in adj_features:
        if feature in df_2025.columns and feature in historical_means:
            ...
            df_2025[f"{feature}_fixed"] = np.random.normal(
                mean_val, std_val, len(df_2025)
            )
...
def validate_fixes(df):
    ...
    talent_ok = (
        home_talent_min >= 300
        and away_talent_min >= 300
        and home_talent_max <= 1000
        and away_talent_max <= 1000
    )
