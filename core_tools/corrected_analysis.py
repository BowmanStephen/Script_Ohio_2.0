...
def run_corrected_analysis():
    ...
    for team in pd.concat([df["home_team"], df["away_team"]]).unique():
        ...
        home_wins = len(
            team_df[home_mask & (team_df["home_points"] > team_df["away_points"])]
        )
        away_wins = len(
            team_df[away_mask & (team_df["away_points"] > team_df["home_points"])]
        )
    ...
    sorted_teams = sorted(
        team_metrics.items(), key=lambda x: x[1]["win_percentage"], reverse=True
    )
    top_teams = []
    ...
    for team in top_teams[:10]:
        print(
            f"{team['rank']}. {team['team']} - {team['win_percentage']:.1%} win rate ({team['games_played']} games)"
        )
    ...
    high_scorers = sorted(
        team_metrics.items(), key=lambda x: x[1]["avg_points_scored"], reverse=True
    )[:5]
    best_defenders = sorted(
        team_metrics.items(), key=lambda x: x[1]["avg_points_allowed"]
    )[:5]
    ...
    for _, row in df.iterrows():
        ...
        if home_strength and away_strength:
            ...
            predicted_spread = (
                home_strength.get("avg_points_scored", 0)
                - away_strength.get("avg_points_scored", 0)
            ) * 0.5 + 2.5
    ...
    correct_predictions = sum(
        ...
        if (
            ...
            and p["actual_home_score"] > p["actual_away_score"]
        )
        or (
            p["predicted_winner"] == p["away_team"]
            and p["actual_away_score"] > p["actual_home_score"]
        )
    )
    ...
    print(
        f"Prediction accuracy: {accuracy:.1%} ({correct_predictions}/{len(predictions)} correct)"
    )
    print(
        f"Average confidence: {np.mean([p['confidence_score'] for p in predictions]):.1f}%"
    )
    ...
    analysis_results = {
        ...
        "high_scorers": [
            {"team": t, "ppg": m["avg_points_scored"]} for t, m in high_scorers
        ],
        "best_defenders": [
            {"team": t, "ppg_allowed": m["avg_points_allowed"]}
            for t, m in best_defenders
        ],
    }
...
"""
    for team_info in analysis_results["best_defenders"]:
        markdown_content += (
            f"- **{team_info['team']}**: {team_info['ppg_allowed']:.1f} PPG allowed\n"
        )
