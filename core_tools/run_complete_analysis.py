...
def run_complete_analysis():
    ...
    for team in pd.concat([df["home_team"], df["away_team"]]).unique():
        ...
        wins = len(
            team_df[
                (team_df["home_team"] == team) & (df["home_points"] > df["away_points"])
            ]
        ) + len(
            team_df[(df["away_team"] == team) & (df["away_points"] > df["home_points"])]
        )

        win_pct = wins / len(team_df) if len(team_df) > 0 else 0
        avg_points_scored = np.mean(
            [
                df[df["home_team"] == team]["home_points"].mean(),
                df[df["away_team"] == team]["away_points"].mean(),
            ]
        )
        avg_points_allowed = np.mean(
            [
                df[df["home_team"] == team]["away_points"].mean(),
    ...
    insights.append(
        {
            ...
            "insights": [
                f"{team}: {metrics['avg_points_scored']:.1f} PPG"
    ...
    insights.append(
        {
            "type": "best_defenses",
            "insights": [
                f"{team}: {metrics['avg_points_allowed']:.1f} PPG allowed"
    ...
    json_summary = {
        ...
        "confidence_metrics": {
            ...
            "high_confidence_predictions": len(
                [p for p in predictions if p["confidence_score"] >= 80]
            ),
        },
        "insights": insights,
    }
