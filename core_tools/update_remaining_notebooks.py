...
"""
    ...
    for cell in notebook.get("cells", []):
        if cell.get("cell_type") == "code":
            ...
            if (
                'DATA_DIR = "./data"' in source
                and "get_starter_pack_config" not in source
            ):
                # Check if config import is already in this cell
                if (
                    "import sys" not in source
                    or "get_starter_pack_config" not in source
                ):
                    # Add config import at the beginning
                    if source.strip().startswith("DATA_DIR"):
                        source = config_import + source.replace(
                            'DATA_DIR = "./data"', "DATA_DIR = str(config.data_dir)"
                        )
                    else:
            ...
            if "config.current_year.csv" in source:
                source = source.replace(
                    '"config.current_year.csv"', 'f"{config.current_year}.csv"'
                )
                source = source.replace(
                    "'config.current_year.csv'", "f'{config.current_year}.csv'"
                )
                ...
                if (
                    "os.path.join(DATA_DIR" in source
                    and "config.current_year" in source
                ):
            ...
            if (
                "season == 2023" in source
                or "season == 2024" in source
                or "season == 2025" in source
            ):
                source = source.replace(
                    "season == 2023", f"season == {config.current_year}"
                )
                source = source.replace(
                    "season == 2024", f"season == {config.current_year}"
                )
                source = source.replace(
                    "season == 2025", f"season == {config.current_year}"
                )
