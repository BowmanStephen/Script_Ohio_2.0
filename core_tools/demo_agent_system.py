...
)  # Go up three levels to Script_Ohio_2.0 root
...
try:
    ...
    print(
        f"Files in agents/core directory: {os.listdir(agents_core_path) if os.path.exists(agents_core_path) else 'Directory not found'}"
    )
    print(f"Looking for agents in: {agents_path}")
    print(
        f"Files in agents directory: {os.listdir(agents_path) if os.path.exists(agents_path) else 'Directory not found'}"
    )
    print(
...
def demo_agent_framework():
    ...
    try:
        ...
        response = learning_agent.execute_request(
            test_request, PermissionLevel.READ_EXECUTE
        )
...
def demo_analytics_orchestrator():
    ...
    try:
        ...
        print(
            f"  Context Manager Cache Entries: {system_status['context_manager']['cache_entries']}"
        )
        print(f"  Active Agents: {system_status['agent_factory']['active_agents']}")
        print(
            f"  Total Interactions: {system_status['sessions']['total_interactions']}"
        )
        ...
        test_requests = [
            ...
            },
            {
                ...
                "request": AnalyticsRequest(
                    ...
                    parameters={},
                    context_hints={"focus_areas": ["data_structure", "features"]},
                    priority=2,
                ),
            },
            {
...
def demo_integration():
    ...
    try:
        ...
        print(
            f"  Total Requests Processed: {final_metrics['orchestrator']['performance_metrics']['total_requests']}"
        )
        print(
            f"  Success Rate: {final_metrics['orchestrator']['performance_metrics']['successful_requests'] / max(1, final_metrics['orchestrator']['performance_metrics']['total_requests']) * 100:.1f}%"
        )
        print(
            f"  Average Response Time: {final_metrics['orchestrator']['performance_metrics']['average_response_time']:.3f}s"
        )
        print(
            f"  Cache Hit Rate: {final_metrics['orchestrator']['performance_metrics']['cache_hit_rate']:.1f}%"
        )
