...
class Week12PredictionGenerator:
    ...
    def __init__(self):
        ...
        if MODEL_ENGINE_AVAILABLE:
            ...
            logger.warning(
                "⚠️  Model execution engine not available - will use simplified predictions"
            )
    ...
    def load_week12_games(self) -> bool:
        ...
        try:
            ...
            if self.week12_data_path.exists():
                ...
                logger.error(
                    f"❌ Enhanced data file not found: {self.week12_data_path}"
                )
                return False
    ...
    def generate_predictions_for_game(
        self, home_team: str, away_team: str, game_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.model_engine is None:
                return self._generate_fallback_predictions(
                    home_team, away_team, game_data, features
                )
            ...
            if ridge_result.get("success", False):
                ...
                predictions.update(
                    {
                        "ridge_margin_prediction": ridge_pred.get(
                            "predicted_margin", np.nan
                        ),
                        "ridge_confidence": ridge_pred.get("confidence", np.nan),
                        "ridge_predicted_winner": ridge_pred.get(
                            "predicted_winner", np.nan
                        ),
                    }
                )
            else:
                logger.warning(
                    f"⚠️  Ridge prediction failed for {home_team} vs {away_team}"
                )
                predictions.update(
                    {
            ...
            if xgb_result.get("success", False):
                ...
                predictions.update(
                    {
                        ...
                        "xgb_predicted_winner": xgb_pred.get(
                            "predicted_winner", np.nan
                        ),
                    }
                )
                ...
                logger.warning(
                    f"⚠️  XGBoost prediction failed for {home_team} vs {away_team}"
                )
                predictions.update(
                    {
            ...
            if not np.isnan(ridge_margin) and not np.isnan(xgb_margin):
                ...
                predictions["ensemble_predicted_winner"] = (
                    home_team if predictions["ensemble_margin"] > 0 else away_team
                )
            elif not np.isnan(ridge_margin):
                predictions["ensemble_margin"] = ridge_margin
                predictions["ensemble_margin_std"] = 0
                predictions["ensemble_predicted_winner"] = (
                    home_team if ridge_margin > 0 else away_team
                )
            elif not np.isnan(xgb_margin):
                predictions["ensemble_margin"] = xgb_margin
                predictions["ensemble_margin_std"] = 0
                predictions["ensemble_predicted_winner"] = (
                    home_team if xgb_margin > 0 else away_team
                )
            else:
            ...
            if not np.isnan(predictions.get("ridge_margin_prediction", np.nan)):
                ...
                ridge_win_prob = stats.norm.cdf(
                    predictions["ridge_margin_prediction"] / 14
                )

            xgb_win_prob = predictions.get("xgb_win_probability", 0.5)

            if ridge_win_prob is not None and not np.isnan(xgb_win_prob):
                predictions["ensemble_win_probability"] = (
                    0.5 * ridge_win_prob + 0.5 * xgb_win_prob
                )
            ...
            logger.error(
                f"❌ Prediction generation failed for {home_team} vs {away_team}: {e}"
            )
            return None
    ...
    def _generate_fallback_predictions(
        ...
        try:
            ...
            predicted_margin = (
                talent_diff + random_variation + 3
            )  # 3 points home field advantage
    ...
    def generate_all_predictions(self) -> bool:
        ...
        try:
            ...
            for idx, game in self.week12_games.iterrows():
                try:
                    ...
                    if game_predictions:
                        ...
                        win_prob = game_predictions.get(
                            "ensemble_win_probability", np.nan
                        )
                        confidence = game_predictions.get("confidence_score", np.nan)
                        winner = game_predictions.get(
                            "ensemble_predicted_winner", "Unknown"
                        )

                        logger.info(
                            f"   ✅ {winner} to win by {margin:.1f} points "
                            f"(Win Prob: {win_prob:.1%}, Confidence: {confidence:.1%})"
                        )
                    else:
                        logger.error(
                            f"   ❌ Failed to generate predictions for {home_team} vs {away_team}"
                        )
            ...
            logger.info(
                f"✅ Generated predictions for {successful_predictions}/{len(self.week12_games)} games"
            )
    ...
    def analyze_predictions(self) -> Dict[str, Any]:
        ...
        try:
                ...
                analysis["prediction_summary"].update(
                    {
                        ...
                        "high_home_win_prob_games": int(
                            (ensemble_win_probs > 0.7).sum()
                        ),
                        "low_home_win_prob_games": int(
                            (ensemble_win_probs < 0.3).sum()
                        ),
                    }
                )
            ...
            for _, pred in self.predictions.iterrows():
                ...
                if (
                    pred.get("ridge_confidence", 0) > 0
                    and pred.get("xgb_confidence", 0) > 0
                ):
                    # This is a simplified upset detection
                    if (
                        pred["ensemble_win_probability"] < 0.4
                        and pred["ensemble_win_probability"] > 0.25
                    ):
            ...
            analysis["upset_predictions"].sort(
                key=lambda x: x["confidence"], reverse=True
            )
            analysis["high_confidence_games"].sort(
                key=lambda x: x["confidence"], reverse=True
            )
            analysis["close_predictions"].sort(key=lambda x: abs(x["predicted_margin"]))

            logger.info(
                f"✅ Analysis complete: {len(analysis['high_confidence_games'])} high confidence, "
    ...
    def save_results(self, analysis: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            publication_summary = {
                ...
                "key_insights": {
                    "high_confidence_picks": len(
                        analysis.get("high_confidence_games", [])
                    ),
                    "potential_upsets": len(analysis.get("upset_predictions", [])),
                    "close_games": len(analysis.get("close_predictions", [])),
                    "average_confidence": analysis.get("confidence_analysis", {}).get(
                        "average_confidence", 0
                    ),
                    "home_team_advantage": analysis.get("prediction_summary", {}).get(
                        "home_team_wins", 0
                    ),
                },
            }
    ...
    def run_complete_pipeline(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        execution_time = time.time() - start_time
        logger.info(
            f"✅ Week 12 Prediction Pipeline completed in {execution_time:.1f} seconds"
        )
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print(
            f"🎯 Found {len(analysis.get('high_confidence_games', []))} high confidence predictions"
        )
        print(
            f"⚠️  Identified {len(analysis.get('upset_predictions', []))} potential upsets"
        )
