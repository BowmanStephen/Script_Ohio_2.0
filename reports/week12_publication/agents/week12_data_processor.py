...
class Week12DataProcessor:
    ...
    def load_models(self) -> bool:
        ...
        try:
            ...
            if self.xgb_model_path.exists():
                ...
                logger.warning(
                    "⚠️  XGBoost model not found - will skip XGBoost predictions"
                )
            ...
            if self.fastai_model_path.exists():
                ...
                logger.warning(
                    "⚠️  FastAI model not found - will skip FastAI predictions"
                )
    ...
    def load_training_data_schema(self) -> bool:
        ...
        try:
            ...
            if self.training_data_path.exists():
                ...
                logger.info(
                    f"✅ Training schema loaded: {len(self.feature_columns)} features"
                )
                return True
            else:
    ...
    def validate_data_quality(self) -> Dict[str, Any]:
        ...
        if self.week12_games is not None and len(self.week12_games) > 0:
            ...
            complete_mask = (
                self.week12_games[self.feature_columns].notnull().all(axis=1)
            )
            ...
            for feature in self.feature_columns:
                ...
                completeness = (len(self.week12_games) - missing_count) / len(
                    self.week12_games
                )
            ...
            overall_completeness = np.mean(
                list(quality_report["feature_completeness"].values())
            )
            quality_report["overall_completeness_score"] = overall_completeness

            logger.info(f"📊 Data quality score: {overall_completeness:.1%}")
            logger.info(
                f"✅ Complete games: {quality_report['complete_games']}/{quality_report['total_games']}"
            )

            if quality_report["missing_features"]:
                logger.warning(
                    f"⚠️  Features with missing data: {quality_report['missing_features']}"
                )
            ...
            try:
                ...
                if self.feature_columns:
                    ...
                    if "xgb" in self.models:
                        try:
                            xgb_pred = self.models["xgb"].predict_proba(features_array)[
                                0, 1
                            ]
                    ...
                    if "fastai" in self.models:
                        try:
                            fastai_pred = self.models["fastai"].predict_proba(
                                features_array
                            )[0, 1]
                ...
                if valid_margin_preds:
                    game_prediction["ensemble_margin_prediction"] = np.mean(
                        valid_margin_preds
                    )
                    game_prediction["margin_prediction_std"] = np.std(
                        valid_margin_preds
                    )
                else:
                ...
                if valid_win_prob_preds:
                    game_prediction["ensemble_win_probability"] = np.mean(
                        valid_win_prob_preds
                    )
                    game_prediction["win_probability_std"] = np.std(
                        valid_win_prob_preds
                    )
                else:
                ...
                if not np.isnan(
                    game_prediction.get("ensemble_win_probability", np.nan)
                ):
                    confidence = (
                        abs(game_prediction["ensemble_win_probability"] - 0.5) * 2
                    )
        ...
        try:
            ...
            summary_stats = {
                ...
                "games_with_predictions": len(
                    predictions_df.dropna(subset=["ensemble_margin_prediction"])
                ),
                "average_confidence": predictions_df["confidence_score"].mean(),
                "high_confidence_games": len(
                    predictions_df[predictions_df["confidence_score"] > 0.7]
                ),
    ...
    def run_complete_pipeline(self) -> Tuple[bool, pd.DataFrame]:
        ...
        logger.info(
            f"✅ Week 12 Data Processing Pipeline completed in {execution_time:.1f} seconds"
        )
