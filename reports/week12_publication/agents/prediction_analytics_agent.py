...
class PredictionAnalyticsAgent:
    ...
    def calculate_confidence_intervals(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Calculated confidence intervals for {len(margins)} predictions"
            )
    ...
    def analyze_prediction_distribution(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Distribution analysis complete: {close_games} close, {blowouts} blowouts"
            )
    ...
    def identify_key_insights(self) -> Dict[str, Any]:
        ...
        try:
            ...
            high_conf = df_sorted.nlargest(5, "confidence_score")[
                [
                    "home_team",
                    "away_team",
            ...
            low_conf = df_sorted[df_sorted["confidence_score"] > 0.1].nsmallest(
                5, "confidence_score"
            )[
                [
                    "home_team",
            ...
            close_games = df_sorted.nsmallest(5, "abs_margin")[
                [
                    "home_team",
                    "away_team",
                    "ensemble_predicted_winner",
            ...
            largest_margins = df_sorted.nlargest(5, "abs_margin")[
                [
                    "home_team",
                    "away_team",
                    "ensemble_predicted_winner",
            ...
            away_favored = df_sorted[
                (df_sorted["ensemble_margin"] < 0)
                & (df_sorted["confidence_score"] > 0.4)
            ]
            if len(away_favored) > 0:
                upset_games = away_favored[
                    [
                        "home_team",
                        "away_team",
                        "ensemble_predicted_winner",
            ...
            for home, away in rivalry_matchups:
                ...
                if len(rivalry_game) > 0:
                    rivalry_info = rivalry_game.iloc[0][
                        [
                            "home_team",
                            "away_team",
                            "ensemble_predicted_winner",
            ...
            logger.info(
                f"✅ Key insights identified: {len(insights['rivalry_highlights'])} rivalry games, "
    ...
    def generate_betting_analytics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            home_value = df_betting[df_betting["home_team_ev"] > 0.05].nlargest(
                5, "home_team_ev"
            )
            away_value = df_betting[df_betting["away_team_ev"] > 0.05].nlargest(
                5, "away_team_ev"
            )
            ...
            if len(home_value) > 0:
                home_value_picks = home_value[
                    [
                        ...
                        "away_team",
                        "ensemble_predicted_winner",
                        "ensemble_win_probability",
                        "home_team_ev",
                        "confidence_score",
                    ]
                ].rename(columns={"home_team_ev": "expected_value"})
                betting_insights["value_opportunities"].extend(
                    home_value_picks.to_dict("records")
                )
            ...
            if len(away_value) > 0:
                ...
                betting_insights["value_opportunities"].extend(
                    away_value_picks.to_dict("records")
                )

            # High confidence picks (for straight bets)
            high_conf_games = df_betting[df_betting["confidence_score"] > 0.8].nlargest(
                5, "confidence_score"
            )
            if len(high_conf_games) > 0:
                ...
                betting_insights["high_value_picks"] = high_conf_picks.to_dict(
                    "records"
                )
            ...
            # Confidence rankings
            confidence_ranked = df_betting.sort_values(
            ...
            positive_ev_games = len(
                df_betting[
                    (df_betting["home_team_ev"] > 0) | (df_betting["away_team_ev"] > 0)
                ]
            )
            ...
            logger.info(
                f"✅ Betting analytics complete: {positive_ev_games} value opportunities found"
            )
    ...
    def create_performance_benchmarks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if "actual_margin" in df.columns and df["actual_margin"].notna().any():
                ...
                margin_errors = abs(
                    actual_available["ensemble_margin"]
                    - actual_available["actual_margin"]
                )
                benchmarks["model_performance_metrics"]["margin_mae"] = float(
                    margin_errors.mean()
                )
                benchmarks["model_performance_metrics"]["margin_rmse"] = float(
                    np.sqrt((margin_errors**2).mean())
                )
                ...
                benchmarks["model_performance_metrics"]["win_prediction_accuracy"] = (
                    float(win_accuracy)
                )
            ...
            benchmarks["prediction_accuracy_indicators"] = {
                ...
                "extreme_predictions": {
                    "games_predicted_over_30_points": int(
                        (df["ensemble_margin"].abs() > 30).sum()
                    ),
                    "games_predicted_under_3_points": int(
                        (df["ensemble_margin"].abs() < 3).sum()
                    ),
                },
            }
            ...
            high_risk_high_reward = df[
                (df["risk_score"] > 0.5) & (df["abs_margin"] > 14)
            ]
            ...
            benchmarks["risk_reward_analysis"] = {
                ...
                "risk_return_correlation": float(
                    df[["risk_score", "abs_margin"]].corr().iloc[0, 1]
                ),
            }
            ...
            quality_metrics = {
                "data_completeness": {
                    ...
                    "complete_predictions": len(
                        df.dropna(subset=["ensemble_margin", "confidence_score"])
                    ),
                    "completeness_rate": float(
                        len(df.dropna(subset=["ensemble_margin", "confidence_score"]))
                        / len(df)
                    ),
                },
                "prediction_range": {
                    ...
                    "margin_range": float(
                        df["ensemble_margin"].max() - df["ensemble_margin"].min()
                    ),
                },
            }
        ...
        try:
            ...
            summary = {
                ...
                "key_findings": {
                    ...
                    .get("mean", 0),
                    "high_confidence_games": all_results.get(
                        "distribution_analysis", {}
                    )
                    .get("confidence_distribution", {})
                    .get("high_confidence_games", 0),
                    "close_competitive_games": all_results.get(
                        "distribution_analysis", {}
                    )
                    .get("game_categories", {})
                    .get("close_games", 0),
                    "predicted_blowouts": all_results.get("distribution_analysis", {})
                    .get("game_categories", {})
                    .get("blowout_predictions", 0),
                },
                "betting_insights": {
                    "value_opportunities": all_results.get("betting_analytics", {})
                    .get("summary", {})
                    .get("value_opportunities", 0),
                    "high_confidence_picks": all_results.get("betting_analytics", {})
                    .get("summary", {})
                    .get("high_confidence_picks", 0),
                },
            }
            ...
            if all_results.get("key_insights"):
                ...
                if key_insights.get("highest_confidence_games"):
                    high_conf_df = pd.DataFrame(
                        key_insights["highest_confidence_games"]
                    )
    ...
    def run_complete_analytics(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        logger.info(
            f"✅ Prediction Analytics completed in {execution_time:.2f} seconds"
        )
