...
class RandomForestScorePredictor:
    ...
    def train(self, data: pd.DataFrame):
        ...
        if "home_points" not in data.columns or "away_points" not in data.columns:
            raise ValueError(
                "Training data must contain 'home_points' and 'away_points' columns."
            )

        # Drop rows with NaN in targets or features
        train_data = data.dropna(
            subset=self.features + ["home_points", "away_points"]
        ).copy()
        ...
        results = pd.DataFrame(
            {
                ...
                "predicted_margin": pred_away
                - pred_home,  # margin = away - home usually? Or home - away?
