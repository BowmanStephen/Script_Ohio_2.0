...
def get_teams_from_data(
    ...
    try:
        ...
        home_teams = (
            df["home_team"].unique().tolist() if "home_team" in df.columns else []
        )
        away_teams = (
            df["away_team"].unique().tolist() if "away_team" in df.columns else []
        )
...
def get_popular_matchups(
    csv_path: Optional[Path] = None, season: Optional[int] = None, limit: int = 10
) -> List[Tuple[str, str]]:
    """
    ...
    try:
        ...
        if "home_team" in df.columns and "away_team" in df.columns:
            matchups = (
                df.groupby(["home_team", "away_team"]).size().reset_index(name="count")
            )
...
def get_sample_matchup(
    csv_path: Optional[Path] = None, season: Optional[int] = None
) -> Tuple[str, str]:
    """
