...
class KellyCriterionCalculator:
    ...
    def __init__(self, min_bet_size: float = 1.0, max_kelly_fraction: float = 0.25):
        ...
        logger.info(
            f"Kelly Calculator initialized: min=${min_bet_size}, max_kelly={max_kelly_fraction * 100}%"
        )
    ...
    def calculate_kelly_fraction(
        ...
        growth_rate = p * math.log(1 + b * kelly_fraction) + q * math.log(
            1 - kelly_fraction
        )
        ...
        if kelly_fraction <= 0.01:  # 1% or less
            ...
            recommendation = (
                "Exceptional edge - use fractional Kelly (25-50%) for safety"
            )

        return KellyCriterionResult(
            kelly_fraction=kelly_fraction,
            suggested_bet_size=max(
                self.min_bet_size, kelly_fraction * 1000
            ),  # Assume $1k bankroll
    ...
    def calculate_fractional_kelly(
        self, base_kelly: float, fraction: float = 0.5, bankroll: float = 1000.0
    ) -> KellyCriterionResult:
        """
...
class ValueBetDetector:
    ...
    def __init__(self, min_edge_threshold: float = 0.02):
        ...
        logger.info(
            f"Value Bet Detector initialized: min edge={min_edge_threshold * 100}%"
        )

    def identify_value_bets(
        self, prediction: BettingPrediction, odds: BettingOdds, bankroll: float = 1000.0
        ...
        if odds.home_moneyline is not None:
            opportunities["MONEYLINE_HOME"] = self._analyze_moneyline_value(
                prediction.home_win_probability, odds.home_moneyline, "HOME", bankroll
            )

        if odds.away_moneyline is not None:
            opportunities["MONEYLINE_AWAY"] = self._analyze_moneyline_value(
                prediction.away_win_probability, odds.away_moneyline, "AWAY", bankroll
            )
        ...
        if odds.home_spread is not None and odds.home_spread_odds is not None:
            opportunities["SPREAD_HOME"] = self._analyze_spread_value(
                prediction, odds.home_spread, odds.home_spread_odds, "HOME", bankroll
            )
        ...
        if odds.over_under is not None and odds.over_odds is not None:
            opportunities["OVER_UNDER"] = self._analyze_total_value(
                prediction, odds.over_under, odds.over_odds, bankroll
            )

        return opportunities

    def _analyze_moneyline_value(
        self, true_probability: float, american_odds: float, team: str, bankroll: float
    ) -> ValueBetOpportunity:
        ...
        if is_value_bet:
            if edge_percentage > 0.10:  # 10%+ edge
                ...
                recommendation = (
                    f"MARGINAL VALUE: {team} has {edge_percentage:.1%} edge"
                )
        else:
            if edge_percentage < -self.min_edge_threshold:
                recommendation = (
                    f"AVOID: {team} is overvalued by {abs(edge_percentage):.1%}"
                )
            else:
        ...
        return ValueBetOpportunity(
            ...
            confidence_level=min(true_probability, 1 - true_probability)
    ...
    def _analyze_spread_value(
        ...
        if team == "HOME":
            ...
            margin_to_cover = (
                spread  # Away team needs to win or lose by less than spread
            )
...
class BettingAnalyticsEngine:
    ...
    def __init__(self, model_execution_engine=None):
        ...
        logger.info(
            "🎲 Betting Analytics Engine initialized with Kelly Criterion and Value Detection"
        )

    def create_betting_prediction(
        self, model_prediction: Dict[str, Any], home_team: str, away_team: str
    ) -> BettingPrediction:
        """
        ...
        try:
            ...
            if model_prediction.get("model_type") == "regression":
                ...
                home_win_prob = 1 / (
                    1 + math.exp(-predicted_margin / 10)
                )  # Logistic function
    ...
    def analyze_betting_opportunity(
        ...
        try:
            ...
            if self.model_engine:
                ...
                model_result = self.model_engine._predict_game_outcome(
                    prediction_request, {}
                )
                ...
                if not model_result["success"]:
                    return {
                        "success": False,
                        "error_message": model_result.get(
                    ...
                    if "MONEYLINE" in bet_type:
                        if "HOME" in bet_type and odds.home_moneyline:
                            kelly_result = self.kelly_calculator.calculate_kelly_fraction(
                                ...
                                true_probability=betting_prediction.home_win_probability
                                * (1 + opportunity.edge_percentage),
                            ...
                            kelly_result = self.kelly_calculator.calculate_kelly_fraction(
                                ...
                                true_probability=betting_prediction.away_win_probability
                                * (1 + opportunity.edge_percentage),
            ...
            analysis = {
                ...
                "odds": asdict(odds),
                "value_opportunities": {
                    k: asdict(v) for k, v in value_opportunities.items()
                },
                "kelly_recommendations": {
                    k: asdict(v) for k, v in kelly_recommendations.items()
                },
                "summary": self._generate_betting_summary(
                    betting_prediction, value_opportunities, kelly_recommendations
                ),
                "risk_assessment": self._assess_betting_risk(
                    betting_prediction, value_opportunities
                ),
    ...
    def _update_betting_analytics(
        self, value_opportunities: Dict[str, ValueBetOpportunity]
    ):
        ...
        if value_opportunities:
            edges = [
                op.edge_percentage
                for op in value_opportunities.values()
                if op.edge_percentage > 0
            ]
            if edges:
    ...
    def _generate_betting_summary(
        ...
        max_kelly = (
            max(kelly_recs.values(), key=lambda x: x.kelly_fraction)
            if kelly_recs
            else None
        )
        ...
        return {
            ...
            "best_edge_type": max(
                opportunities.keys(), key=lambda k: opportunities[k].edge_percentage
            ),
    ...
    def _assess_betting_risk(
        ...
        if margin_width > 30:  # Very wide CI
            risk_factors.append(
                "Wide prediction confidence interval indicates uncertainty"
            )
            risk_score += 15
        ...
        negative_edges = [
            ...
            if op.edge_percentage < -0.05
        ]
        if negative_edges:
            risk_factors.append(
                f"Market appears efficient or overvalued ({len(negative_edges)} bets)"
            )
            risk_score += 10
        ...
        if risk_score < 15:
            ...
            risk_level = "HIGH"
            assessment = (
                "High risk - limited value, consider waiting for better opportunities"
            )
        else:
            risk_level = "VERY HIGH"
    ...
    def _generate_betting_recommendations(
        ...
        if value_bets:
            ...
            for bet_type, opportunity in value_bets:
                if bet_type != best_bet[0]:  # Don't repeat best bet
                    recommendations.append(
                        f"Additional Value: {bet_type} ({opportunity.edge_percentage:.1%} edge)"
                    )
        ...
        if kelly_recs:
            ...
            if max_kelly.kelly_fraction > 0.01:
                ...
                recommendations.append(
                    f"Risk Level: {max_kelly.risk_level} - {max_kelly.recommendation}"
                )

        # Model confidence recommendations
        if prediction.prediction_confidence > 0.7:
            recommendations.append(
                "High model confidence - consider larger position sizing"
            )
        elif prediction.prediction_confidence < 0.5:
            recommendations.append(
                "Low model confidence - consider smaller bets or skip"
            )

        # Market efficiency recommendations
        if len(value_bets) == 0:
            recommendations.append(
                "Market appears efficient - consider waiting for line movement"
            )
        elif len(value_bets) >= 3:
            recommendations.append(
                "Multiple value opportunities - diversify across bet types"
            )

        # Risk management recommendations
        recommendations.append("Never bet more than 25% of bankroll on a single game")
        recommendations.append(
            "Consider fractional Kelly (50-75%) for conservative approach"
        )
    ...
    def get_betting_performance_report(self) -> Dict[str, Any]:
        ...
        if not self.betting_history:
            return {"success": False, "message": "No betting history available"}
    ...
    def _generate_performance_recommendations(self) -> List[str]:
        ...
        if self.performance_metrics["average_edge"] < 0.02:
            recommendations.append(
                "Consider refining models to increase edge detection"
            )
        elif self.performance_metrics["average_edge"] > 0.08:
            recommendations.append(
                "Excellent edge detection - consider increasing bet sizes appropriately"
            )

        value_rate = self.performance_metrics["value_bets_identified"] / max(
            self.performance_metrics["total_bets_analyzed"], 1
        )
        if value_rate < 0.2:
            recommendations.append(
                "Low value bet rate - wait for better opportunities or improve models"
            )
        elif value_rate > 0.5:
...
def create_betting_analytics_engine(
...
def test_betting_integration():
    ...
    try:
        ...
        logger.info(
            f"✅ Betting prediction created: {betting_prediction.home_team} {betting_prediction.home_win_probability:.1%}"
        )

        # Test Kelly Criterion
        kelly = engine.kelly_calculator.calculate_kelly_fraction(
            implied_probability=0.45, true_probability=0.55, american_odds=-110
        )
        logger.info(
            f"✅ Kelly Criterion: {kelly.kelly_fraction:.1%} bankroll, {kelly.recommendation}"
        )
        ...
        logger.info(
            f"✅ Value bets found: {len([o for o in opportunities.values() if o.is_value_bet])}"
        )
