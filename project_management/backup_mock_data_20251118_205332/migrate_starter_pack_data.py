...
and processes it to match the model pack's 86-feature format.
...
class StarterPackDataMigrator:
    ...
    def load_starter_pack_data(self):
        """Load current season games from starter pack"""
        ...
        if current_week >= 12:
            ...
            if len(osu_ucla) > 0:
                print(f"  Game ID: {osu_ucla.iloc[0]['id']}")
                print(f"  Date: {osu_ucla.iloc[0]['start_date']}")
