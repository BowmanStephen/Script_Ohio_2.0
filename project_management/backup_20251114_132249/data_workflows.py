    ...
    def fetch_games(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(
                "cfbd package missing. Install `pip install cfbd` or "
                "use requirements-optional.txt before refreshing starter data."
            ) from exc
        ...
        kwargs = {"year": season, "season_type": season_type}
        if week is not None:
            kwargs["week"] = week
        response = games_api.get_games(**kwargs)

        games_dicts = [game.to_dict() for game in response]
        if not games_dicts:
