...
class CrossSourceValidator:
    ...
    def _get_cfbd_sample_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results["critical_issues"].append("CFBD library not available")
    ...
    def _validate_team_records(self, cfbd_sample: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            with cfbd.ApiClient(configuration) as api_client:
                ...
                if fbs_teams:
                    sample_teams = fbs_teams[:10]  # Validate first 10 teams
                    results["teams_validated"] = len(sample_teams)

                    for team in sample_teams:
                        team_name = getattr(team, 'school', None)
                        if team_name:
                            # Get team's 2025 games to calculate record
                            try:
                                games_api = cfbd.GamesApi(api_client)
            ...
            results["warnings"].append("CFBD library not available for team record validation")
        except Exception as e:
