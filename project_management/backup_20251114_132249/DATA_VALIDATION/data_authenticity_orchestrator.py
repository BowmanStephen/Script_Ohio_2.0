...
class DataAuthenticityOrchestrator:
    ...
    def _validate_api_source(self) -> Dict[str, Any]:
        ...
        # Import CFBD for testing
        try:
            import cfbd
            import os
        except ImportError:
            return {
                "critical_issues": ["CFBD library not available"],
        ...
        if not api_key:
            results["critical_issues"].append("CFBD_API_KEY not found in environment")
            return results
        ...
        try:
            with cfbd.ApiClient(configuration) as api_client:
                for endpoint_test in test_endpoints:
                    result = self._test_cfbd_endpoint(api_client, endpoint_test)
    ...
    def _test_cfbd_endpoint(self, api_client, endpoint_test: Dict) -> Dict[str, Any]:
