...
class ContextManager:
    ...
    def __init__(self, base_path: Optional[str] = None):
        ...
        logger.info(
            f"Context Manager initialized with conversation memory at: {self.base_path}"
        )
    ...
    def detect_user_role(self, user_context: Dict[str, Any]) -> UserRole:
        ...
        if any(
            keyword in query_type.lower()
            for keyword in ["learn", "tutorial", "introduction", "explain"]
        ):
            role_scores[UserRole.ANALYST] += 2
        elif any(
            keyword in query_type.lower()
            for keyword in ["predict", "production", "api", "deploy"]
        ):
            role_scores[UserRole.PRODUCTION] += 2
        elif any(
            keyword in query_type.lower()
            for keyword in ["feature", "model", "optimize", "advanced"]
        ):
            role_scores[UserRole.DATA_SCIENTIST] += 2
        ...
        logger.info(
            f"Detected user role: {detected_role.value} (scores: {role_scores})"
        )
        return detected_role

    def load_context_for_role(
        self, role: Union[UserRole, str], request_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            raise ValueError(
                f"Role '{role.value}' is not configured. Available roles: {list(self.profiles.keys())}"
            )
        ...
        logger.info(
            f"Loaded optimized context for {role.value}: {len(str(optimized_context))} chars"
        )
        return optimized_context

    def _apply_personalization(
        self, context: Dict[str, Any], request_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if "preferred_models" in user_preferences:
            ...
            for model in preferred_models:
                if any(
                    model_file.endswith(model)
                    for model_file in self._get_available_models()
                ):
    ...
    def _get_available_models(self) -> List[str]:
        ...
        try:
            ...
            if model_path.exists():
                return [
                    f.name
                    for f in model_path.glob("*.joblib")
                    or f in model_path.glob("*.pkl")
                ]
        except Exception:
            pass
        return []
    ...
    def _calculate_context_quality(self, context: Dict[str, Any]) -> float:
        ...
        if "notebooks" in context and context["notebooks"]:
            ...
            if isinstance(notebooks, list):
                notebook_score = min(
                    1.0, len(notebooks) / 5.0
                )  # Normalize to 5 notebooks
        ...
        if "optimization_metadata" in context:
            ...
            if metadata.get("optimization_applied", False):
                ...
                opt_score = 0.5 + min(
                    0.5, savings / 100.0
                )  # Bonus for effective optimization
            score += opt_score
    ...
    def _load_features_for_role(self, profile: ContextProfile) -> List[Dict[str, Any]]:
        ...
        feature_descriptions = {
            "home_talent": {
                "type": "numeric",
                "description": "Home team talent composite rating",
            },
            "away_talent": {
                "type": "numeric",
                "description": "Away team talent composite rating",
            },
            "home_elo": {
                "type": "numeric",
                "description": "Home team pre-game Elo rating",
            },
            "away_elo": {
                "type": "numeric",
                "description": "Away team pre-game Elo rating",
            },
            "spread": {
                "type": "numeric",
                "description": "Vegas betting spread (negative = home favored)",
            },
            "home_adjusted_epa": {
                "type": "numeric",
            ...
            "home_adjusted_success": {
                "type": "numeric",
                "description": "Home team adjusted success rate",
            },
            "away_adjusted_success": {
                "type": "numeric",
                "description": "Away team adjusted success rate",
            },
            "home_adjusted_explosiveness": {
                "type": "numeric",
                "description": "Home team explosiveness metric",
            },
            "away_adjusted_explosiveness": {
                "type": "numeric",
                "description": "Away team explosiveness metric",
            },
            "home_total_havoc_offense": {
                "type": "numeric",
                "description": "Home team havoc rate allowed",
            },
            "away_total_havoc_offense": {
                "type": "numeric",
                "description": "Away team havoc rate allowed",
            },
        }
        ...
        for feature_name in profile.features_priority:
            if feature_name in feature_descriptions:
                ...
                feature_info["priority"] = (
                    profile.features_priority.index(feature_name) + 1
                )
                features.append(feature_info)

        return features

    def _load_documentation_for_role(
        ...
        if profile.role == UserRole.ANALYST:
            base_docs.extend(
                [
                    {
                        "file": "starter_pack/README.md",
                        "name": "Starter Pack Guide",
                        "priority": 3,
                    }
                ]
            )
            ...
            base_docs.extend(
                [
                    {
                        "file": "model_pack/model_deployment_guide_2025.md",
                        "name": "Model Deployment Guide",
                        "priority": 3,
                    },
                    {
                        "file": "MODEL_USAGE_GUIDE.md",
                        "name": "Model Usage Guide",
                        "priority": 4,
                    },
                ]
            )
            ...
            base_docs.extend(
                [
                    {
                        "file": "2025_UPDATE_GUIDE.md",
                        "name": "Production Update Guide",
                        "priority": 3,
                    }
                ]
            )
    ...
    def _optimize_context(
        ...
        if "data" in optimized_context:
            optimized_context["data"] = self._compress_data_info(
                optimized_context["data"], optimization_ratio
            )

        # 2. Limit notebook descriptions
        if "notebooks" in optimized_context:
            optimized_context["notebooks"] = self._compress_notebook_info(
                optimized_context["notebooks"], optimization_ratio
            )

        # 3. Limit model information
        if "models" in optimized_context:
            optimized_context["models"] = self._compress_model_info(
                optimized_context["models"], optimization_ratio
            )

        # 4. Prioritize features
        if "features" in optimized_context:
            optimized_context["features"] = self._prioritize_features(
                optimized_context["features"], optimization_ratio
            )

        # 5. Compress documentation
        if "documentation" in optimized_context:
            optimized_context["documentation"] = self._compress_documentation(
                optimized_context["documentation"], optimization_ratio
            )
    ...
    def _compress_data_info(self, data: Dict[str, Any], ratio: float) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                compressed[key] = {
                    k: v for k, v in list(value.items())[: int(len(value) * ratio)]
                }
            else:
                compressed[key] = value
        return compressed

    def _compress_notebook_info(
        self, notebooks: List[Dict], ratio: float
    ) -> List[Dict]:
    ...
    def _compress_documentation(
        self, documentation: List[Dict], ratio: float
    ) -> List[Dict]:
    ...
    def _get_notebook_description(self, notebook_path: str) -> str:
        ...
        return descriptions.get(
            notebook_name, "Analytics notebook for college football data"
        )
    ...
    def _generate_cache_key(
        self, role: UserRole, request_context: Dict[str, Any]
    ) -> str:
        ...
        if "notebooks" in request_context:
            ...
            if isinstance(notebooks, list) and notebooks:
                key_elements.extend(
                    sorted(str(nb) for nb in notebooks[:3])
                )  # Limit to prevent long keys
            else:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "token_budgets": {
                role.value: budget for role, budget in self.token_budgets.items()
            },
    ...
    def get_context_summary(self, role: UserRole) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_entries": len(
                [k for k in self.context_cache.keys() if role.value in k]
            ),
        }
    ...
    def get_conversation_context(
        self, user_id: str, max_turns: int = 5
    ) -> Dict[str, Any]:
    ...
    def enhance_context_with_memory(
    ...
    def end_conversation_session(
        self, user_id: str, effectiveness_score: Optional[float] = None
        ...
        # Get session turns
        session_turns = [
            turn for turn in memory.recent_turns if turn.session_id == session_id
        ]
        ...
        summary = self._create_session_summary(
            session_id, user_id, session_turns, effectiveness_score
        )
        if summary is None:
            return None
        ...
        memory.average_session_length = sum(
            s.total_turns for s in memory.sessions
        ) / len(memory.sessions)
    ...
    def _extract_main_topic(self, query: str) -> str:
        ...
        if any(keyword in query_lower for keyword in ["predict", "outcome", "winner"]):
            ...
            keyword in query_lower for keyword in ["model", "algorithm", "regression"]
        ):
            return "modeling"
        elif any(keyword in query_lower for keyword in ["data", "dataset", "features"]):
            return "data_analysis"
        elif any(
            keyword in query_lower for keyword in ["learn", "tutorial", "explain"]
        ):
            return "learning"
    ...
    def _create_session_summary(
        ...
        topics_discussed = list(
            set(self._extract_main_topic(turn.query) for turn in turns)
        )
        ...
        for turn in turns:
            if any(
                keyword in turn.response.lower()
                for keyword in ["important", "key", "critical", "essential"]
            ):
                key_insights.append(
                    turn.response[:150] + "..."
    ...
    def _load_conversation_memory(self):
        ...
        try:
            ...
            if memory_file.exists():
                ...
                logger.info(
                    f"Loaded conversation memory for {len(self.conversation_memory)} users"
                )
