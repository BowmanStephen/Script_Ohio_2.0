...
class WeeklyModelValidationAgent(BaseAgent):
    ...
    def __init__(self, week: int, season: int = 2025, agent_id: Optional[str] = None):
        ...
        self.agent_description = (
            f"Validates Week {week} ML models and data compatibility."
        )
    ...
    def _check_data_compatibility(
        ...
        try:
            weekly_data = self._load_weekly_data()
            loaded_models = self._load_all_models()
            compatibility_results = self._validate_model_data_compatibility(
                loaded_models, weekly_data
            )
    ...
    def execute_task(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            individual_validations = self._validate_individual_models(
                loaded_models, weekly_data
            )
            ...
            cross_validation_results = self._cross_validate_models(
                loaded_models, weekly_data
            )

            # Step 7: Generate ensemble validation
            ensemble_validation = self._validate_ensemble_performance(
                loaded_models, weekly_data
            )
            ...
            if self.week > 1:
                previous_week_validation = self._validate_previous_week_results(
                    loaded_models
                )
    ...
    def _load_weekly_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log_info(
                f"Loaded {len(features_df)} feature records with {len(features_df.columns)} columns from {features_path}"
            )
            ...
            try:
                ...
                self.log_info(
                    f"Loaded training data with {len(training_df)} records from {training_data_path}"
                )
            except FileNotFoundError:
                training_df = None
    ...
    def _validate_weekly_data(self, weekly_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "features" in weekly_data:
            ...
            if len(features_df.columns) < 86:
                validation_results["issues"].append(
                    f"Insufficient features: {len(features_df.columns)} < 86"
                )
                validation_results["is_valid"] = False
            else:
                validation_results["feature_completeness"] = min(
                    1.0, len(features_df.columns) / 86
                )
            ...
            if not high_missing.empty:
                validation_results["warnings"].append(
                    f"High missing data in: {list(high_missing.index)}"
                )
            ...
            if len(numeric_columns) < 80:
                validation_results["warnings"].append(
                    f"Low numeric feature count: {len(numeric_columns)}"
                )
        ...
        if "games" in weekly_data and weekly_data["games"] is not None:
            ...
            missing_columns = [
                col for col in required_columns if col not in games_df.columns
            ]

            if missing_columns:
                validation_results["warnings"].append(
                    f"Missing game columns: {missing_columns}"
                )
        ...
        if "training_data" in weekly_data and weekly_data["training_data"] is not None:
            ...
            if feature_overlap < 0.8:
                validation_results["warnings"].append(
                    f"Low feature overlap: {feature_overlap:.1%}"
                )

        self.log_info(
            f"Week {self.week} data validation completed. Score: {validation_results['data_quality_score']:.1f}"
        )
    ...
    def _load_all_models(self) -> Dict[str, Any]:
        ...
        self.log_info(
            f"Loaded {sum(1 for m in models.values() if m['load_status'] == 'success')}/{len(models)} models"
        )
        return models
    ...
    def _load_fastai_model(self, file_path: str) -> Any:
        ...
        try:
            ...
            raise RuntimeError(
                f"Unable to load FastAI model at {file_path}: {exc}"
            ) from exc
    ...
    def _check_model_availability(
        ...
        self.log_info(
            f"Model availability: {availability_results['available_models']}/{availability_results['total_models']} ready"
        )
    ...
    def _get_feature_defaults(self, features: List[str]) -> Dict[str, float]:
        ...
        try:
            ...
            if usecols:
                ...
                self._feature_defaults.update(
                    {k: float(v) for k, v in medians.items() if pd.notna(v)}
                )
        except Exception as exc:
    ...
    def _align_features_to_schema(
        self, features_df: pd.DataFrame, canonical: List[str]
    ) -> pd.DataFrame:
        ...
        missing_cols = [
            feature for feature in canonical if feature not in features_df.columns
        ]
        if missing_cols:
            preview = ", ".join(missing_cols[:5])
            suffix = "..." if len(missing_cols) > 5 else ""
            self.log_warning(
                f"Aligned features using defaults for missing columns: {preview}{suffix}"
            )

        return aligned

    def _determine_model_features(
        self, model_info: Optional[Dict[str, Any]]
    ) -> List[str]:
        ...
        model_type = (
            model_info["config"].get("model_type") if model_info.get("config") else None
        )
    ...
    def _prepare_features_for_model(
        ...
        if model_info and model_info.get("config", {}).get("model_type") == "fastai":
            prepared_df = self._prepare_fastai_features(
                features_df, model_info, target_features
            )
            ...
            prepared_df = ModelExecutionEngine.prepare_training_features(
                features_df, exclude_metadata=True
            )

        self._record_model_feature_schema(
            model_name, target_features, features_df, prepared_df
        )
        return prepared_df
    ...
    def _ensure_numeric_only(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(object_columns) > 0:
            self.log_warning(
                f"Removing object columns from features: {list(object_columns)}"
            )
    ...
    def _prepare_fastai_features(
        ...
        for cat in cat_names:
            ...
            if pd.api.types.is_bool_dtype(series):
                ...
                aligned[cat] = (
                    series.fillna("unknown").replace("", "unknown").astype(str)
                )
    ...
    def _record_model_feature_schema(
        ...
        missing_features = [
            feature for feature in derived_features if feature not in source_df.columns
        ]
        schema_entry = {
        ...
        if missing_features:
            ...
            self.log_info(
                f"{model_name}: derived {schema_entry['derived_feature_count']} feature columns"
            )

    def _generate_predictions(
        self, model_info: Dict[str, Any], input_df: pd.DataFrame
        ...
        if model_type == "fastai":
            ...
            if (
                ...
                and ((probability_array < 0) | (probability_array > 1)).any()
            ):
                self.log_warning(
                    "FastAI probabilities out of [0,1]; clipping to valid range"
                )
    ...
    def _validate_model_data_compatibility(
        self, loaded_models: Dict[str, Any], weekly_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for model_name, model_info in loaded_models.items():
            ...
            try:
                prepared_features = self._prepare_features_for_model(
                    features_df, model_info, model_name
                )
                if prepared_features.empty:
                    raise ValueError("Unable to align features for model")

                sample_input = prepared_features.iloc[:1]
                predictions, prediction_type = self._generate_predictions(
                    model_info, sample_input
                )
    ...
    def _validate_individual_models(
        self, loaded_models: Dict[str, Any], weekly_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for model_name, model_info in loaded_models.items():
            ...
            try:
                model_features = self._prepare_features_for_model(
                    raw_features, model_info, model_name
                )
                ...
                validation_result = self._validate_single_model(
                    model_name, model_info, model_features
                )
                ...
                self.log_info(
                    f"{status} {model_name} validation: {validation_result.get('validation_score', 0):.1f}/100"
                )
    ...
    def _validate_single_model(
        self, model_name: str, model_info: Dict[str, Any], features_df: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                if len(predictions) > 1:
                    ...
                    if prediction_variance == 0:
                        validation_result["issues_found"].append(
                            "All predictions identical - potential overfitting"
                        )
            ...
            try:
                if predictions.size > 0 and prediction_type == "probability":
                    if not (
                        0 <= predictions.min() and predictions.max() <= 1.01
                    ):  # Small tolerance
                        validation_result["issues_found"].append(
                            "Probability predictions outside [0,1] range"
                        )
                        validation_result["validation_status"] = "warning"
            ...
            validation_result["validation_score"] = round(
                (net_passes / total_checks) * 100, 1
            )
    ...
    def _test_model_performance(
        ...
        for model_name, model_info in loaded_models.items():
            ...
            try:
                ...
                performance_result = self._test_single_model_performance(
                    model_name, model_info, model_features
                )
    ...
    def _test_single_model_performance(
        self, model_name: str, model_info: Dict[str, Any], X: pd.DataFrame
    ) -> Dict[str, Any]:
            ...
            if prediction_type == "probability":
                ...
                performance_result["performance_indicators"] = {
                    ...
                    "high_confidence_rate": np.mean(
                        (predictions > 0.7) | (predictions < 0.3)
                    ),
                }
    ...
    def _cross_validate_models(
        ...
        for model_name, model_info in loaded_models.items():
            ...
            try:
                cv_result = self._cross_validate_single_model(
                    model_name, model_info, training_df
                )
                ...
                cv_results[model_name] = {"cv_status": "error", "reason": str(e)}
    ...
    def _cross_validate_single_model(
        self, model_name: str, model_info: Dict[str, Any], training_df: pd.DataFrame
    ) -> Dict[str, Any]:
        """Cross-validate a single model"""

        cv_result = {"cv_status": "completed", "cv_folds": 5, "cv_scores": {}}

        try:
            # Prepare training data using only numeric home/away features to avoid dtype issues
            feature_columns = [
                col for col in training_df.columns if col.startswith(("home_", "away_"))
            ]
            if not feature_columns:
                raise ValueError(
                    "No home/away feature columns available for cross-validation"
                )

            candidate_df = training_df[feature_columns]
            numeric_columns = candidate_df.select_dtypes(
                include=[np.number]
            ).columns.tolist()
            if not numeric_columns:
                raise ValueError(
                    "No numeric home/away features available for cross-validation"
                )
            ...
            if model_info["config"]["prediction_type"] == "margin":
                ...
                y = training_df.get(
                    "margin", training_df["home_points"] - training_df["away_points"]
                )
            else:
                # For win probability
                y = (training_df["home_points"] > training_df["away_points"]).astype(
                    int
                )
            ...
            cv_scores = cross_val_score(
                model_info["model"], X, y, cv=5, scoring=scoring[0]
            )
        ...
        try:
            ...
            if len(available_models) < 2:
                ...
                ensemble_result["reason"] = (
                    f"Need at least 2 models, got {len(available_models)}"
                )
            ...
            if len(available_models) >= 2:
                ensemble_predictions = self._create_averaging_ensemble(
                    available_models, loaded_models, X
                )
    ...
    def _create_averaging_ensemble(
        self, model_names: List[str], loaded_models: Dict[str, Any], X: pd.DataFrame
    ) -> np.ndarray:
    ...
    def _analyze_model_biases(
        ...
        try:
            ...
            if "features" in weekly_data and weekly_data["features"] is not None:
                bias_checks = self._check_home_team_bias(
                    loaded_models, weekly_data["features"]
                )
    ...
    def _check_home_team_bias(
        self, loaded_models: Dict[str, Any], features_df: pd.DataFrame
    ) -> Dict[str, Any]:
        """Check for home team bias in models"""

        bias_check = {"significant_bias": False, "bias_magnitude": 0}
    ...
    def _validate_previous_week_results(
        self, loaded_models: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        self.log_info(
            f"Validating models against Week {previous_week} actual results..."
        )
        ...
        try:
            ...
            if os.path.exists(predictions_path):
                ...
                self.log_info(
                    f"Loaded {len(previous_predictions)} Week {previous_week} predictions"
                )
            else:
                self.log_warning(
                    f"Week {previous_week} predictions not found - skipping validation"
                )
                previous_week_validation["validation_status"] = "skipped"
                previous_week_validation["reason"] = (
                    f"Week {previous_week} predictions not available"
                )
            ...
            if os.path.exists(games_path):
                ...
                if not previous_actuals.empty:
                    ...
                    self.log_info(
                        f"Loaded {len(previous_actuals)} Week {previous_week} completed games"
                    )
                else:
                    self.log_warning(
                        f"Week {previous_week} actual results not yet available"
                    )
                    previous_week_validation["validation_status"] = "pending"
                    previous_week_validation["reason"] = (
                        f"Week {previous_week} games not yet completed"
                    )
                    return previous_week_validation
            else:
            ...
            if (
                previous_week_validation["predictions_available"]
                and previous_week_validation["actuals_available"]
            ):
                ...
                for _, pred in previous_predictions.iterrows():
                    ...
                    actual = previous_actuals[
                        (
                            (previous_actuals["home_team"] == home_team)
                            & (previous_actuals["away_team"] == away_team)
                        )
                        | (
                            (previous_actuals["home_team"] == away_team)
                            & (previous_actuals["away_team"] == home_team)
                        )
                    ]
                    ...
                    if not actual.empty:
                        ...
                        actual_winner = (
                            actual_row["home_team"]
                            if actual_row["home_points"] > actual_row["away_points"]
                            else actual_row["away_team"]
                        )
                ...
                if total_predictions > 0:
                    ...
                    self.log_info(
                        f"Week {previous_week} prediction accuracy: {accuracy:.1%} ({correct_predictions}/{total_predictions})"
                    )
    ...
    def _generate_validation_report(
        ...
        model_score = (
            (
                ...
                / model_availability["total_models"]
            )
            * 100
            if model_availability["total_models"] > 0
            else 0
        )
        ...
        individual_score = (
            np.mean(
                [
                    v.get("validation_score", 0)
                    for v in individual_validations.values()
        ...
        models_ready = [
            ...
            if result.get("compatible", False)
            and individual_validations.get(name, {}).get("validation_status")
            in ["passed", "warning"]
        ]
        ...
        if data_score < 80:
            recommendations.append(
                "Improve data quality - address missing values and feature completeness"
            )

        if model_score < 80:
            recommendations.append(
                "Ensure all models are properly loaded and functional"
            )
        ...
        if overall_score < 70:
            recommendations.append(
                "Major validation issues found - comprehensive review required"
            )
        elif overall_score < 85:
            recommendations.append(
                "Minor validation issues found - consider improvements"
            )

        # Add recommendations based on previous week validation
        if (
            previous_week_validation
            and previous_week_validation.get("validation_status") == "completed"
        ):
            accuracy = previous_week_validation.get("accuracy_metrics", {}).get(
                "accuracy", 0
            )
            if accuracy < 0.5:
                recommendations.append(
                    f"⚠️ Previous week (Week {previous_week_validation.get('previous_week')}) accuracy was {accuracy:.1%} - models may need retraining"
                )
            elif accuracy < 0.6:
                recommendations.append(
                    f"Previous week (Week {previous_week_validation.get('previous_week')}) accuracy was {accuracy:.1%} - consider model adjustments"
                )
            else:
                recommendations.append(
                    f"✅ Previous week (Week {previous_week_validation.get('previous_week')}) accuracy was {accuracy:.1%} - models performing well"
                )
    ...
    def _generate_next_steps(self, validation_report: Dict[str, Any]) -> List[str]:
        ...
        if validation_report["summary"]["overall_validation_passed"]:
            ...
            next_steps.append(
                "❌ Address validation issues before proceeding with predictions"
            )

        if validation_report["summary"]["models_available"] < 3:
            next_steps.append(
                "🔧 Attempt to load missing models or retrain failed models"
            )

        if (
            validation_report["detailed_results"]["data_validation"][
                "data_quality_score"
            ]
            < 85
        ):
    ...
    def _save_validation_results(self, validation_report: Dict[str, Any]) -> None:
        ...
        report_path = os.path.join(
            output_dir, f"week{self.week}_model_validation_report.json"
        )
