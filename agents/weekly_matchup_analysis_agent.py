...
class WeeklyMatchupAnalysisAgent(BaseAgent):
    ...
    def __init__(self, week: int, season: int = 2025, agent_id: Optional[str] = None):
        ...
        self.agent_description = (
            f"Analyzes Week {week} matchups and produces strategic insights."
        )
    ...
    def execute_task(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            matchup_insights = self._generate_matchup_insights(
                enhanced_data, strength_metrics
            )
    ...
    def _load_enhanced_weekly_data(self) -> Dict[str, Any]:
        ...
        try:
            enhanced_data_path = get_weekly_enhanced_file(
                self.week, "games", self.season
            )
            enhanced_games = pd.read_csv(enhanced_data_path)
            self.log_info(
                f"Loaded {len(enhanced_games)} enhanced games from {enhanced_data_path}"
            )
        except FileNotFoundError:
            raise FileNotFoundError(
                f"Enhanced Week {self.week} games data not found. Run mock enhancement agent first."
            )
        ...
        try:
            ...
            self.log_info(
                f"Loaded {len(features_data)} feature records from {features_path}"
            )
        except FileNotFoundError:
        ...
        try:
            ...
            self.log_info(
                f"Loaded {len(training_data)} completed games from training data at {training_data_path}"
            )
        except FileNotFoundError:
            self.log_info(
                f"Warning: Training data not found, win-loss calculations may be limited"
            )
    ...
    def _analyze_head_to_head_matchups(
        ...
        for _, game in enhanced_data["games"].iterrows():
            ...
            head_to_head_results[matchup_key] = {
                ...
                "rivalry_intensity": self._calculate_rivalry_intensity(
                    home_team, away_team
                ),
            }

        self.log_info(
            f"Analyzed head-to-head data for {len(head_to_head_results)} matchups"
        )
        return head_to_head_results

    def _calculate_team_strength_metrics(
        self, enhanced_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        # Get unique teams
        all_teams = set(
            enhanced_data["games"]["home_team"].tolist()
        ...
        for team in all_teams:
            ...
            if not team_games.empty:
                ...
                strength_metrics[team] = {
                    ...
                    "special_teams_rating": self._calculate_special_teams_rating(
                        team, enhanced_data
                    ),
                    "overall_rating": self._calculate_overall_rating(
                        team, enhanced_data
                    ),
                    "strength_of_schedule": self._calculate_strength_of_schedule(
                        team, enhanced_data
                    ),
                    ...
                    "clutch_performance": self._calculate_clutch_performance(
                        team, enhanced_data
                    ),
                    "adjusted_rating": self._calculate_adjusted_rating(
                        team, enhanced_data
                    ),
                }
    ...
    def _generate_matchup_insights(
        self, enhanced_data: Dict[str, Any], strength_metrics: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for _, game in enhanced_data["games"].iterrows():
            ...
            matchup_insight = {
                ...
                "home_team": home_team,
                "away_team": away_team,
                "key_matchups": self._identify_key_positional_matchups(
                    home_team, away_team, strength_metrics
                ),
                "offensive_comparison": self._compare_offenses(
                    home_team, away_team, strength_metrics
                ),
                "defensive_comparison": self._compare_defenses(
                    home_team, away_team, strength_metrics
                ),
                "special_teams_battle": self._compare_special_teams(
                    home_team, away_team, strength_metrics
                ),
                "coaching_matchup": self._analyze_coaching_matchup(
                    home_team, away_team
                ),
                "x_factors": self._identify_x_factors(
                    home_team, away_team, strength_metrics
                ),
                "game_flow_prediction": self._predict_game_flow(
                    home_team, away_team, strength_metrics
                ),
                "critical_situations": self._analyze_critical_situations(
                    home_team, away_team, strength_metrics
                ),
            }
    ...
    def _analyze_situational_factors(
        ...
        for _, game in enhanced_data["games"].iterrows():
            ...
            # Weather analysis
            situational_analysis["weather_impacts"][game_id] = (
                self._analyze_weather_impact(game)
            )

            # Travel factors
            situational_analysis["travel_factors"][game_id] = (
                self._analyze_travel_factors(home_team, away_team)
            )

            # Motivational factors
            situational_analysis["motivational_factors"][game_id] = (
                self._analyze_motivation_factors(game)
            )

            # Injury situations
            situational_analysis["injury_situations"][game_id] = (
                self._estimate_injury_situation(home_team, away_team)
            )

            # Playoff implications
            situational_analysis["playoff_implications"][game_id] = (
                self._analyze_playoff_implications(game)
            )

            # Rivalry factors
            situational_analysis["rivalry_factors"][game_id] = (
                self._analyze_rivalry_factors(home_team, away_team)
            )

        self.log_info(
            f"Analyzed situational factors for {len(situational_analysis['weather_impacts'])} games"
        )
        return situational_analysis

    def _calculate_advanced_matchup_stats(
        self, enhanced_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for _, game in enhanced_data["games"].iterrows():
            ...
            advanced_stats[game_id] = {
                ...
                "scoring_probability": self._calculate_scoring_probability(
                    home_team, away_team
                ),
                "turnover_projection": self._project_turnovers(home_team, away_team),
                "explosive_play_probability": self._calculate_explosive_play_probability(
                    home_team, away_team
                ),
                "third_down_efficiency_projection": self._project_third_down_efficiency(
                    home_team, away_team
                ),
                "red_zone_efficiency_projection": self._project_red_zone_efficiency(
                    home_team, away_team
                ),
                "time_of_possession_prediction": self._predict_time_of_possession(
                    home_team, away_team
                ),
    ...
    def _generate_strategic_recommendations(
        ...
        for _, game in enhanced_data["games"].iterrows():
            ...
            game_insight = next(
                (
                    insight
                    for insight in matchup_insights
                    if insight["game_id"] == game_id
                ),
                {},
            )
            ...
            recommendation = {
                ...
                "home_team": home_team,
                "away_team": away_team,
                "winner_recommendation": self._predict_winner(
                    home_team, away_team, strength_metrics
                ),
                ...
                "over_under_analysis": self._analyze_over_under(
                    home_team, away_team, strength_metrics
                ),
                ...
                "key_factors": self._identify_key_factors(
                    home_team, away_team, strength_metrics, game_insight
                ),
                "strategic_advantages": self._identify_strategic_advantages(
                    home_team, away_team, strength_metrics
                ),
                "game_script_prediction": self._predict_game_script(
                    home_team, away_team, strength_metrics
                ),
            }
        ...
        self.log_info(
            f"Generated strategic recommendations for {len(recommendations)} games"
        )
    ...
    def _compile_comprehensive_analysis(
        ...
        comprehensive_analysis = {
            ...
            "team_rankings": self._generate_power_rankings(
                strength_metrics, enhanced_data
            ),
            "top_matchups": self._identify_top_matchups(
                enhanced_data, strategic_recommendations
            ),
            "upset_alerts": self._identify_upset_alerts(strategic_recommendations),
            "playoff_implications": self._analyze_playoff_picture(
                enhanced_data, strategic_recommendations
            ),
            "conference_races": self._analyze_conference_races(
                enhanced_data, strength_metrics
            ),
            "heisman_implications": self._analyze_heisman_implications(
                enhanced_data, strength_metrics
            ),
            "coaching_hot_seat": self._analyze_coaching_situations(
                enhanced_data, strength_metrics
            ),
            "bowl_projections": self._project_bowl_implications(
                enhanced_data, strategic_recommendations
            ),
            "detailed_analysis": {
    ...
    def _save_analysis_results(self, comprehensive_analysis: Dict[str, Any]) -> None:
        ...
        analysis_path = os.path.join(
            output_dir, f"week{self.week}_comprehensive_analysis.json"
        )
        ...
        if comprehensive_analysis["detailed_analysis"]["recommendations"]:
            recommendations_df = pd.DataFrame(
                comprehensive_analysis["detailed_analysis"]["recommendations"]
            )
            rec_path = os.path.join(
                output_dir, f"week{self.week}_strategic_recommendations.csv"
            )
    ...
    def _get_historical_h2h_record(
    ...
    def _calculate_momentum_advantage(self, recent_form: Dict[str, Any]) -> str:
        ...
        home_points = (
            recent_form["home_team"]["points_per_game"]
            - recent_form["home_team"]["points_allowed_per_game"]
        )
        away_points = (
            recent_form["away_team"]["points_per_game"]
            - recent_form["away_team"]["points_allowed_per_game"]
        )
    ...
    def _calculate_rivalry_intensity(self, home_team: str, away_team: str) -> float:
        ...
        try:
            ...
            matchup_count = sum(
                1
                for h, a in popular_matchups
                if (h == home_team and a == away_team)
                or (h == away_team and a == home_team)
            )
    ...
    def _calculate_offensive_rating(self, team: str, data: Dict[str, Any]) -> float:
        ...
        avg_epa = np.mean(
            [h for h in [home_epa, away_epa] if not pd.isna(h) and h != 0.0]
        )
    ...
    def _calculate_defensive_rating(self, team: str, data: Dict[str, Any]) -> float:
        ...
        # Use features if games don't have the column
        if (
            not enhanced_games.empty
            and "home_adjusted_epa_allowed" in enhanced_games.columns
        ):
            data_source = enhanced_games
        elif (
            not features_data.empty
            and "home_adjusted_epa_allowed" in features_data.columns
        ):
            data_source = features_data
        else:
            return 75.0  # Default neutral rating
        ...
        home_epa_allowed = (
            home_games["home_adjusted_epa_allowed"].mean()
            if not home_games.empty
            and "home_adjusted_epa_allowed" in home_games.columns
            else 0.0
        )
        away_epa_allowed = (
            away_games["away_adjusted_epa_allowed"].mean()
            if not away_games.empty
            and "away_adjusted_epa_allowed" in away_games.columns
            else 0.0
        )

        # Average EPA allowed (lower is better, so we invert)
        avg_epa_allowed = np.mean(
            [
                h
                for h in [home_epa_allowed, away_epa_allowed]
                if not pd.isna(h) and h != 0.0
            ]
        )
    ...
    def _calculate_overall_rating(self, team: str, data: Dict[str, Any]) -> float:
        ...
        # Average Elo
        home_elo = (
            home_games["home_elo"].mean()
            if not home_games.empty and "home_elo" in home_games.columns
            else 1500.0
        )
        away_elo = (
            away_games["away_elo"].mean()
            if not away_games.empty and "away_elo" in away_games.columns
            else 1500.0
        )
        ...
        # Average talent
        home_talent = (
            home_games["home_talent"].mean()
            if not home_games.empty and "home_talent" in home_games.columns
            else 500.0
        )
        away_talent = (
            away_games["away_talent"].mean()
            if not away_games.empty and "away_talent" in away_games.columns
            else 500.0
        )
    ...
    def _calculate_strength_of_schedule(self, team: str, data: Dict[str, Any]) -> float:
        ...
        team_games = enhanced_games[
            (enhanced_games["home_team"] == team)
            | (enhanced_games["away_team"] == team)
        ]
    ...
    def _calculate_adjusted_rating(self, team: str, data: Dict[str, Any]) -> float:
        ...
        consistency_adjustment = (
            consistency - 0.75
        ) * 5.0  # ±1.25 points based on consistency

        adjusted = overall + sos_adjustment + consistency_adjustment
        return max(0.0, min(100.0, adjusted))

    def _identify_key_positional_matchups(
        self, home: str, away: str, metrics: Dict
    ) -> List[str]:
        return ["QB vs Secondary", "OL vs DL", "WR vs CB", "LB vs RB"]

    def _compare_offenses(self, home: str, away: str, metrics: Dict) -> Dict[str, Any]:
        return {
            "advantage": home if random.random() > 0.5 else away,
            "margin": random.uniform(1, 10),
        }
    ...
    def _analyze_coaching_matchup(self, home: str, away: str) -> Dict[str, Any]:
        return {
            "advantage": home if random.random() > 0.5 else away,
            "experience_edge": random.uniform(0.8, 1.2),
        }
    ...
    def _predict_game_flow(self, home: str, away: str, metrics: Dict) -> str:
        return random.choice(
            ["High-scoring shootout", "Defensive struggle", "Balanced game"]
        )

    def _analyze_critical_situations(
        self, home: str, away: str, metrics: Dict
    ) -> List[str]:
    ...
    def _analyze_motivation_factors(self, game: Dict) -> Dict[str, Any]:
        return {
            "home_motivation": random.uniform(0.8, 1.2),
            "away_motivation": random.uniform(0.8, 1.2),
        }

    def _estimate_injury_situation(self, home: str, away: str) -> Dict[str, Any]:
        return {
            "home_impact": random.uniform(0.9, 1.0),
            "away_impact": random.uniform(0.9, 1.0),
        }
    ...
    def _analyze_rivalry_factors(self, home: str, away: str) -> Dict[str, Any]:
        return {
            ...
            "historical_significance": random.uniform(0.8, 1.2),
        }

    def _predict_game_pace(self, home: str, away: str) -> Dict[str, Any]:
        return {
            "plays_per_game": random.randint(140, 180),
            "tempo": random.choice(["Fast", "Medium", "Slow"]),
        }
    ...
    def _project_third_down_efficiency(self, home: str, away: str) -> Dict[str, Any]:
        return {
            "home_rate": random.uniform(0.35, 0.5),
            "away_rate": random.uniform(0.35, 0.5),
        }

    def _project_red_zone_efficiency(self, home: str, away: str) -> Dict[str, Any]:
        return {
            "home_rate": random.uniform(0.7, 0.9),
            "away_rate": random.uniform(0.7, 0.9),
        }

    def _predict_time_of_possession(self, home: str, away: str) -> Dict[str, Any]:
        return {
            "home_minutes": random.randint(28, 35),
            "away_minutes": random.randint(25, 32),
        }

    def _project_penalties(self, home: str, away: str) -> Dict[str, Any]:
        return {
            "home_penalties": random.randint(5, 12),
            "away_penalties": random.randint(5, 12),
        }
    ...
    def _project_big_plays(self, home: str, away: str) -> Dict[str, Any]:
        return {
            "home_big_plays": random.randint(3, 8),
            "away_big_plays": random.randint(3, 8),
        }
    ...
    def _predict_score(self, home: str, away: str, metrics: Dict) -> Dict[str, int]:
        return {
            ...
            "away_score": random.randint(15, 40),
        }
    ...
    def _analyze_spread(self, home: str, away: str, metrics: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendation": "Home" if spread < 0 else "Away",
        }
    ...
    def _analyze_over_under(
        ...
        return {
            ...
            "recommendation": "Over" if total > 55 else "Under",
        }
    ...
    def _generate_betting_recommendations(
        ...
        return random.sample(
            ["Home ATS", "Away ML", "Over Total", "Under Total", "Prop Bet"], 2
        )
    ...
    def _identify_key_factors(
        self, home: str, away: str, metrics: Dict, insight: Dict
    ) -> List[str]:
        return random.sample(
            ["Turnover margin", "Third downs", "Red zone", "Special teams"], 3
        )
    ...
    def _predict_game_script(self, home: str, away: str, metrics: Dict) -> str:
        return random.choice(
            ["High-scoring", "Defensive battle", "Close game", "Blowout"]
        )

    def _calculate_matchup_metrics(
        self, home_team: str, away_team: str, enhanced_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "home_team_strength": strength_metrics.get(home_team, {}).get(
                "overall_rating", 75.0
            ),
            "away_team_strength": strength_metrics.get(away_team, {}).get(
                "overall_rating", 75.0
            ),
            "offensive_comparison": {
                "home_rating": strength_metrics.get(home_team, {}).get(
                    "offensive_rating", 75.0
                ),
                "away_rating": strength_metrics.get(away_team, {}).get(
                    "offensive_rating", 75.0
                ),
            },
    ...
    def _generate_power_rankings(
        self, strength_metrics: Dict, enhanced_data: Optional[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        ...
        if enhanced_data and "training_data" in enhanced_data:
            ...
            if os.path.exists(training_data_path):
                ...
                if self.season and "season" in training_data.columns:
                    training_data = training_data[
                        training_data["season"] == self.season
                    ]
                    self.log_info(
                        f"Filtered to {self.season} season: {len(training_data)} games"
                    )
        ...
        if training_data is not None and not training_data.empty:
            for team in strength_metrics.keys():
                team_games = training_data[
                    (training_data["home_team"] == team)
                    | (training_data["away_team"] == team)
                ]
    ...
    def _identify_top_matchups(
        self, data: Dict, recommendations: List[Dict]
    ) -> List[Dict[str, Any]]:
        # Return top 5 most interesting matchups
        return recommendations[:5]

    def _identify_upset_alerts(
        self, recommendations: List[Dict]
    ) -> List[Dict[str, Any]]:
        # Return games with high upset probability
        upset_games = [
            rec for rec in recommendations if rec.get("upset_probability", 0) > 0.3
        ]
        return upset_games[:3]

    def _analyze_playoff_picture(
        self, data: Dict, recommendations: List[Dict]
    ) -> Dict[str, Any]:
        return {
            "teams_in_hunt": random.randint(8, 12),
            "bubble_teams": random.randint(4, 6),
        }

    def _analyze_conference_races(
        self, data: Dict, strength_metrics: Dict
    ) -> Dict[str, Any]:
        return {
            "tight_races": random.randint(2, 4),
            "decided_races": random.randint(2, 5),
        }

    def _analyze_heisman_implications(
        self, data: Dict, strength_metrics: Dict
    ) -> Dict[str, Any]:
        return {
            "candidates_affected": random.randint(3, 6),
            "boost_opportunities": random.randint(1, 3),
        }

    def _analyze_coaching_situations(
        self, data: Dict, strength_metrics: Dict
    ) -> Dict[str, Any]:
        return {
            "hot_seat_games": random.randint(1, 3),
            "contract_extension_games": random.randint(2, 4),
        }

    def _project_bowl_implications(
        self, data: Dict, recommendations: List[Dict]
    ) -> Dict[str, Any]:
        return {
            "teams_affected": random.randint(15, 25),
            "bowl_jumps": random.randint(3, 6),
        }
