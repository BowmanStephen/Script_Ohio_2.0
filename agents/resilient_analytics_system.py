...
class ResilientDataLoader:
    ...
    @handle_errors(
        ...
        "Unable to load data. Using cached version.",
    )
    ...
    def load_team_data_fallback_secondary(
        self, season: str, **kwargs
    ) -> Dict[str, Any]:
...
class ResilientModelEngine:
    ...
    @handle_errors(
        "ModelInference",
        ErrorSeverity.HIGH,
        "Prediction unavailable. Using fallback model.",
    )
    ...
    def predict_game_outcome_backup_model(
        self, home_team: str, away_team: str, features: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
    ...
    def predict_game_outcome_simple_model(
        self, home_team: str, away_team: str, features: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
...
class ResilientAnalyticsAgent:
        ...
        try:
            ...
            try:
                ...
                prediction = self.model_engine.predict_game_outcome(
                    home_team=team_name, away_team="Opponent", features=features
                )
            ...
            logger.info(
                f"Analysis completed for {team_name}: {'SUCCESS' if error_count == 0 else 'DEGRADED'}"
            )
            return final_result
    ...
    def get_agent_metrics(self) -> Dict[str, Any]:
        ...
        avg_analysis_time = (
            sum(r["analysis_time"] for r in self.analysis_results) / total_analyses
        )
...
def demo_resilient_analytics():
    ...
    data_loader = ResilientDataLoader()
    model_engine = ResilientModelEngine()
    analytics_agent = ResilientAnalyticsAgent(
        "resilient_analyst_001", data_loader, model_engine
    )
    ...
    from src.utils.data import get_current_season, get_teams_from_data
    ...
    result1 = analytics_agent.analyze_team_performance(
        sample_team1, str(current_season)
    )
    ...
    result2 = analytics_agent.analyze_team_performance(
        sample_team2, str(current_season)
    )
    ...
    result3 = analytics_agent.analyze_team_performance(
        sample_team3, str(current_season)
    )
    ...
    result4 = analytics_agent.analyze_team_performance(
        sample_team4, str(current_season)
    )
    ...
    if error_metrics:
        ...
        print(
            f"   Active circuit breakers: {len(error_metrics.get('circuit_breakers', {}))}"
        )
        print(
            f"   Fallback strategies: {error_metrics.get('fallback_metrics', {}).get('registered_fallbacks', 0)}"
        )
