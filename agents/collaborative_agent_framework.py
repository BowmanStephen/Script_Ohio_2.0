...
try:
    ...
    from system.communication.secure_messaging import (
        create_message,
        register_agent,
        send_message,
    )
    ...
    logger.warning(
        "Legacy system.communication not available - collaborative features disabled"
    )
...
class CollaborativeAgent:
    ...
    def __init__(
        self, agent_id: str, capabilities: List[str], expertise_domains: List[str]
    ):
        ...
        if LEGACY_SYSTEM_AVAILABLE and collaboration_manager is not None:
            ...
            logger.warning(
                f"Collaborative agent {agent_id} initialized without legacy collaboration system"
            )
    ...
    def request_peer_review(
        self, work_to_review: Dict[str, Any], context: Dict[str, Any]
    ) -> str:
        """Request peer review of analysis work"""
        if not LEGACY_SYSTEM_AVAILABLE:
            logger.warning(
                "Peer review requested but legacy collaboration system not available"
            )
            return None
    ...
    def find_expert_help(
        self, required_expertise: str, context: Dict[str, Any]
    ) -> Optional[str]:
    ...
    def initiate_collaborative_analysis(
        self,
        analysis_objective: str,
        context: Dict[str, Any],
        collaboration_type: CollaborationType = CollaborationType.SWARM_ANALYSIS,
    ) -> str:
        """Initiate a collaborative analysis with multiple agents"""
        if not LEGACY_SYSTEM_AVAILABLE or collaboration_manager is None:
            logger.warning(
                "Collaborative analysis requested but legacy collaboration system not available"
            )
            return None
...
class EnhancedAnalyticsAgent(CollaborativeAgent):
    ...
    def analyze_with_collaboration(
        ...
        use_knowledge_sharing: bool = True,
    ) -> Dict[str, Any]:
        ...
        if use_peer_review:
            collaboration_id = self.request_peer_review(
                work_to_review=analysis_result, context=analysis_request
            )
...
def demo_collaborative_football_analysis():
    ...
    print(
        f"Data analysis completed with collaboration: {data_analysis['collaboration_enhanced']}"
    )
    ...
    print(
        f"Model development completed with peer review: {model_development['peer_review'] is not None}"
    )
    ...
    viz_analysis = viz_agent.analyze_with_collaboration(
        {
            ...
            "shared_knowledge_count": len(
                viz_agent.search_shared_knowledge("team rankings")
            ),
        }
    )

    print(
        f"Visualization enhanced with {len(viz_analysis['shared_knowledge'])} shared insights"
    )
