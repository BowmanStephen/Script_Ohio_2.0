...
class AnalyticsOrchestrator:
        ...
        if CFBDSubscriptionManager and os.getenv("CFBD_API_KEY"):
            try:
                self.subscription_manager = CFBDSubscriptionManager(
                    telemetry_hook=self._record_cfbd_event, use_websockets=True
                )
            ...
            logger.info(
                "CFBDSubscriptionManager not available - CFBD features disabled"
            )
    ...
    def _initialize_cfbd_provider(self) -> Optional["CFBDDataProvider"]:
        ...
        if not CFBDDataProvider:
            logger.warning(
                "CFBDDataProvider not available - CFBD integration will be limited"
            )
            return None
        ...
        if not api_key:
            logger.warning(
                "CFBD_API_KEY not set - skipping CFBD provider initialization"
            )
            return None
    ...
    def _load_agents(self):
        ...
        try:
            ...
            self.agent_factory.register_agent_class(
                ModelExecutionEngine, "model_engine"
            )
            self.agent_factory.register_agent_class(
                InsightGeneratorAgent, "insight_generator"
            )
            ...
            self.agent_factory.register_agent_class(
                LearningNavigatorAgent, "learning_navigator"
            )
            ...
            self.agent_factory.create_agent(
                "learning_navigator", "default_learning_nav"
            )
            ...
            try:
                ...
                from agents.weekly_analysis_orchestrator import (
                    WeeklyAnalysisOrchestrator,
                )

                self.agent_factory.register_agent_class(
                    CFBDIntegrationAgent, "cfbd_integration"
                )
                self.agent_factory.register_agent_class(
                    QualityAssuranceAgent, "quality_assurance"
                )
                self.agent_factory.register_agent_class(
                    PostseasonProjectionAgent, "postseason_projection"
                )
                self.agent_factory.register_agent_class(
                    WeeklyAnalysisOrchestrator, "weekly_analysis_orchestrator"
                )
    ...
    def process_analytics_request(self, request: AnalyticsRequest) -> AnalyticsResponse:
        ...
        try:
            ...
            agent_responses = self._execute_agent_requests(
                request_id, required_agents, request, context
            )

            # 4. Synthesize results into high-level response
            response = self._synthesize_response(
                request_id, request, agent_responses, context
            )
            ...
            self._store_session_interaction(
                request, response, context.get("role", "analyst"), context
            )

            self._log_health_event(request, agent_responses, execution_time)

            logger.info(
                f"Completed analytics request {request_id} in {execution_time:.2f}s"
            )
            return response
    ...
    def _prepare_shared_datasets(
        self, request: AnalyticsRequest, context: Dict[str, Any]
    ) -> None:
    ...
    def _fetch_cfbd_dataset(
        self, team: str, season: Optional[int], week: Optional[int]
        ...
        try:
            ...
            if snapshot and self.cache_manager:
                self.cache_manager.put(
                    cache_key, snapshot, ttl_seconds=300, tags=["cfbd", "shared"]
                )
            return snapshot
    ...
    def _log_health_event(
        ...
        event = {
            ...
            "failure_count": len(
                [resp for resp in agent_responses if resp.status == AgentStatus.ERROR]
            ),
            "execution_time": execution_time,
        }
    ...
    def _analyze_request_requirements(
        self, request: AnalyticsRequest, context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        if "week13" in query_lower or request.parameters.get("week") == 13:
            if (
                "consolidate" in query_lower
                or "catalog" in query_lower
                or "organize" in query_lower
            ):
                requirements.append(
                    {
                ...
                "legacy" in query_lower
                or "template" in query_lower
            ...
            requirements.append(
                {
                    ...
                    "parameters": {
                        ...
                        "skill_level": request.context_hints.get(
        ...
        if any(
            keyword in query_lower
            for keyword in ["data", "dataset", "explore", "show me", "tell me about"]
        ):
        ...
        if any(
            keyword in query_lower
            for keyword in ["predict", "model", "analyze", "forecast", "outcome"]
        ):
                ...
                requirements.append(
                    {
                        ...
                        "parameters": {
                            "query": request.query,
                            "teams": request.parameters.get("teams", []),
                            "model_type": request.parameters.get(
        ...
        if (
            any(keyword in query_lower for keyword in conversational_keywords)
            or len(query_lower.split()) > 10
        ):
            requirements.append(
                {
        ...
        if any(keyword in query_lower for keyword in analysis_keywords):
            ...
            if (
                target_role in ["data_scientist", "analyst", "production"]
                or skill_level in ["advanced", "intermediate", "expert"]
                or "analysis" in query_lower
                or "insights" in query_lower
                or "advanced" in query_lower
            ):
                requirements.append(
                    {
                        ...
                        "parameters": {
                            ...
                            "focus_areas": request.parameters.get(
                                "focus_areas", ["performance", "descriptive"]
                            ),
                        },
                    }
                )
        ...
        graphql_keywords = [
            "graphql",
            "gql",
            "scoreboard via graphql",
            "recruiting via graphql",
            "graphql api",
        ]
        if any(keyword in query_lower for keyword in graphql_keywords):
            season_param = request.parameters.get("season") or request.parameters.get(
                "year"
            )
            week_param = request.parameters.get("week")
            team_param = request.parameters.get("team") or request.parameters.get(
                "school"
            )
            ...
            if any(
                word in query_lower
                for word in ["scoreboard", "games", "game", "matchup"]
            ):
                requirements.append(
                    {
                ...
                word in query_lower for word in ["recruiting", "recruit", "recruits"]
            ):
                requirements.append(
                    {
                        "agent_type": "cfbd_integration",
        ...
        # CFBD ingestion requests (REST-based) - Only if GraphQL not specified
            ...
            keyword in query_lower
            for keyword in ["cfbd", "ingest", "next api", "rest api", "live data"]
        ):
            ...
            requirements.append(
                {
                    ...
                    "parameters": {
                        ...
                        "week": week_param,
                        "team": team_param,
                        "use_next": "next" in query_lower
                        or request.parameters.get("use_next", False),
                    },
                }
            )
        ...
        if any(
            keyword in query_lower
            for keyword in ["week", "weekly", "prediction", "matchup"]
        ):
            # Extract week number if present
            week = self._extract_week_from_query(
                request.query
            ) or request.parameters.get("week")
                ...
                requirements.append(
                    {
                        ...
                        "parameters": {"week": week, "season": season},
                        "priority": 1,
                    }
                )
                ...
                requirements.append(
                    {
                        ...
                        "parameters": {"week": 12, "season": season},
                        "priority": 1,
                    }
                )
            ...
            requirements.append(
                {
                    "agent_type": "learning_navigator",
                    "action": "recommend_content",
                    "parameters": {"query": request.query, "general_guidance": True},
                }
            )

        return requirements

    def _execute_agent_requests(
        self,
        request_id: str,
        ...
        priorities = [
            req.get("priority", analytics_request.priority) for req in required_agents
        ]

        logger.info(
            f"[ORCHESTRATOR_AUDIT] ROUTER_CALL: request_id={request_id} agent_count={agent_count} agent_types={agent_types} priorities={priorities}"
        )

        user_role = context.get("role", "analyst")
        # Check context hints for advanced users to grant appropriate permissions
        if user_role == "analyst" and (
            analytics_request.context_hints.get("skill_level") == "advanced"
            or analytics_request.context_hints.get("role") == "data_scientist"
        ):
        ...
        for agent_req in required_agents:
            try:
                ...
                if agent_type.startswith("weekly_"):
                    ...
                    if week is not None:
                        ...
                        if agent_id not in self.agent_factory.agents:
                            try:
                                self.agent_factory.create_agent(
                                    agent_type, agent_id, week=week, season=season
                                )
                                logger.info(
                                    f"Created weekly agent {agent_type} for week {week}, season {season}"
                                )
                            except Exception as exc:
                                logger.error(
                                    f"Failed to create weekly agent {agent_type}: {exc}"
                                )
                        ...
                        logger.warning(
                            f"Missing week parameter for weekly agent: {agent_type}"
                        )
                        # Create error response
                        error_response = AgentResponse(
                            request_id=f"{request_id}_{agent_type}",
                ...
                logger.info(
                    f"[ORCHESTRATOR_AUDIT] ROUTER_CALL: request_id={agent_request.request_id} action=submit"
                )
                self.request_router.submit_request(agent_request, user_permissions)

                logger.info(
                    f"[ORCHESTRATOR_AUDIT] ROUTER_CALL: request_id={agent_request.request_id} action=process"
                )
                ...
                logger.info(
                    f"[ORCHESTRATOR_AUDIT] ROUTER_CALL: request_id={agent_request.request_id} router_overhead_ms={router_overhead_ms:.2f}"
                )

                # Get response
                response = self.request_router.get_request_status(
        ...
        successful_agents = len(
            [r for r in agent_responses if r.status == AgentStatus.COMPLETED]
        )
        failed_agents = len(
            [r for r in agent_responses if r.status == AgentStatus.ERROR]
        )

        # Calculate router overhead (sum of all router calls)
        total_router_overhead_ms = (
            sum(router_overhead_times) if router_overhead_times else 0.0
        )

        logger.info(
            f"[ORCHESTRATOR_AUDIT] ROUTER_CALL: request_id={request_id} total_time_ms={total_time_ms:.2f} total_router_overhead_ms={total_router_overhead_ms:.2f} successful_agents={successful_agents} failed_agents={failed_agents}"
        )
    ...
    def _synthesize_response(
                ...
                if isinstance(response.result, dict):
                    ...
                    if "recommendations" in response.result:
                        for rec in response.result["recommendations"]:
                            insights.append(
                                f"Recommendation: {rec.get('reason', 'No reason provided')}"
                            )
                    if "recommended_path" in response.result:
                        insights.append(
                            f"Suggested learning path: {' → '.join(response.result['recommended_path'])}"
                        )
                    if "next_steps" in response.result:
                        insights.extend(
                            [
                                f"Next step: {step}"
                                for step in response.result["next_steps"]
                            ]
                        )
                    if "resources" in response.result:
                        resources = response.result["resources"]
                        insights.append(
                            f"Found {len(resources)} learning resources for you"
                        )
                        for resource in resources[
                            :3
                        ]:  # Add first 3 resources as insights
                            title = resource.get("title", "Untitled Resource")
                            difficulty = resource.get(
                    ...
                    if "insights" in response.result:
                        ...
                        if isinstance(insight_generator_insights, list):
                            for insight in insight_generator_insights[
                                :5
                            ]:  # Top 5 insights
                                if isinstance(insight, dict):
                                    ...
                                    if "confidence" in insight:
                                        insights.append(
                                            f"Analysis Insight ({insight['confidence']}% confidence): {insight_text}"
                                        )
                                    else:
                                        insights.append(
                                            f"Analysis Insight: {insight_text}"
                                        )
                                else:
                                    insights.append(f"Analysis Insight: {insight}")
                        else:
                            insights.append(
                                f"Analysis Insight: {insight_generator_insights}"
                            )
                    ...
                    if "analysis_results" in response.result:
                        ...
                        insights.append(
                            f"Analysis completed: {len(str(analysis))} characters of results"
                        )
                    ...
                    if "data_quality" in response.result:
                        ...
                        if isinstance(quality, dict):
                            ...
                            insights.append(
                                f"Data quality: {completeness:.1%} complete"
                            )
                    ...
                    for key, value in response.result.items():
                        if key not in [
                            ...
                            if (
                                ...
                                insights.append(
                                    f"Found {len(value)} {key.replace('_', ' ')}"
                                )
        ...
        for response in successful_responses:
            if response.result and isinstance(response.result, dict):
                ...
                if (
                    response.agent_type == "learning_navigator"
                    and "recommended_path" in response.result
                ):
                ...
                if (
                    response.agent_type == "insightgenerator"
                    or response.agent_type == "insight_generator"
                ):
                ...
                if (
                    response.agent_type == "modelengine"
                    or response.agent_type == "model_engine"
                ):
                    if "predictions" in response.result:
                        predictions = response.result["predictions"]
                        visualizations.append(
        ...
        if any("notebook" in str(result).lower() for result in successful_responses):
            learning_path = results.get("learning_navigator", {}).get(
                "recommended_path", []
            )
            if learning_path and not any(
                viz.get("type") == "learning_path" for viz in visualizations
            ):
                visualizations.append(
                    {
                        "type": "learning_path",
    ...
    def _store_session_interaction(
        self,
        request: AnalyticsRequest,
    ...
    def get_session_summary(self, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        avg_execution_time = (
            sum(h["execution_time"] for h in history) / total_interactions
        )
