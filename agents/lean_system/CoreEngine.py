...
class ModelManager:
    ...
    def predict_game_outcome(
        self, team1: str, team2: str, include_confidence: bool = True
    ) -> Dict[str, Any]:
...
class DataProcessor:
            ...
            matchup_stats = {
                "team1": {
                    ...
                    "defense": {
                        "ppg_allowed": 21.2,
                        "ypg_allowed": 358.9,
                        "pass_ypg_allowed": 224.1,
                        "rush_ypg_allowed": 134.8,
                    },
                    "misc": {
                        "turnover_margin": 1.2,
                        "penalties_per_game": 6.8,
                        "time_of_possession": 32.5,
                    },
                },
                "team2": {
                    "name": team2,
                    ...
                    "defense": {
                        "ppg_allowed": 23.7,
                        "ypg_allowed": 389.4,
                        "pass_ypg_allowed": 246.2,
                        "rush_ypg_allowed": 143.2,
                    },
                    "misc": {
                        "turnover_margin": 0.8,
                        "penalties_per_game": 7.2,
    ...
    def _analyze_advantages(self, matchup_stats: Dict[str, Any]) -> List[str]:
        ...
        if team1["offense"]["ppg"] > team2["offense"]["ppg"]:
            advantages.append(
                f"{team1['name']} has scoring advantage ({team1['offense']['ppg']:.1f} vs {team2['offense']['ppg']:.1f} PPG)"
            )

        if team2["offense"]["ypg"] > team1["offense"]["ypg"]:
            advantages.append(
                f"{team2['name']} has yardage advantage ({team2['offense']['ypg']:.1f} vs {team1['offense']['ypg']:.1f} YPG)"
            )

        # Defensive advantages
        if team1["defense"]["ppg_allowed"] < team2["defense"]["ppg_allowed"]:
            advantages.append(
                f"{team1['name']} has defensive advantage ({team1['defense']['ppg_allowed']:.1f} vs {team2['defense']['ppg_allowed']:.1f} PPG allowed)"
            )
...
class ContentGenerator:
    ...
    def generate_explanation(
        ...
        base_explanation = explanations.get(concept, {}).get(
            level, f"Content for {concept} at {level.value} level"
        )
        ...
        if context:
            ...
            if "sport" in context:
                context_additions.append(
                    f"In {context['sport']}, this concept applies to..."
                )
    ...
    def _extract_key_points(self, text: str) -> List[str]:
        ...
        for sentence in sentences:
            if any(
                ...
                for keyword in ["like", "involves", "employs", "includes", "techniques"]
            ):
    ...
    def generate_matchup_narrative(
        self, matchup_data: Dict[str, Any], predictions: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
    ...
    def _extract_talking_points(self, narrative_parts: List[str]) -> List[str]:
        ...
        for part in narrative_parts:
            if any(
                keyword in part
                for keyword in ["averaging", "defense", "offense", "favored"]
            ):
...
def test_core_engine():
    ...
    try:
        ...
        prediction_result = engine._handle_prediction(
            {"team1": "Ohio State", "team2": "Michigan"}, {"user_id": "test"}
        )
        ...
        education_result = engine._handle_education(
            {"concept": "machine_learning", "level": "beginner"}, {"user_id": "test"}
        )
