...
try:
    from config.model_features import (
        RIDGE_FEATURES,
        XGB_FEATURES,
        get_model_features,
        validate_features,
...
class WeeklyAnalysisOrchestrator(BaseAgent):
    ...
    def __init__(self, week: int, season: int = 2025, agent_id: Optional[str] = None):
        ...
        self.prediction_agent = WeeklyPredictionGenerationAgent(
    ...
    def _validate_features_for_models(
        ...
        if RIDGE_FEATURES:
            ...
            if missing_ridge:
                ...
                validation_result["warnings"].append(
                    ...
                    + (
                        f" (and {len(missing_ridge) - 5} more)"
                        if len(missing_ridge) > 5
                        else ""
                    )
                )
        ...
        if XGB_FEATURES:
            ...
            if missing_xgb:
                ...
                validation_result["warnings"].append(
                    ...
                    + (
                        f" (and {len(missing_xgb) - 5} more)"
                        if len(missing_xgb) > 5
                        else ""
                    )
                )
    ...
    def run_complete_analysis(
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Model validation completed: {validation_result.get('status', 'unknown')}"
                )
            ...
            try:
                ...
                logger.info(
                    f"Matchup analysis completed: {matchup_result.get('status', 'unknown')}"
                )
            except Exception as e:
            ...
            try:
                ...
                if enhanced_path and enhanced_path.exists():
                    ...
                    if not feature_validation["is_valid"]:
                        error_msg = (
                            "Feature validation failed. Critical features missing."
                        )
                        logger.error(f"❌ {error_msg}")
                        for error in feature_validation["errors"]:
                            logger.error(f"   - {error}")
                        # Don't fail the pipeline, but log the issue
                        results["feature_validation_warnings"] = feature_validation[
                            "warnings"
                        ]
                        results["feature_validation_errors"] = feature_validation[
                            "errors"
                        ]
                    else:
                    ...
                    logger.warning(
                        f"⚠️  Could not find enhanced features file at {enhanced_path}"
                    )
            except Exception as e:
            ...
            try:
                ...
                logger.info(
                    f"Prediction generation completed: {prediction_result.get('status', 'unknown')}"
                )
            except Exception as e:
            ...
            try:
                report_result = self.report_agent.execute_task(
                    {
                        "stats_path": results.get("matchup_analysis", {}).get(
                            "output_path"
                        )
                    }
                )
                ...
                logger.info(
                    f"Report generation completed: {report_result.get('status', 'unknown')}"
                )
            except Exception as e:
            ...
            logger.info(
                f"Week {self.week} analysis pipeline completed: {results['final_status']}"
            )
            return results
