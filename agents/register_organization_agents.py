...
def register_organization_agents():
    ...
    try:
        ...
        from agents.file_organization_agent import (
            FileOrganizationAgent,
            create_file_organization_agent,
        )
        ...
        try:
            file_org_agent = factory.create_agent(
                "file_organization", "test_file_org_001"
            )
            logger.info(
                f"✅ Created File Organization Agent: {file_org_agent.agent_id}"
            )
            logger.info(
                f"   Capabilities: {[cap.name for cap in file_org_agent.capabilities]}"
            )
            ...
            logger.info(
                f"   Capabilities: {[cap.name for cap in validation_agent.capabilities]}"
            )
...
def test_agent_functionality():
    ...
    try:
        ...
        if result["status"] == "success":
            logger.info(
                f"✅ File classification works: {result['result']['total_files']} files found"
            )
        else:
            logger.error(
                f"❌ File classification failed: {result.get('error', 'Unknown error')}"
            )
        ...
        if result["status"] == "success":
            ...
            logger.error(
                f"❌ Import validation failed: {result.get('error', 'Unknown error')}"
            )
