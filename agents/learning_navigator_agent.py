...
class LearningNavigatorAgent(BaseAgent):
        ...
        return [
            ...
            AgentCapability(
                ...
                data_access=[
                    ...
                    "data/training/weekly/training_data_2025_week*.csv",
                ],
    ...
    def _guide_learning_path(
        ...
        learning_path = self.learning_paths.get(
            user_role, self.learning_paths[UserRole.ANALYST]
        )
        ...
        current_notebook_info = (
            learning_path[current_position]
            if current_position < len(learning_path)
            else None
        )
        next_notebook_info = (
            learning_path[current_position + 1]
            if current_position + 1 < len(learning_path)
            else None
        )
    ...
    def _track_progress(
        ...
        progress_percentage = (
            (completed_count / total_notebooks) * 100 if total_notebooks > 0 else 0
        )
    ...
    def _generate_learning_recommendations(
        ...
        if current_position == 0:
            recommendations.append(
                "Start with the introduction notebook to understand the data structure"
            )
            ...
            recommendations.append(
                "You're making good progress! Try applying concepts to your own questions"
            )
        else:
            recommendations.append(
                "Advanced topics ahead! Consider exploring model_pack for ML techniques"
            )

        return recommendations

    def _get_concept_explanation(
        self, concept: str, skill_level: str
    ) -> Dict[str, Any]:
    ...
    def _get_learning_resources(
        self, user_role: UserRole, current_topic: str, skill_level: str
    ) -> List[Dict[str, Any]]:
    ...
    def _get_next_steps(
        self, completed_notebooks: List[str], current_notebook: Optional[str]
    ) -> List[str]:
        ...
        if not completed_notebooks:
            steps.append(
                "Start with '01_intro_to_data.ipynb' to understand the data structure"
            )

        if len(completed_notebooks) >= 3:
            steps.append(
                "Consider exploring some model_pack notebooks for ML techniques"
            )
            ...
            badges.append(
                {
                    ...
                    "icon": "🚀",
                    "description": "Completed first notebook",
                }
            )
            ...
            badges.append(
                {
                    "name": "Data Explorer",
                    "icon": "🔍",
                    "description": "Completed 3 notebooks",
                }
            )
            ...
            badges.append(
                {
                    ...
                    "icon": "📊",
                    "description": "Completed 6 notebooks",
                }
            )
            ...
            badges.append(
                {
                    ...
                    "icon": "🏆",
                    "description": "Completed all starter notebooks",
                }
            )

        return badges

    def _bridge_to_model_pack(
        self, parameters: Dict[str, Any], user_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        weekly_data_preview = self._preview_weekly_training_data(
            bridge_info.get("weekly_data_example")
        )
            ...
            if weekly_file.exists():
                ...
                feature_categories = {
                    ...
                    "strength_metrics": [
                        c
                        for c in df.columns
                        if "elo" in c.lower() or "talent" in c.lower()
                    ],
                    "adjusted_epa": [c for c in df.columns if "adjusted_epa" in c],
                    "adjusted_success": [
                        c
                        for c in df.columns
                        if "adjusted_success" in c and "allowed" not in c
                    ],
                    "adjusted_explosiveness": [
                        c
                        for c in df.columns
                        if "adjusted" in c and "explosiveness" in c
                    ],
                    "havoc_metrics": [c for c in df.columns if "havoc" in c],
                    "points_and_margin": [
                        c
                        for c in df.columns
                        if c in ["home_points", "away_points", "margin", "spread"]
                    ],
                }
                ...
                return {
                    ...
                    "feature_categories": {
                        k: len(v) for k, v in feature_categories.items()
                    },
