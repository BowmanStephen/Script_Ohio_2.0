...
class WeeklyPredictionGenerationAgent(BaseAgent):
        ...
        if training_path.exists() and features:
            try:
                usecols = [
                    col
                    for col in features
                    if col in pd.read_csv(training_path, nrows=0).columns
                ]
                if usecols:
    ...
    def _align_model_features(self, features_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing_cols:
            self.log_warning(
                f"Aligned features using defaults for missing columns: {missing_cols[:5]}{'...' if len(missing_cols) > 5 else ''}"
            )
        ...
        if len(object_columns) > 0:
            logger.warning(
                f"Removing object columns from aligned features: {list(object_columns)}"
            )
    ...
    def _execute_action(
        ...
        if action == "generate_predictions":
            ...
            return {
                "status": "error",
                "message": "Use generate_predictions action instead",
            }
    ...
    def execute_task(self, _task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if canonical:
                missing_features = [
                    f for f in canonical if f not in aligned_features.columns
                ]
            if missing_features:
                self.log_warning(
                    f"Missing {len(missing_features)} expected features; filled with defaults/NaN."
                )
            ...
            _compatibility_check = self._validate_model_data_compatibility(
                validated_models, weekly_data
            )
            ...
            if not individual_predictions or individual_predictions.get("error"):
                self.log_warning(
                    "No validated model predictions available; falling back to heuristic predictions."
                )
                individual_predictions = self._generate_fallback_predictions(
                    weekly_data
                )
                if not individual_predictions:
                    raise RuntimeError(
                        f"Unable to generate fallback predictions for Week {self.week}."
                    )

            # Step 5: Create ensemble predictions
            ensemble_predictions = self._create_ensemble_predictions(
                individual_predictions, weekly_data
            )
            if (
                "combined_ensemble" not in ensemble_predictions
                or ensemble_predictions.get("combined_ensemble") is None
            ):
                raise RuntimeError(
                    f"Unable to create ensemble predictions for Week {self.week}."
                )
            ...
            prediction_explanations = self._generate_prediction_explanations(
                ensemble_predictions, weekly_data
            )
            ...
            prediction_insights = self._analyze_prediction_patterns(
            ...
            summary_report = self._create_prediction_summary_report(
                comprehensive_predictions
            )
            ...
            result = {
                ...
                "high_confidence_predictions": len(
                    [
                        p
                        for p in confidence_scores["game_confidences"]
    ...
    def _load_validated_models(self) -> Dict[str, Any]:
        ...
        for config in model_configs:
            try:
                ...
                self.log_info(
                    f"✅ Loaded validated model: {config['name']} from {file_path}"
                )
        ...
        available_count = sum(
            1 for m in models.values() if m["load_status"] == "success"
        )
        self.log_info(
            f"Successfully loaded {available_count}/{len(models)} validated models for predictions"
        )
    ...
    def _load_enhanced_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                self.log_info(
                    f"Loaded {len(games_df)} enhanced games from {games_path}"
                )
            except FileNotFoundError:
                games_df = None
            ...
            analysis_candidates = [
                Path("analysis")
                / f"week{self.week}"
                / f"week{self.week}_comprehensive_analysis.json",
                Path("analysis")
    ...
    def _validate_model_data_compatibility(
        self, validated_models: Dict[str, Any], weekly_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        feature_key = (
            "features_aligned" if "features_aligned" in weekly_data else "features"
        )
        ...
        self.log_info(
            f"Compatibility check: {ready_count}/{len(compatibility_results)} models ready for predictions"
        )

        return compatibility_results

    def _generate_individual_predictions(
        self, validated_models: Dict[str, Any], weekly_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for model_name, model_info in validated_models.items():
            ...
            try:
                ...
                self.log_info(
                    f"✅ Generated {len(predictions)} {pred_type} predictions using {model_name}"
                )
        ...
        self.log_info(
            f"Generated predictions using {len(individual_predictions)} models"
        )
    ...
    def _generate_fallback_predictions(
        ...
        for _, row in features_df.iterrows():
            ...
            motivation_diff = (
                row.get("home_motivation_factor", 1.0)
                - row.get("away_motivation_factor", 1.0)
            ) * 3.0
            rivalry_adj = (row.get("rivalry_game_adjustment", 1.0) - 1.0) * 3.0
            champ_adj = (
                row.get("championship_implication_adjustment", 1.0) - 1.0
            ) * 10.0
        ...
        self.log_info(
            f"Generated heuristic predictions for {len(margin_predictions)} games."
        )
        return heuristic_predictions

    def _create_ensemble_predictions(
        self, individual_predictions: Dict[str, Any], weekly_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if margin_predictions:
                ...
                weighted_margin = np.average(
                    margin_array, axis=0, weights=margin_weights
                )
            ...
            if probability_predictions:
                ...
                prob_weights = np.array(
                    [
                        individual_predictions[name]["model_weight"]
                        for name in individual_predictions
                        if individual_predictions[name]["prediction_type"]
                        == "probability"
                    ]
                )

                weighted_probability = np.average(
                    probability_array, axis=0, weights=prob_weights
                )
            ...
            if margin_predictions and probability_predictions:
                ...
                margin_probs = 1 / (
                    1 + np.exp(-np.array(margin_predictions) / 10)
                )  # Scale margin to reasonable probability range
    ...
    def _calculate_prediction_confidence(
        ...
        try:
            if "combined_ensemble" not in ensemble_predictions:
                return {
                    "error": "No ensemble predictions available for confidence calculation"
                }
            ...
            for i, prob in enumerate(ensemble_probs):
                ...
                model_agreement_boost = self._calculate_model_agreement_boost(
                ...
                confidence_scores["game_confidences"].append(
                    {
                        ...
                        "prediction_confidence_level": self._get_confidence_level(
                            confidence
                        ),
                    }
                )
            ...
            confidences = [
                item["confidence"] for item in confidence_scores["game_confidences"]
            ]
            confidence_scores["average_confidence"] = np.mean(confidences)
            confidence_scores["high_confidence_count"] = sum(
                1 for c in confidences if c > 0.75
            )
            confidence_scores["low_confidence_count"] = sum(
            ...
            self.log_info(
                f"Calculated confidence scores: Average={confidence_scores['average_confidence']:.2f}"
            )
    ...
    def _calculate_model_agreement_boost(
        self, game_index: int, individual_predictions: Dict[str, Any]
    ) -> float:
        ...
        try:
            ...
            agreement_boost = max(
                0, 0.1 - std_dev
            )  # Boost up to 0.1 for strong agreement
    ...
    def _generate_prediction_explanations(
        self, ensemble_predictions: Dict[str, Any], weekly_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        if (
            "combined_ensemble" not in ensemble_predictions
            or "features" not in weekly_data
        ):
            return explanations
        ...
        for i, prob in enumerate(ensemble_probs):
            ...
            explanation = {
                ...
                "explanation_text": self._generate_explanation_text(
                    prob, home_team, away_team, game_data
                ),
            }
    ...
    def _estimate_margin_from_probability(self, probability: float) -> float:
        ...
        if probability == 0.5:
            ...
            return (
                -np.log((1 - probability) / probability) * 7
            )  # Scale to realistic margin range
        else:
            return np.log(probability / (1 - probability)) * 7

    def _identify_key_prediction_factors(
        self, game_data: pd.Series, probability: float
    ) -> List[str]:
        ...
        if abs(talent_diff) > 50:
            key_factors.append(
                f"{'Home team' if talent_diff > 0 else 'Away team'} has significant talent advantage ({abs(talent_diff)} points)"
            )
        ...
        if abs(epa_diff) > 0.2:
            key_factors.append(
                f"Strong {'offensive' if epa_diff > 0 else 'defensive'} efficiency advantage"
            )
    ...
    def _get_contextual_factors(
        self, game_index: int, weekly_data: Dict[str, Any]
    ) -> List[str]:
        ...
        if self.week >= 10:
            contextual_factors.append(
                f"Week {self.week} - Late season momentum important"
            )
    ...
    def _generate_explanation_text(
        self, prob: float, home_team: str, away_team: str, game_data: pd.Series
    ) -> str:
        ...
        explanation = (
            f"{predicted_winner} is favored to win with {win_prob:.1%} confidence. "
        )

        if margin_estimate > 10:
            explanation += (
                f"Expected to win by approximately {margin_estimate:.0f} points. "
            )
    ...
    def _create_comprehensive_predictions(
        self,
        ensemble_predictions: Dict[str, Any],
        ...
        for i, prob in enumerate(ensemble_probs):
            ...
            game_data = features_df.iloc[i]
            explanation = (
                prediction_explanations[i] if i < len(prediction_explanations) else {}
            )
            ...
            prediction_record = {
                "game_id": game_data.get(
                    "game_id", f"{self.season}_week{self.week}_{i + 1:03d}"
                ),
                "season": self.season,
                "week": self.week,
                "home_team": home_team,
                ...
                "confidence_score": confidence_scores["game_confidences"][i][
                    "confidence"
                ],
                "confidence_level": confidence_scores["game_confidences"][i][
                    "prediction_confidence_level"
                ],
                "risk_assessment": explanation.get("risk_assessment", "Medium Risk"),
                "key_factors": explanation.get("key_factors", []),
                "prediction_explanation": explanation.get(
                    "explanation_text", f"{home_team} vs {away_team} prediction"
                ),
        ...
        self.log_info(
            f"Created {len(comprehensive_predictions)} comprehensive prediction records"
        )
        return comprehensive_predictions

    def _analyze_prediction_patterns(
        self, comprehensive_predictions: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for pred in comprehensive_predictions:
            ...
            if (
                pred["predicted_winner"] == pred["away_team"]
                and home_talent > away_talent + 30
            ) or (
                pred["predicted_winner"] == pred["home_team"]
                and away_talent > home_talent + 30
            ):
        ...
        analysis["prediction_patterns"] = {
            "home_win_rate": analysis["home_win_predictions"]
            / len(comprehensive_predictions),
            "upset_rate": analysis["upset_predictions"]
            / len(comprehensive_predictions),
            "close_game_rate": analysis["close_games"] / len(comprehensive_predictions),
            "blowout_rate": analysis["blowout_predictions"]
            / len(comprehensive_predictions),
        }

        self.log_info(
            f"Pattern analysis completed: {analysis['home_win_predictions']}/{analysis['total_predictions']} home wins predicted"
        )
        return analysis
    ...
    def _generate_actionable_recommendations(
        ...
        high_confidence_preds = [
            pred for pred in comprehensive_predictions if pred["confidence_score"] > 0.8
        ]
        ...
        upset_preds = [
            pred
            for pred in comprehensive_predictions
            if "upset_prediction" in pred.get("key_factors", [])
        ]
        ...
        close_games = [
            pred
            for pred in comprehensive_predictions
            if abs(pred["predicted_margin"]) < 3
        ]
    ...
    def _generate_markdown_summary(
        ...
        avg_confidence = np.mean(
            [pred["confidence_score"] for pred in comprehensive_predictions]
        )
        avg_margin = np.mean(
            [abs(pred["predicted_margin"]) for pred in comprehensive_predictions]
        )
        close_games = sum(
            1 for pred in comprehensive_predictions if abs(pred["predicted_margin"]) < 4
        )
        high_conf = sum(
            1 for pred in comprehensive_predictions if pred["confidence_score"] > 0.75
        )

        # Get top confident predictions
        sorted_predictions = sorted(
            comprehensive_predictions, key=lambda x: x["confidence_score"], reverse=True
        )
        ...
        if top_predictions:
            md_lines.extend(["## Top Predictions (Highest Confidence)", ""])
        ...
        if actionable_recommendations:
            md_lines.extend(["## Actionable Recommendations", ""])

            # Group by type
            high_conf_recs = [
                r
                for r in actionable_recommendations
                if r.get("type") == "high_confidence"
            ]
            upset_recs = [
                r for r in actionable_recommendations if r.get("type") == "upset_alert"
            ]
            close_recs = [
                r for r in actionable_recommendations if r.get("type") == "close_game"
            ]
        ...
        *,
        ...
        try:
            if comprehensive_predictions:
                ...
                available_columns = [
                    col for col in key_columns if col in predictions_df.columns
                ]
                ...
                self.log_info(
                    f"Saved {len(comprehensive_predictions)} predictions to {csv_path} ({len(final_columns)} columns)"
                )
            else:
    ...
    def _create_prediction_summary_report(
        self, comprehensive_predictions: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        self.log_info(
            f"Prediction summary report generated (included in markdown summary)"
        )
