...
class APISourceValidator:
    ...
    def _validate_authentication(self) -> Dict[str, Any]:
        ...
        try:
            with self._session() as session:
                teams = session.teams_api.get_fbs_teams(year=2025)
        ...
        try:
            with self._session() as session:
                for endpoint_test in test_endpoints:
                    ...
                    endpoint_result = self._test_single_endpoint(session, endpoint_test)
    ...
    def _test_single_endpoint(self, session, endpoint_config: Dict) -> Dict[str, Any]:
        """Test a single CFBD endpoint"""
        try:
            # Determine API method
            if endpoint_config["api_method"] == "get_games":
                api = session.games_api
