...
class StarterPack2025DataGenerator:
    ...
    def load_reference_data(self):
        ...
        try:
            ...
            self.adv_game_sample = pd.read_csv(
                self.data_dir / "advanced_game_stats" / "2024.csv", nrows=5
            )
            self.adv_season_sample = pd.read_csv(
                self.data_dir / "advanced_season_stats" / "2024.csv", nrows=5
            )
    ...
    def get_fbs_teams(self):
        ...
        return self.teams_df[self.teams_df["classification"].isin(["fbs", "FBS"])][
            "school"
        ].tolist()
    ...
    def generate_2025_games(self):
        ...
        for week in range(1, self.current_week + 1):
            ...
            for i in range(0, len(fbs_teams) - 1, 2):
                ...
                # Get conference info
                home_conf = (
                    self.teams_df[self.teams_df["school"] == home_team][
                        "conference"
                    ].iloc[0]
                    if len(self.teams_df[self.teams_df["school"] == home_team]) > 0
                    else "FBS"
                )
                away_conf = (
                    self.teams_df[self.teams_df["school"] == away_team][
                        "conference"
                    ].iloc[0]
                    if len(self.teams_df[self.teams_df["school"] == away_team]) > 0
                    else "FBS"
                )
    ...
    def generate_advanced_game_stats(self, games):
        ...
        for game in games:
            ...
            for is_home in [True, False]:
                ...
                # Offense metrics
                offense_metrics = {
                    "passingPlays_explosiveness": np.clip(
                        np.random.normal(1.3, 0.4) * performance_factor, 0.5, 3.0
                    ),
                    "passingPlays_successRate": np.clip(
                        np.random.normal(0.48, 0.08) * performance_factor, 0.2, 0.8
                    ),
                    "passingPlays_totalPPA": np.clip(
                        np.random.normal(25, 10) * performance_factor, 0, 60
                    ),
                    "passingPlays_ppa": np.clip(
                        np.random.normal(0.25, 0.1) * performance_factor, -0.5, 1.0
                    ),
                    "rushingPlays_explosiveness": np.clip(
                        np.random.normal(1.1, 0.3) * performance_factor, 0.5, 2.5
                    ),
                    "rushingPlays_successRate": np.clip(
                        np.random.normal(0.45, 0.07) * performance_factor, 0.2, 0.75
                    ),
                    "rushingPlays_totalPPA": np.clip(
                        np.random.normal(20, 8) * performance_factor, 0, 50
                    ),
                    "rushingPlays_ppa": np.clip(
                        np.random.normal(0.2, 0.08) * performance_factor, -0.3, 0.8
                    ),
                    "passingDowns_explosiveness": np.clip(
                        np.random.normal(1.4, 0.4), 0.6, 3.0
                    ),
                    "passingDowns_successRate": np.clip(
                        np.random.normal(0.35, 0.1), 0.15, 0.7
                    ),
                    "passingDowns_ppa": np.clip(np.random.normal(0.3, 0.12), -0.2, 1.0),
                    "standardDowns_explosiveness": np.clip(
                        np.random.normal(1.0, 0.3), 0.4, 2.0
                    ),
                    "standardDowns_successRate": np.clip(
                        np.random.normal(0.52, 0.06), 0.3, 0.8
                    ),
                    "standardDowns_ppa": np.clip(
                        np.random.normal(0.18, 0.08), -0.1, 0.6
                    ),
                    ...
                    "explosiveness": np.clip(
                        np.random.normal(1.2, 0.3) * performance_factor, 0.6, 2.5
                    ),
                    "successRate": np.clip(
                        np.random.normal(0.47, 0.07) * performance_factor, 0.25, 0.75
                    ),
                    "totalPPA": np.clip(
                        np.random.normal(45, 15) * performance_factor, 0, 100
                    ),
                    "ppa": np.clip(
                        np.random.normal(0.23, 0.09) * performance_factor, -0.2, 0.8
                    ),
                ...
                defense_metrics = {
                    "passingPlays_explosiveness": np.clip(
                        np.random.normal(1.0, 0.3) / performance_factor, 0.5, 2.0
                    ),
                    "passingPlays_successRate": np.clip(
                        np.random.normal(0.42, 0.08) / performance_factor, 0.2, 0.7
                    ),
                    "passingPlays_totalPPA": max(
                        -10, np.random.normal(15, 8) / performance_factor
                    ),
                    "passingPlays_ppa": np.clip(
                        np.random.normal(0.15, 0.1) / performance_factor, -0.3, 0.6
                    ),
                    "rushingPlays_explosiveness": np.clip(
                        np.random.normal(0.9, 0.25) / performance_factor, 0.4, 1.8
                    ),
                    "rushingPlays_successRate": np.clip(
                        np.random.normal(0.40, 0.07) / performance_factor, 0.2, 0.65
                    ),
                    "rushingPlays_totalPPA": max(
                        -5, np.random.normal(12, 6) / performance_factor
                    ),
                    "rushingPlays_ppa": np.clip(
                        np.random.normal(0.12, 0.08) / performance_factor, -0.2, 0.5
                    ),
                    "passingDowns_explosiveness": np.clip(
                        np.random.normal(0.9, 0.3), 0.4, 2.0
                    ),
                    "passingDowns_successRate": np.clip(
                        np.random.normal(0.32, 0.08), 0.15, 0.6
                    ),
                    "passingDowns_ppa": np.clip(np.random.normal(0.1, 0.1), -0.3, 0.5),
                    "standardDowns_explosiveness": np.clip(
                        np.random.normal(0.8, 0.25), 0.35, 1.5
                    ),
                    "standardDowns_successRate": np.clip(
                        np.random.normal(0.46, 0.06), 0.25, 0.7
                    ),
                    "standardDowns_ppa": np.clip(
                        np.random.normal(0.14, 0.07), -0.1, 0.4
                    ),
    ...
    def generate_advanced_season_stats(self):
        ...
        for team in fbs_teams:
            # Get team conference
            team_conf = (
                self.teams_df[self.teams_df["school"] == team]["conference"].iloc[0]
                if len(self.teams_df[self.teams_df["school"] == team]) > 0
                else "FBS"
            )
            ...
            offense = {
                "passingPlays_explosiveness": np.clip(
                    np.random.normal(1.25, 0.35), 0.6, 2.5
                ),
                "passingPlays_successRate": np.clip(
                    np.random.normal(0.46, 0.07), 0.25, 0.75
                ),
                ...
                "rushingPlays_explosiveness": np.clip(
                    np.random.normal(1.05, 0.3), 0.5, 2.0
                ),
                "rushingPlays_successRate": np.clip(
                    np.random.normal(0.43, 0.06), 0.25, 0.65
                ),
                ...
                "passingDowns_explosiveness": np.clip(
                    np.random.normal(1.35, 0.4), 0.6, 2.8
                ),
                "passingDowns_successRate": np.clip(
                    np.random.normal(0.33, 0.09), 0.15, 0.65
                ),
                "passingDowns_ppa": np.clip(np.random.normal(0.28, 0.11), -0.2, 0.8),
                "passingDowns_rate": np.clip(np.random.normal(0.37, 0.05), 0.2, 0.55),
                "standardDowns_explosiveness": np.clip(
                    np.random.normal(0.95, 0.28), 0.4, 1.8
                ),
                "standardDowns_successRate": np.clip(
                    np.random.normal(0.50, 0.05), 0.3, 0.75
                ),
                ...
                "fieldPosition_averagePredictedPoints": np.clip(
                    np.random.normal(2.45, 0.45), 1.5, 4.0
                ),
                "fieldPosition_averageStart": np.clip(
                    np.random.normal(29.5, 2.8), 22, 38
                ),
                "pointsPerOpportunity": np.clip(np.random.normal(2.75, 0.48), 1.8, 4.5),
                "totalOpportunies": max(
                    50, int(np.random.normal(180, 35) * games_played / 12)
                ),
                "openFieldYardsTotal": max(
                    100, int(np.random.normal(1450, 400) * games_played / 12)
                ),
                "openFieldYards": max(5, int(np.random.normal(18, 6))),
                "secondLevelYardsTotal": max(
                    80, int(np.random.normal(950, 300) * games_played / 12)
                ),
                "secondLevelYards": max(3, int(np.random.normal(12, 4))),
                "lineYardsTotal": max(
                    60, int(np.random.normal(780, 250) * games_played / 12)
                ),
                ...
                "totalPPA": max(
                    50, int(np.random.normal(500, 150) * games_played / 12)
                ),
            ...
            for key, value in offense.items():
                if "Rate" in key or "successRate" in key:
                    defense[key] = np.clip(
                        1 - value + np.random.normal(0, 0.08), 0.15, 0.85
                    )
                elif "explosiveness" in key:
                    defense[key] = np.clip(
                        value * np.random.normal(0.92, 0.08), 0.4, 2.0
                    )
            ...
            # Combine into row
            row = {"season": 2025, "team": team, "conference": team_conf}
        ...
        for game in games:
            ...
            for drive_num in range(1, num_drives + 1):
                ...
                for is_home in [True, False]:
                    ...
                    # Get conference info
                    offense_conf = (
                        game["home_conference"] if is_home else game["away_conference"]
                    )
                    defense_conf = (
                        game["away_conference"] if is_home else game["home_conference"]
                    )
                    ...
                    drive = {
                        ...
                        "scoring": is_scoring,
                        "startPeriod": np.random.choice(
                            [1, 2, 3, 4], p=[0.35, 0.4, 0.2, 0.05]
                        ),
                        "startYardline": start_field,
                        "startYardsToGoal": max(1, 100 - start_field),
                        "startTime": json.dumps(
                            {
                                "minutes": int(np.random.uniform(0, 15)),
                                "seconds": int(np.random.uniform(0, 60)),
                            }
                        ),
                        "endPeriod": np.random.choice(
                            [1, 2, 3, 4], p=[0.3, 0.45, 0.2, 0.05]
                        ),
        ...
        for game in games:
            ...
            for is_home in [True, False]:
                ...
                pass_completions = int(
                    pass_attempts * np.clip(np.random.normal(0.62, 0.08), 0.35, 0.85)
                )
                pass_yards = max(50, int(pass_completions * np.random.normal(8.5, 2.5)))
                passing_tds = max(
                    0, min(6, int(team_score * np.random.normal(0.65, 0.15)))
                )
                ...
                rush_tds = max(
                    0, min(5, int(team_score * np.random.normal(0.35, 0.12)))
                )
                ...
                stats = {
                    ...
                    "turnovers": max(
                        0, interceptions + int(np.random.normal(1.3, 1.2))
                    ),
        ...
        for game in games:
            ...
            for play_num in range(total_plays):
                ...
                offense_conf = (
                    ...
                    if is_home_offense
                    else game["away_conference"]
                )
                defense_conf = (
                    game["away_conference"]
                    if is_home_offense
                    else game["home_conference"]
                )
                ...
                if play_type in ["Pass", "Run"]:
                    if np.random.random() < 0.85:  # Most plays have downs
                        down = np.random.choice(
                            [1, 2, 3, 4], p=[0.38, 0.33, 0.22, 0.07]
                        )
                        distance = max(1, int(np.random.normal(7.5, 4.2)))

                yards_gained = (
                    int(np.random.normal(2.5, 8.5))
                    if play_type in ["Pass", "Run"]
                    else 0
                )
                ...
                play = {
                    ...
                    "clock": json.dumps(
                        {
                            "minutes": max(0, 15 - int(np.random.uniform(0, 15))),
                            "seconds": int(np.random.uniform(0, 60)),
                        }
                    ),
    ...
    def generate_season_stats(self):
        ...
        for team in fbs_teams:
            ...
            stats = {
                ...
                "games": games_played,
                "firstDowns": max(
                    60, int(np.random.normal(240, 50) * games_played / 12)
                ),
                "firstDownsOpponent": max(
                    60, int(np.random.normal(220, 45) * games_played / 12)
                ),
                "fourthDownConversions": max(
                    2, int(np.random.normal(10, 4) * games_played / 12)
                ),
                "fourthDownConversionsOpponent": max(
                    2, int(np.random.normal(9, 3) * games_played / 12)
                ),
                "fourthDowns": max(8, int(np.random.normal(18, 6) * games_played / 12)),
                "fourthDownsOpponent": max(
                    8, int(np.random.normal(16, 5) * games_played / 12)
                ),
                "fumblesLost": max(2, int(np.random.normal(9, 3) * games_played / 12)),
                "fumblesLostOpponent": max(
                    2, int(np.random.normal(8, 3) * games_played / 12)
                ),
                "fumblesRecovered": max(
                    2, int(np.random.normal(6, 3) * games_played / 12)
                ),
                "fumblesRecoveredOpponent": max(
                    2, int(np.random.normal(5, 2) * games_played / 12)
                ),
                "interceptionTDs": max(
                    0, int(np.random.normal(1.5, 1.2) * games_played / 12)
                ),
                "interceptionTDsOpponent": max(
                    0, int(np.random.normal(1.2, 1.0) * games_played / 12)
                ),
                "interceptionYards": max(
                    20, int(np.random.normal(120, 60) * games_played / 12)
                ),
                "interceptionYardsOpponent": max(
                    20, int(np.random.normal(100, 50) * games_played / 12)
                ),
                "interceptions": max(
                    3, int(np.random.normal(10, 4) * games_played / 12)
                ),
                "interceptionsOpponent": max(
                    3, int(np.random.normal(9, 4) * games_played / 12)
                ),
                "kickReturnTDs": max(
                    0, int(np.random.normal(1.2, 1.0) * games_played / 12)
                ),
                "kickReturnTDsOpponent": max(
                    0, int(np.random.normal(1.0, 0.9) * games_played / 12)
                ),
                "kickReturnYards": max(
                    100, int(np.random.normal(600, 200) * games_played / 12)
                ),
                "kickReturnYardsOpponent": max(
                    100, int(np.random.normal(550, 180) * games_played / 12)
                ),
                "kickReturns": max(
                    10, int(np.random.normal(30, 10) * games_played / 12)
                ),
                "kickReturnsOpponent": max(
                    10, int(np.random.normal(28, 9) * games_played / 12)
                ),
                "netPassingYards": max(
                    500, int(np.random.normal(2800, 800) * games_played / 12)
                ),
                "netPassingYardsOpponent": max(
                    500, int(np.random.normal(2600, 750) * games_played / 12)
                ),
                "passAttempts": max(
                    100, int(np.random.normal(420, 100) * games_played / 12)
                ),
                "passAttemptsOpponent": max(
                    100, int(np.random.normal(400, 95) * games_played / 12)
                ),
                "passCompletions": max(
                    50, int(np.random.normal(260, 70) * games_played / 12)
                ),
                "passCompletionsOpponent": max(
                    50, int(np.random.normal(240, 65) * games_played / 12)
                ),
                "passesIntercepted": max(
                    3, int(np.random.normal(10, 4) * games_played / 12)
                ),
                "passesInterceptedOpponent": max(
                    3, int(np.random.normal(9, 4) * games_played / 12)
                ),
                "passingTDs": max(5, int(np.random.normal(22, 8) * games_played / 12)),
                "passingTDsOpponent": max(
                    5, int(np.random.normal(20, 7) * games_played / 12)
                ),
                "penalties": max(20, int(np.random.normal(70, 20) * games_played / 12)),
                "penaltiesOpponent": max(
                    20, int(np.random.normal(65, 18) * games_played / 12)
                ),
                "penaltyYards": max(
                    100, int(np.random.normal(580, 150) * games_played / 12)
                ),
                "penaltyYardsOpponent": max(
                    100, int(np.random.normal(550, 140) * games_played / 12)
                ),
                ...
                "puntReturnTDs": max(
                    0, int(np.random.normal(0.8, 0.9) * games_played / 12)
                ),
                "puntReturnTDsOpponent": max(
                    0, int(np.random.normal(0.6, 0.8) * games_played / 12)
                ),
                "puntReturnYards": max(
                    50, int(np.random.normal(250, 120) * games_played / 12)
                ),
                "puntReturnYardsOpponent": max(
                    50, int(np.random.normal(220, 100) * games_played / 12)
                ),
                "puntReturns": max(
                    15, int(np.random.normal(40, 12) * games_played / 12)
                ),
                "puntReturnsOpponent": max(
                    15, int(np.random.normal(38, 11) * games_played / 12)
                ),
                "rushingAttempts": max(
                    150, int(np.random.normal(450, 100) * games_played / 12)
                ),
                "rushingAttemptsOpponent": max(
                    150, int(np.random.normal(430, 95) * games_played / 12)
                ),
                "rushingTDs": max(3, int(np.random.normal(18, 6) * games_played / 12)),
                "rushingTDsOpponent": max(
                    3, int(np.random.normal(16, 6) * games_played / 12)
                ),
                "rushingYards": max(
                    300, int(np.random.normal(1900, 500) * games_played / 12)
                ),
                "rushingYardsOpponent": max(
                    300, int(np.random.normal(1750, 450) * games_played / 12)
                ),
                "sacks": max(5, int(np.random.normal(28, 10) * games_played / 12)),
                "sacksOpponent": max(
                    5, int(np.random.normal(25, 9) * games_played / 12)
                ),
                "tacklesForLoss": max(
                    10, int(np.random.normal(70, 20) * games_played / 12)
                ),
                "tacklesForLossOpponent": max(
                    10, int(np.random.normal(65, 18) * games_played / 12)
                ),
                "thirdDownConversions": max(
                    15, int(np.random.normal(45, 12) * games_played / 12)
                ),
                "thirdDownConversionsOpponent": max(
                    15, int(np.random.normal(40, 11) * games_played / 12)
                ),
                "thirdDowns": max(
                    40, int(np.random.normal(110, 25) * games_played / 12)
                ),
                "thirdDownsOpponent": max(
                    40, int(np.random.normal(100, 23) * games_played / 12)
                ),
                "totalYards": max(
                    1000, int(np.random.normal(4700, 1200) * games_played / 12)
                ),
                "totalYardsOpponent": max(
                    1000, int(np.random.normal(4350, 1100) * games_played / 12)
                ),
                "turnovers": max(5, int(np.random.normal(15, 6) * games_played / 12)),
                "turnoversOpponent": max(
                    5, int(np.random.normal(14, 5) * games_played / 12)
                ),
            }
    ...
    def write_csv_files(self, data_dict):
        ...
        if "advanced_game_stats" in data_dict:
            ...
            logger.info(
                f"  ✅ Advanced game stats: {len(df):,} records, {len(df.columns)} columns -> {output_path}"
            )
        ...
        if "advanced_season_stats" in data_dict:
            ...
            df.to_csv(output_path, index=False)
            logger.info(
                f"  ✅ Advanced season stats: {len(df):,} records, {len(df.columns)} columns -> {output_path}"
            )
        ...
        if "drives" in data_dict:
            ...
            df.to_csv(output_path, index=False)
            logger.info(
                f"  ✅ Drives: {len(df):,} records, {len(df.columns)} columns -> {output_path}"
            )
        ...
        if "game_stats" in data_dict:
            ...
            df.to_csv(output_path, index=False)
            logger.info(
                f"  ✅ Game stats: {len(df):,} records, {len(df.columns)} columns -> {output_path}"
            )
        ...
        if "plays" in data_dict:
            ...
            for week in range(1, self.current_week + 1):
                ...
                end_idx = (
                    week * plays_per_week if week < self.current_week else total_plays
                )

                week_plays = plays_df.iloc[start_idx:end_idx].copy()
                if not week_plays.empty:
                    output_path = (
                        self.output_dir / "plays" / "2025" / f"regular_{week}_plays.csv"
                    )
                    week_plays.to_csv(output_path, index=False)
                    logger.info(
                        f"  ✅ Plays Week {week}: {len(week_plays):,} records, {len(week_plays.columns)} columns -> {output_path}"
                    )
        ...
        if "season_stats" in data_dict:
            ...
            df.to_csv(output_path, index=False)
            logger.info(
                f"  ✅ Season stats: {len(df):,} records, {len(df.columns)} columns -> {output_path}"
            )
    ...
    def validate_output_files(self):
            ...
            if full_path.exists():
                try:
                    ...
                    if actual_columns == expected_columns:
                        logger.info(
                            f"  ✅ {file_path}: {actual_columns} columns (expected {expected_columns}), {file_size:.1f}MB"
                        )
                        validation_results[file_path] = "PASS"
                    else:
                        logger.error(
                            f"  ❌ {file_path}: {actual_columns} columns (expected {expected_columns})"
                        )
                        validation_results[file_path] = (
                            f"FAIL: {actual_columns} vs {expected_columns} columns"
                        )
        ...
        if plays_dir.exists():
            ...
            logger.info(
                f"  ✅ Plays directory: {len(plays_files)} weekly files created"
            )
            ...
            if plays_files:
                ...
                logger.info(
                    f"    Sample plays file ({sample_file.name}): {len(df.columns)} columns, {file_size:.1f}MB"
                )
        else:
    ...
    def generate_summary_report(self, data_dict, validation_results):
        ...
        plays_dir = self.output_dir / "plays" / "2025"
        if plays_dir.exists():
            total_plays_size = sum(
                f.stat().st_size for f in plays_dir.glob("*.csv")
            ) / (1024 * 1024)
