...
for _, our_row in our_preds.iterrows():
    ...
    for _, ext_row in external_df.iterrows():
        ...
        # Match
        home_match = (
            (our_home == ext_home_norm)
            or (our_home == ext_home_raw)
            or (our_home.lower() == ext_home_raw.lower())
            or (our_home.lower() == ext_home_norm.lower())
        )
        away_match = (
            (our_away == ext_away_norm)
            or (our_away == ext_away_raw)
            or (our_away.lower() == ext_away_raw.lower())
            or (our_away.lower() == ext_away_norm.lower())
        )
...
# Save comparison
output_path = (
    project_root / "predictions" / "week13" / "week13_prediction_comparison.csv"
)
