#!/usr/bin/env python3
"""
Combine Weeks 5-14 Data and Retrain Models
===========================================

This script:
1. Combines all week-specific training data files (week05-week14) from canonical location (data/training/weekly/)
2. Optionally includes weeks 1-4 if they are missing from training data
3. Fetches game outcomes from CFBD API or existing data (crucial for Week 13 results)
4. Merges with existing updated_training_data.csv
5. Retrains Ridge, XGBoost, and FastAI models with the updated dataset

Usage:
    python3 scripts/integrate_weeks_1_14_and_retrain.py [--include-weeks-1-4]
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(log_dir / "combine_weeks_1_14_retrain.log"),
...
class WeekDataCombiner:
    ...
    def __init__(
        self, project_root: Path = PROJECT_ROOT, include_weeks_1_4: bool = False
    ):
        self.project_root = project_root
        self.include_weeks_1_4 = include_weeks_1_4

        # Base week files (weeks 5-14)
        self.week_files = [
    ...
    def setup_cfbd_client(self) -> bool:
        """Set up CFBD API access"""
        api_key = os.environ.get("CFBD_API_KEY")
        if not api_key:
            logger.warning(
                "CFBD_API_KEY not found in environment. Will use existing data only."
            )
            return False

        self.api_key = api_key
        logger.info("✅ CFBD API access initialized")
        return True
    ...
    def load_week_files(self) -> pd.DataFrame:
        ...
        for week_file in self.week_files:
            ...
            try:
                ...
                file_path = get_weekly_training_file(
                    week=week_num, season=2025, base_path=self.project_root
                )
                ...
                logger.debug(
                    f"   Searched locations: data/training/weekly/, data/weekly_training/, root"
                )
                continue
            ...
            try:
                ...
                if len(df) > 0:
                    ...
                    if len(df.columns) < 80:
                        logger.warning(
                            f"⚠️  {week_file} has fewer columns than expected: {len(df.columns)}"
                        )

                    all_weeks.append(df)
                    loaded_files.append(week_file)
                    logger.info(
                        f"  ✅ Loaded {week_file}: {len(df)} games, {len(df.columns)} columns"
                    )
                else:
        ...
        if missing_files:
            logger.warning(
                f"⚠️  {len(missing_files)} files not found: {', '.join(missing_files)}"
            )
        ...
        logger.info(
            f"✅ Combined {len(combined)} unique games from {len(loaded_files)} week files"
        )
    ...
    def merge_outcomes_from_existing(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if (
                "home_points" not in existing_df.columns
                or "away_points" not in existing_df.columns
            ):
            ...
            # Merge on id
            df = df.merge(
                existing_outcomes, on="id", how="left", suffixes=("", "_existing")
            )
            ...
            logger.info(
                f"  ✅ Merged outcomes from existing data: {outcomes_added} games"
            )
    ...
    def add_game_outcomes(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if hasattr(self, "api_key") and self.api_key and "id" in df.columns:
            ...
            if outcomes:
                for idx, row in df.iterrows():
                    ...
                    if game_id in outcomes:
                        ...
                        if (
                            outcome["home_points"] is not None
                            and outcome["away_points"] is not None
                        ):
                            if "home_points" not in df.columns:
                                df["home_points"] = np.nan
                                df["away_points"] = np.nan
        ...
        if "home_points" in df.columns and "away_points" in df.columns:
            ...
            logger.info(
                f"  ✅ Calculated margin for {games_with_outcomes} games ({games_with_outcomes / len(df) * 100:.1f}%)"
            )
        else:
            logger.warning(
                "  ⚠️  Could not add game outcomes. Models will need outcomes for training."
            )
    ...
    def save_combined_data(
        ...
        try:
            ...
            if "home_points" in df.columns and "away_points" in df.columns:
                ...
                logger.info(
                    f"   Games with outcomes: {games_with_outcomes} ({games_with_outcomes / len(df) * 100:.1f}%)"
                )
        ...
        for model_file in model_files:
            ...
            if model_path.exists():
                backup_path = model_path.with_suffix(
                    model_path.suffix + ".backup_" + timestamp
                )
                try:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Combine weeks 5-14 data and retrain models"
    )
    parser.add_argument(
        "--include-weeks-1-4",
        action="store_true",
    ...
    logger.info("=" * 70)
    logger.info("COMBINING WEEK 5-14 DATA AND RETRAINING MODELS")
    logger.info("=" * 70)
    ...
    try:
        ...
        if missing_weeks:
            ...
            if 14 in missing_weeks:
                logger.info("  → Week 14 will be integrated")
            if (
                any(w in [1, 2, 3, 4] for w in missing_weeks)
                and not args.include_weeks_1_4
            ):
                logger.warning(
                    "  ⚠️  Weeks 1-4 are missing. Use --include-weeks-1-4 to add them."
                )
        else:
            logger.info("✅ All weeks 1-14 are present in training data")
        ...
        try:
            ...
            logger.info(f"📝 Log file: {log_dir / 'combine_weeks_1_14_retrain.log'}")
