...
class DataAuditor:
        ...
        if training_path.exists():
            ...
            try:
                ...
                if len(df_2025) > 0:
                    ...
                    logger.info(
                        f"   2025 Season: {len(df_2025)} games, Weeks {result['latest_week']['weeks_covered']}"
                    )
                else:
                ...
                if result["training_data_date"]:
                    ...
                    if model_date < data_date:
                        ...
                        result["issues"].append(
                            f"{name} model is older than training data"
                        )

                logger.info(
                    f"✅ {name}: {model_info['file_size_mb']} MB, modified {model_info['last_modified']}"
                )
    ...
    def audit_notebooks(self) -> Dict[str, Any]:
        ...
        for nb_path in notebooks:
            try:
                ...
                if (
                    "get_data_config" in nb_content
                    or "config.get_training_data_path" in nb_content
                ):
                ...
                if (
                    'training_data.csv"' in nb_content
                    and "updated_training_data" not in nb_content
                ):
                    result["hardcoded_paths"].append(nb_path.name)
                    result["issues"].append(
                        f"{nb_path.name} may have hardcoded old path"
                    )
                    logger.warning(
                        f"⚠️  {nb_path.name}: May have hardcoded 'training_data.csv'"
                    )
    ...
    def audit_scripts(self) -> Dict[str, Any]:
        ...
        for script_path in scripts:
            try:
                ...
                if "updated_training_data.csv" in content:
                    ...
                    "training_data.csv" in content
                    and "updated_training_data" not in content
                ):
                    result["using_old_data"].append(script_path.name)
                    result["issues"].append(
                        f"{script_path.name} may reference old training_data.csv"
                    )
        ...
        logger.info(
            f"   Scripts using updated data: {len(result['using_updated_data'])}"
        )
        if result["using_old_data"]:
            logger.warning(
                f"   Scripts potentially using old data: {result['using_old_data']}"
            )
    ...
    def generate_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {"total_issues": 0, "critical_issues": [], "recommendations": []}
        ...
        if models_need_retrain:
            summary["critical_issues"].append(
                "Models are older than training data - retraining recommended"
            )
            summary["recommendations"].append(
                "Run: python3 scripts/combine_weeks_5_13_and_retrain.py --retrain"
            )
        ...
        if latest_week:
            ...
            if max_week < 13:
                summary["recommendations"].append(
                    f"Training data only goes to Week {max_week}. Update to Week 13+"
                )

        # Check for hardcoded paths
        if results["notebooks"].get("hardcoded_paths"):
            summary["recommendations"].append(
                "Some notebooks may have hardcoded old paths - review and update"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Audit data synchronization across system"
    )
    parser.add_argument(
        "--fix", action="store_true", help="Attempt to fix issues automatically"
    )
    parser.add_argument(
        "--retrain", action="store_true", help="Retrain models after fixing data"
    )
    ...
    if args.fix:
        ...
        logger.info(
            "Auto-fix not yet implemented. Please follow recommendations above."
        )
