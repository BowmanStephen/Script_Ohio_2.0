    ...
    for idx, row in features_df.iterrows():
        ...
        if "ridge" in models:
            try:
                ...
                pred_result["ridge_home_win_prob"] = float(
                    1 / (1 + np.exp(-ridge_pred / 7))
                )  # Convert margin to prob
            except Exception as e:
        ...
        if "xgb" in models:
            try:
                ...
                pred_result["xgb_margin"] = float(
                    (xgb_proba[1] - 0.5) * 14
                )  # Rough estimate
            except Exception as e:
                print(
                    f"⚠️  XGBoost prediction failed for {home_team} vs {away_team}: {e}"
                )
        ...
        # Ensemble prediction
        if (
            pred_result.get("ridge_margin") is not None
            and pred_result.get("xgb_home_win_prob") is not None
        ):
            # Weighted average
            ensemble_margin = (
                pred_result["ridge_margin"] * 0.6 + pred_result["xgb_margin"] * 0.4
            )
            ensemble_prob = (
                pred_result["ridge_home_win_prob"] * 0.4
                + pred_result["xgb_home_win_prob"] * 0.6
            )
            pred_result["ensemble_margin"] = float(ensemble_margin)
            pred_result["ensemble_home_win_prob"] = float(ensemble_prob)
            pred_result["predicted_winner"] = (
                home_team if ensemble_prob > 0.5 else away_team
            )
            ...
            pred_result["predicted_winner"] = (
                home_team if pred_result["ridge_home_win_prob"] > 0.5 else away_team
            )
            ...
            pred_result["predicted_winner"] = (
                home_team if pred_result["xgb_home_win_prob"] > 0.5 else away_team
            )
        else:
...
def main():
    ...
    print(
        f"✅ Loaded {len(features_df)} games with {len(features_df.columns)} features"
    )
    ...
    enhanced_games_path = (
        ...
        / "week13"
        / "enhanced"
        / "week13_enhanced_games_all_60.csv"
    )
