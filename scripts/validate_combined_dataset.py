...
def validate_combined_dataset() -> dict[str, Any]:
    ...
    if "home_points" in df.columns and "away_points" in df.columns:
        games_with_outcomes = df[df["home_points"].notna() & df["away_points"].notna()]
        results["metrics"]["games_with_outcomes"] = len(games_with_outcomes)
        results["metrics"]["outcomes_percentage"] = (
            len(games_with_outcomes) / len(df) * 100
        )
        ...
        results["metrics"]["2025_outcomes_percentage"] = (
            len(df_2025_with_outcomes) / len(df_2025) * 100 if len(df_2025) > 0 else 0
        )
    else:
        results["warnings"].append(
            "Game outcome columns (home_points, away_points) not found"
        )
...
def print_validation_results(results: dict[str, Any]) -> None:
    ...
    if "games_with_outcomes" in metrics:
        print(
            f"  Games with outcomes: {metrics['games_with_outcomes']} ({metrics.get('outcomes_percentage', 0):.1f}%)"
        )
        print(
            f"  2025 games with outcomes: {metrics.get('2025_games_with_outcomes', 0)} ({metrics.get('2025_outcomes_percentage', 0):.1f}%)"
        )
