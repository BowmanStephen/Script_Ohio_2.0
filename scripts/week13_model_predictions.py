...
class Week13ModelPredictor:
    ...
    def prepare_model_features(
        ...
        def _model_schema(
            model_obj, base_schema: List[str], model_name: str
        ) -> List[str]:
            if model_name == "fastai" and model_obj is not None:
                try:
                    fastai_names = getattr(
                        getattr(model_obj, "dls", None), "x_names", None
                    )
                    if fastai_names:
        ...
        for model_name, schema in feature_sets.items():
            ...
            if missing:
                print(
                    f"  ⚠️  {model_name} missing {len(missing)} features; filling with defaults."
                )
                for col in missing:
    ...
    def generate_predictions(
        ...
        for idx, game in games.iterrows():
            ...
            for model_name, model in models.items():
                try:
                    if model_name == "ridge" and len(
                        game_features.get("ridge", [])
                    ) == len(features["ridge"]):
                        # Ridge predicts margin
                        X = pd.DataFrame(
                            [game_features["ridge"]], columns=features["ridge"]
                        )
                    ...
                    elif model_name in ["xgb", "fastai"] and len(
                        game_features.get(model_name, [])
                    ) == len(features[model_name]):
                        # XGBoost and FastAI predict win probability
                        X = pd.DataFrame(
                            [game_features[model_name]], columns=features[model_name]
                        )
                        ...
                        if model_name == "fastai":
                            ...
                            try:
                                ...
                                home_win_prob = (
                                    float(probs[0])
                                    if hasattr(probs, "__len__") and len(probs)
                                    else 0.5
                                )
            ...
            if idx < 5:  # Show first few predictions
                ...
                print(
                    f"     Ensemble: {game_prediction['ensemble_prediction']['predicted_winner']} "
    ...
    def _generate_ensemble_prediction(
        self, model_predictions: Dict, game: pd.Series
    ) -> Dict:
        ...
        if win_votes:
            ...
            ensemble["confidence"] = min(
                total_confidence / max(valid_predictions, 1), 1.0
            )
        else:
            # Fallback
    ...
    def analyze_model_performance(self, predictions: List[Dict]) -> Dict:
        ...
        confidences = [
            pred["ensemble_prediction"]["confidence"] for pred in predictions
        ]
        if confidences:
        ...
        for pred in predictions:
            ...
            winners = [
                pred["model_predictions"][m]["predicted_winner"]
                for m in models
                if "error" not in pred["model_predictions"][m]
            ]

            if len(winners) > 1:
                agreement = len(set(winners)) / len(
                    winners
                )  # 0 = perfect agreement, 1 = no agreement
        ...
        print(
            f"  🎯 Mean confidence: {performance['confidence_distribution'].get('mean', 0):.3f}"
        )
        print(
            f"  🤝 Mean model agreement: {performance['agreement_analysis'].get('mean_agreement', 0):.3f}"
        )
    ...
    def save_predictions(self, predictions: List[Dict], performance: Dict):
        ...
        predictions_file = (
            output_dir
            / f"week13_model_predictions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
            ...
            summary_data.append(
                {
                    ...
                    "supporting_models": len(
                        pred["ensemble_prediction"]["supporting_models"]
                    ),
                }
            )
        ...
        summary_file = (
            output_dir
            / f"week13_predictions_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
    ...
    def run_prediction_pipeline(self):
        ...
        try:
            ...
            features, games_aligned = self.prepare_model_features(
                games, models, feature_defaults
            )

            # Generate predictions
            predictions = self.generate_predictions(
                games_aligned, models, features, feature_defaults
            )
            ...
            print(
                f"📄 Results saved to: {self.project_root / 'predictions' / 'week13'}"
            )
