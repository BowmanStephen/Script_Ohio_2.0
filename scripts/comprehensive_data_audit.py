...
class ComprehensiveDataAuditor:
    ...
    def __init__(self):
        ...
        self.migrated_files = list(
            self.model_pack_path.glob("*2025*weeks*migrated.csv")
        )
    ...
    def audit_starter_pack(self) -> Dict[str, Any]:
        ...
        games_with_scores = weeks_1_12[
            weeks_1_12["home_points"].notna() & weeks_1_12["away_points"].notna()
        ]
    ...
    def audit_model_pack(self) -> Dict[str, Any]:
        ...
        for migrated_file in self.migrated_files:
            ...
            try:
                ...
                weeks_1_12 = current_season[
                    current_season["week"].between(1, 12)
                ].copy()
    ...
    def audit_training_data(self) -> Dict[str, Any]:
        ...
        missing_columns = [
            col for col in required_columns if col not in weeks_1_12.columns
        ]
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if any(gap["type"] == "missing_in_model_pack" for gap in gaps):
            recommendations.append(
                "Run migration script to migrate missing games to model pack"
            )

        if any(gap["type"] == "missing_in_training" for gap in gaps):
            recommendations.append(
                "Extend training data with missing games from starter pack"
            )

        if any(gap["type"] == "migrated_but_not_in_training" for gap in gaps):
            recommendations.append(
                "Run training data extension workflow to include migrated games"
            )
