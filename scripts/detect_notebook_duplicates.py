    ...
    if len(config_import_cells) > 1:
        for idx, _ in config_import_cells[1:]:  # Skip first occurrence
            issues["duplicate_config_imports"].append(
                (
                    idx,
                    f"Cell {idx} has duplicate config import (first in cell {config_import_cells[0][0]})",
                )
            )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python detect_notebook_duplicates.py <notebook1.ipynb> [notebook2.ipynb ...]"
        )
