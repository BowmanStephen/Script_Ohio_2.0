...
class PushOperations:
    ...
    def safe_push(
        ...
        logger.info(
            f"Starting safe push: branch={branch}, remote={remote}, dry_run={dry_run}"
        )
        ...
        if not skip_validation:
            ...
            validation_results = self.validation_orchestrator.run_validation(
                validation_type="all", validation_scope=validation_scope
            )
    ...
    def _execute_push(
        self, branch: str, remote: str, force_with_lease: bool = False
    ) -> Dict[str, Any]:
