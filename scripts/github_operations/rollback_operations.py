...
class RollbackOperations:
    ...
    def safe_rollback(
        ...
        logger.info(
            f"Starting rollback: commit={target_commit}, emergency={emergency_mode}"
        )
        ...
        if target_commit is None:
            ...
            try:
                result_prev = self.git_utils._run_git(
                    ["log", "-1", "--format=%H", "HEAD~1"], check=False
                )
        ...
        if create_backup:
            ...
            if not backup_branch:
                if not emergency_mode:
                    result["error"] = (
                        "Failed to create backup branch. Use --emergency to skip."
                    )
                    return result
        ...
        try:
            ...
            if backup_branch:
                result["recovery_suggestion"] = (
                    f"Recovery: git checkout {backup_branch}"
                )

        return result

    def _create_backup_branch(
        self, current_branch: str, current_commit: str
    ) -> Optional[str]:
        ...
        try:
            result = self.git_utils._run_git(
                ["branch", backup_branch, current_commit], check=False
            )
    ...
    def _execute_rollback(
        self, target_commit: str, emergency_mode: bool
    ) -> Dict[str, Any]:
        """Execute the actual rollback"""
        try:
            # Reset to target commit
            reset_result = self.git_utils._run_git(
                ["reset", "--hard", target_commit], check=False
            )
