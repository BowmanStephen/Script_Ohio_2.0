...
try:
    ...
    logger.warning(
        "Prediction_Tracker loader not available. Meta-ensemble will not be calculated."
    )
...
def match_team_name(team_name: str, ratings_dict: Dict[str, Dict]) -> Optional[str]:
    ...
    for key in ratings_dict.keys():
        ...
        if key_normalized == normalized and normalized:
            ...
            if len(team_name) == len(key) or (
                normalized == team_name or normalized == key
            ):
                return key
    ...
    try:
        ...
        sp_col = (
            ...
            if "SP+" in df.columns
                ...
                "sp"
                if "sp" in df.columns
                else "spOverall" if "spOverall" in df.columns else None
            )
        )
...
def fetch_sp_fpi_ratings(
    ...
    if GQL_AVAILABLE:
        ...
        if os.getenv("CFBD_GRAPHQL_DISABLED", "false").lower() == "true":
            logger.info(
                "GraphQL explicitly disabled via CFBD_GRAPHQL_DISABLED - skipping GraphQL fetch"
            )
        else:
            ...
            if api_key:
                try:
                    ...
                    if api_ratings:
                        ...
                        logger.info(
                            f"  ✅ Merged API ratings for {len(api_ratings)} teams"
                        )
                except Exception as e:
...
def calculate_sp_predicted_margin(
    home_sp: Optional[float], away_sp: Optional[float], neutral_site: bool
) -> Optional[float]:
...
def calculate_fpi_predicted_margin(
    home_fpi: Optional[float], away_fpi: Optional[float], neutral_site: bool
) -> Optional[float]:
...
def load_calibration_weights() -> Dict[str, float]:
    ...
    if not weights_path.exists():
        ...
        return {"ohio_model_weight": 0.65, "sp_weight": 0.25, "fpi_weight": 0.10}
    ...
    try:
        ...
        logger.info(
            f"  Ohio={weights.get('ohio_model_weight', 0.65):.3f}, "
...
def calculate_calibrated_margin(
    ensemble_margin: float,
    sp_margin: Optional[float],
    fpi_margin: Optional[float],
    weights: Dict[str, float],
) -> Optional[float]:
    ...
    if fpi_margin is not None and not np.isnan(fpi_margin):
        ...
        if len(terms) > 0:
            redist_weight = fpi_weight / sum(
                [ohio_weight, sp_weight if sp_margin is not None else 0]
            )
...
def enhance_predictions(
    ...
    for idx, row in enhanced_df.iterrows():
        ...
        if home_key and away_key:
            ...
            sp_margin = calculate_sp_predicted_margin(
                home_rating.get("sp"), away_rating.get("sp"), neutral_site
            )
            ...
            fpi_margin = calculate_fpi_predicted_margin(
                home_rating.get("fpi"), away_rating.get("fpi"), neutral_site
            )
            if fpi_margin is not None:
            ...
            if calibrated_margin is not None:
                ...
                enhanced_df.at[idx, "calibrated_home_win_prob"] = margin_to_win_prob(
                    calibrated_margin
                )
                enhanced_df.at[idx, "calibrated_vs_ohio_diff"] = (
                    calibrated_margin - ensemble_margin
                )
        else:
    ...
    if missing_teams:
        logger.warning(
            f"⚠️  Missing ratings for {len(missing_teams)} teams: {list(missing_teams)[:10]}"
        )
    ...
    if tracker_df is not None and meta_weights:
        ...
        ohio_col = (
            "ensemble_margin"
            if "ensemble_margin" in enhanced_df.columns
        ...
        for idx, row in tracker_merged.iterrows():
            ...
            if meta_margin is not None:
                ...
                enhanced_df.at[idx, "meta_ensemble_win_prob"] = margin_to_win_prob(
                    meta_margin
                )
                enhanced_df.at[idx, "ohio_vs_system_avg_diff"] = (
                    ohio_margin - system_avg if not np.isnan(system_avg) else np.nan
                )
...
def main():
    ...
    try:
        ...
        predictions_path = (
            ...
            / "predictions"
            / "week13"
            / "week13_predictions_all_60_games.csv"
        )
        ...
        logger.info(
            f"✅ Loaded {len(predictions_df)} predictions from {predictions_path}"
        )
        ...
        if TRACKER_AVAILABLE:
            ...
            try:
                ...
                week = (
                    predictions_df.get("week", pd.Series([13])).iloc[0]
                ...
                if not tracker_df.empty:
                    logger.info(
                        f"✅ Loaded {len(tracker_df)} games from Prediction_Tracker"
                    )
                    ...
                    if meta_weights:
                        ...
                        logger.warning(
                            "⚠️  Meta-ensemble weights not available, using defaults"
                        )
                        meta_weights = {
                            "ohio_weight": 0.35,
        ...
        logger.info("=" * 80)
        enhanced_df = enhance_predictions(
            predictions_df, ratings, weights, tracker_df, meta_weights
        )
        ...
        logger.info(
            f"SP+ predictions: {enhanced_df['sp_predicted_margin'].notna().sum()}"
        )
        logger.info(
            f"FPI predictions: {enhanced_df['fpi_predicted_margin'].notna().sum()}"
        )
        logger.info(
            f"Calibrated predictions: {enhanced_df['calibrated_margin'].notna().sum()}"
        )
        if "meta_ensemble_margin" in enhanced_df.columns:
            logger.info(
                f"Meta-ensemble predictions: {enhanced_df['meta_ensemble_margin'].notna().sum()}"
            )
