...
def get_expected_games() -> Dict[int, Dict[int, List[Dict]]]:
    ...
    if not games_api:
        print(
            "WARNING: No API key available. Cannot fetch expected games from CFBD API."
        )
        print(
            "Please set CFBD_API_KEY environment variable to enable full gap analysis."
        )
        return {}
...
def compare_games(expected: Dict, training_df: pd.DataFrame) -> Dict:
    ...
    for season, weeks in expected.items():
        season_expected = 0
        season_actual = (
            len(training_df[training_df["season"] == season])
            if season in training_df["season"].values
            else 0
        )
        season_missing = 0
        ...
        for week, games in weeks.items():
            ...
            if missing_ids:
                missing_games[season][week] = [
                    g for g in games if str(g.get("id", "")).strip() in missing_ids
                ]
...
def generate_report(missing_games: Dict, stats: Dict, output_path: Path):
    ...
    with open(output_path, "w") as f:
        ...
        f.write(
            f"  Coverage: {(1 - stats['missing_total'] / stats['expected_total']) * 100:.1f}%\n\n"
        )
        ...
        for season in sorted(stats["by_season"].keys()):
            ...
            f.write(
                f"  {season}: Expected {s['expected']}, Actual {s['actual']}, Missing {s['missing']}\n"
            )
            ...
            if missing_games[season]:
                ...
                for week in sorted(missing_games[season].keys()):
                    ...
                    for game in games[:10]:  # Show first 10
                        f.write(
                            f"    - ID: {game.get('id')}, {game.get('home_team')} vs {game.get('away_team')}\n"
                        )
                    if len(games) > 10:
...
def main():
    ...
    if not expected:
        ...
        with open(report_path, "w") as f:
            ...
            f.write("=" * 80 + "\n\n")
            f.write(
                "NOTE: CFBD API key not available. Full gap analysis requires API access.\n\n"
            )
            ...
            for season in sorted(structure_analysis["by_season"].keys()):
                ...
                f.write(
                    f"  {season}: {s['total']} games, Weeks {s['week_range'][0]}-{s['week_range'][1]}\n"
                )
