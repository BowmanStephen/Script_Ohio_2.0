...
def check_training_data_coverage(target_week: int = 13) -> Dict[str, Any]:
    ...
    if not training_path.exists():
        return {"status": "missing", "message": "Training data file not found"}
    ...
    try:
        ...
        if len(df_2025) == 0:
            return {"status": "no_2025_data", "message": "No 2025 season data found"}
        ...
        return {"status": "error", "message": f"Error reading training data: {e}"}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Ensure all system components use latest data"
    )
    parser.add_argument(
        "--week", type=int, default=13, help="Target week to ensure coverage"
    )
    parser.add_argument(
        "--retrain", action="store_true", help="Retrain models if needed"
    )
    parser.add_argument(
        "--force", action="store_true", help="Force update even if appears current"
    )
    parser.add_argument(
        "--update-data", action="store_true", help="Update training data if needed"
    )
    ...
    all_good = (
        data_status["status"] == "up_to_date"
        and len(model_status["needs_retrain"]) == 0
    )
    ...
    if all_good:
        ...
        if model_status["needs_retrain"]:
            logger.warning(
                f"   Models: {', '.join(model_status['needs_retrain'])} may need retraining"
            )
        return 1
