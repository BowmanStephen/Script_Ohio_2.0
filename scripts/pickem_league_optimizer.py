...
def calculate_confidence_score(row):
    ...
    if external_line is not None and not pd.isna(external_line):
        ...
        if external_prob is not None and not pd.isna(external_prob):
            ...
            line_confidence = min(
                abs(external_line) / 30.0, 1.0
            )  # Cap at 30 point line
...
def optimize_point_allocation(df, strategy="standard", use_super_mortal=False):
    ...
    picks["expected_points"] = picks.apply(
        lambda row: row["assigned_points"] * row["confidence_score"], axis=1
    )
...
def format_picks_for_display(result):
    ...
    for idx, row in picks_sorted.iterrows():
        ...
        output.append(
            f"  Confidence: {confidence:.1%} | Line: {line} | Our Margin: {margin:.1f}"
        )
...
def main():
    ...
    if has_external:
        print(
            f"✅ External betting lines available for {df['external_line'].notna().sum()} games"
        )
    else:
