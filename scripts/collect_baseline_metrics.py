...
def collect_loc(project_root: Path) -> Dict[str, Any]:
    ...
    metrics: Dict[str, Any] = {"total_lines": 0, "python_files": 0, "by_directory": {}}
    ...
    for ext in ["*.py"]:
        for file_path in project_root.rglob(ext):
            ...
            if any(
                ...
                for skip in ["__pycache__", ".venv", "venv", ".git"]
            ):
                continue
...
def collect_test_coverage(project_root: Path) -> Dict[str, Any]:
    ...
    test_files.extend(
        (project_root / "agents" / "tests").rglob("*.py")
        if (project_root / "agents" / "tests").exists()
        else []
    )
            ...
            if coverage_result.returncode == 0:
                try:
                    ...
                    if "totals" in coverage_data:
                        ...
                        coverage_metrics["coverage_percent"] = coverage_data[
                            "totals"
...
def collect_file_counts(project_root: Path) -> Dict[str, Any]:
    ...
    for py_file in project_root.rglob("*.py"):
        if any(
            skip in str(py_file) for skip in ["__pycache__", ".venv", "venv", ".git"]
        ):
            continue
...
def main():
    ...
    print(
        f"  - Average complexity: {baseline_metrics['complexity_metrics']['average_complexity']:.2f}"
    )
    if baseline_metrics["coverage_metrics"]["coverage_available"]:
        print(
            f"  - Test coverage: {baseline_metrics['coverage_metrics']['coverage_percent']:.1f}%"
        )
    if baseline_metrics["performance_metrics"]["test_runtime"] > 0:
        print(
            f"  - Test runtime: {baseline_metrics['performance_metrics']['test_runtime']:.2f}s"
        )
