...
try:
    ...
    logging.warning(
        "PyYAML not available. YAML frontmatter generation will use basic formatting."
    )
...
def extract_agent_metadata(content: str, filename: str) -> Dict[str, Any]:
    ...
    trigger_section = re.search(
        r"(?:^##\s+Trigger Phrases|^###\s+Trigger Phrases)\s*\n\n(.+?)(?=\n##|\n#|$)",
        content,
        re.MULTILINE | re.DOTALL,
    )
    if trigger_section:
        ...
        trigger_phrases = re.findall(
            r'["\']([^"\']+)["\']|^[-*]\s+(.+)$', trigger_text, re.MULTILINE
        )
        metadata["trigger_phrases"] = [
            p[0] or p[1] for p in trigger_phrases if p[0] or p[1]
        ]

    # Extract workflow steps from "What It Does" section
    what_it_does = re.search(
        r"(?:^##\s+What It Does|^###\s+What It Does)\s*\n\n(.+?)(?=\n##|\n#|$)",
        content,
        re.MULTILINE | re.DOTALL,
    )
    if what_it_does:
    ...
    quality_section = re.search(
        r"(?:^##\s+Quality Standards|^###\s+Quality Standards)\s*\n\n(.+?)(?=\n##|\n#|$)",
        content,
        re.MULTILINE | re.DOTALL,
    )
    if quality_section:
        ...
        standards = re.findall(
            r"^\d+\.\s+(.+)$|^[-*]\s+(.+)$|^[-*]\s+(.+)$", quality_text, re.MULTILINE
        )
        metadata["quality_standards"] = [
            s[0] or s[1] or s[2] for s in standards if any(s)
        ]

    # Extract usage examples
    examples_section = re.search(
        r"(?:^##\s+Usage Examples|^###\s+Usage Examples)\s*\n\n(.+?)(?=\n##|\n#|$)",
        content,
        re.MULTILINE | re.DOTALL,
    )
    if examples_section:
...
def determine_category_and_purpose(agent_data: Dict) -> Tuple[str, str]:
    ...
    """
    description = (
        agent_data.get("description", "") + " " + agent_data.get("purpose", "")
    ).lower()
    name = agent_data.get("name", "").lower()

    # Category mapping
    if any(
        word in description or word in name
        for word in ["code", "analyze", "pattern", "extract"]
    ):
        category = "code-analysis"
    elif any(
        word in description or word in name
        for word in ["content", "create", "generate", "write"]
    ):
        category = "content-creation"
    elif any(
        word in description or word in name
        for word in ["document", "doc", "guide", "tutorial"]
    ):
        category = "documentation"
    elif any(
        word in description or word in name
        for word in ["workflow", "automate", "process", "pipeline"]
    ):
        category = "workflow-automation"
    elif any(
        word in description or word in name
        for word in ["extract", "data", "structure", "parse"]
    ):
        category = "data-extraction"
    elif any(
        word in description or word in name
        for word in ["meta", "tool", "agent", "prompt"]
    ):
        category = "meta"
    else:
    ...
    if any(
        word in description or word in name
        for word in ["extract", "pull", "get", "retrieve"]
    ):
        purpose = "content-extraction"
    elif any(
        word in description or word in name
        for word in ["curate", "organize", "select", "filter"]
    ):
        purpose = "content-curation"
    elif any(
        word in description or word in name
        for word in ["generate", "create", "write", "produce"]
    ):
        purpose = "content-generation"
    elif any(
        word in description or word in name
        for word in ["analyze", "insight", "statistic", "trend"]
    ):
        purpose = "analysis"
    elif any(
        word in description or word in name
        for word in ["automate", "workflow", "process"]
    ):
        purpose = "automation"
    elif any(
        word in description or word in name
        for word in ["document", "guide", "tutorial"]
    ):
...
def generate_frontmatter(
    agent_data: Dict, category: str, purpose: str, tags: List[str]
) -> str:
    """
...
def generate_prompt_library_content(agent_data: Dict, frontmatter: str) -> str:
    ...
    content += "## Notes\n\n"
    content += (
        f"- **Meta-tool** for documenting other tools - enables self-documentation\n"
    )
...
def update_prompt_library_index(
    entry_title: str, entry_path: Path, index_path: Path
) -> None:
    """
...
def validate_frontmatter(frontmatter: str) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if YAML_AVAILABLE:
            ...
            missing_fields = [
                field for field in required_fields if f"{field}:" not in yaml_content
            ]
