...
def fetch_week13_games(api_key: str):
    ...
    # Import CFBD only after environment check and argument parsing
    try:
        import cfbd
    ...
    if games_2025_path.exists():
        ...
        combined_2025 = combined_2025.sort_values(
            ["season", "week", "start_date"], na_position="last"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Fetch Week 13 games from CFBD API")
    parser.add_argument(
    ...
    try:
        ...
        if games_df is not None and len(games_df) > 0:
            ...
            print(
                "  python3 scripts/run_weekly_analysis.py --week 13 --season 2025 --step predictions"
            )
            print("=" * 80)
        else:
