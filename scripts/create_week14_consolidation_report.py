...
def create_consolidation_report():
    ...
    if enhanced_dir.exists():
        ...
        if features_path.exists():
            ...
            print(
                f"   ✅ Enhanced features: {len(df)} games, {len(df.columns)} features"
            )
    ...
    report_files = ["week14_master_report.md"]
    ...
    completion = {
        "training_data": consolidation_report["assets"]
        .get("training_data", {})
        .get("status")
        == "exists",
        "enhanced_features": bool(
            consolidation_report["assets"].get("enhanced_features", {}).get("features")
        ),
        "predictions_unified": "week14_predictions_unified.json"
        in consolidation_report["assets"].get("predictions", {}),
        "analysis_comprehensive": "week14_comprehensive_analysis.json"
        in consolidation_report["assets"].get("analysis", {}),
        "reports_master": "week14_master_report.md"
        in consolidation_report["assets"].get("reports", {}),
...
"""
    ...
    if consolidation_report["assets"].get("enhanced_features", {}).get("features"):
        ...
        games_info = consolidation_report["assets"]["enhanced_features"].get(
            "games", {}
        )
        md_content += f"""
...
"""
    for filename, info in consolidation_report["assets"].get("predictions", {}).items():
        md_content += (
            f"- `{info.get('path', filename)}` ({info.get('count', 0)} records)\n"
        )
