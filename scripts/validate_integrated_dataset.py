...
class DatasetValidator:
    ...
    def validate(self) -> dict:
        ...
        if feature_count < 80:
            results["warnings"].append(
                f"Feature count ({feature_count}) is lower than expected (86)"
            )
            ...
            if col in df.columns:
                ...
                if missing > 0:
                    ...
                    if missing > len(df) * 0.1:  # More than 10% missing
                        results["warnings"].append(
                            f"{col}: {missing} missing values ({100 * missing / len(df):.1f}%)"
                        )
        ...
        if len(duplicate_keys) > 0:
            results["warnings"].append(
                f"Found {len(duplicate_keys)} duplicate game_keys"
            )
    ...
    def _generate_report(self, results: dict, df: pd.DataFrame):
        ...
        with open(self.report_path, "w") as f:
            ...
            f.write(
                f"Validation Status: {'✅ PASS' if results['valid'] else '❌ FAIL'}\n"
            )
