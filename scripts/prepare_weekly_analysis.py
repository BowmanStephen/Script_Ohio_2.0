...
def fetch_week_schedule(week: int, season: int, api_key: str) -> Dict[str, Any]:
    ...
    try:
        from project_management.TOOLS_AND_CONFIG.fetch_future_week import (
            fetch_week_data,
        )
...
def validate_previous_week_results(week: int, season: int) -> Dict[str, Any]:
    ...
    try:
        ...
        prev_week_games = training_df[
            (training_df["season"] == season) & (training_df["week"] == previous_week)
        ]

        if len(prev_week_games) == 0:
            print(f"⚠️  No Week {previous_week} {season} games found in training data")
            print(
                f"   You may need to update training data with Week {previous_week} results"
            )
            return {
                "status": "warning",
            ...
            print(
                f"✅ Found {len(prev_week_games)} Week {previous_week} {season} games in training data"
            )
            return {
                "status": "success",
...
def check_week_data_availability(week: int) -> Dict[str, Any]:
    ...
    for file_type, file_path in data_files.items():
        if file_path is not None and file_path.exists():
            ...
            print(
                f"⚠️  {file_type}: Not found"
                + (f" (expected at {file_path})" if file_path else "")
            )
...
def suggest_next_steps(
    ...
    if validation_results.get("previous_week_games_found", 0) == 0:
        ...
        print(
            f"   python project_management/core_tools/data_workflows.py refresh-training --max-week {week}"
        )

    # Check if models need retraining
    old_models = [
        name for name, info in model_results.items() if info.get("status") == "old"
    ]
    if old_models:
...
def main():
    ...
    parser.add_argument(
        "--week", type=int, default=13, help="Week number to prepare (default: 13)"
    )
    parser.add_argument(
    ...
    print("=" * 60)
    print(
        f"Week {week} Analysis Preparation - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print("=" * 60)
