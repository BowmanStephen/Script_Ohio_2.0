    ...
    try:
        ...
        for i, cell in enumerate(nb.get("cells", [])):
            if cell.get("cell_type") == "code" and i > 0:  # Skip first cell
                ...
                has_duplicate = any(
                    pattern in source_text for pattern in config_import_patterns
                )
        ...
        print(
            f"Error fixing duplicate imports in {notebook_path}: {e}", file=sys.stderr
        )
        return False
...
def fix_duplicate_data_dir_assignments(notebook_path: Path) -> bool:
    ...
    try:
        ...
        print(
            f"Error fixing duplicate DATA_DIR in {notebook_path}: {e}", file=sys.stderr
        )
        return False
