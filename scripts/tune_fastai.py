...
def load_training_data():
    ...
    if (
        "home_win" not in df.columns
        and "home_points" in df.columns
        and "away_points" in df.columns
    ):
...
def main():
    ...
    logger.info("=" * 80)
    logger.info(
        "✅ You can now retrain the FastAI model with these optimized parameters"
    )
