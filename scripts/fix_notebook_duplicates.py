...
def fix_notebook_duplicates(
    notebook_path: Path, create_backup: bool = True
) -> Tuple[bool, List[str]]:
    """
    ...
    for idx, cell in enumerate(cells):
        ...
        if data_dir_count > 1:
            ...
            if len(new_lines) < len(source_lines):
                ...
                modified = True
                changes.append(
                    f"Removed {data_dir_count - 1} duplicate DATA_DIR assignment(s) from cell {idx}"
                )
        ...
        if current_year_count > 1:
            ...
            if len(new_lines) < len(source_lines):
                ...
                modified = True
                changes.append(
                    f"Removed {current_year_count - 1} duplicate current_year assignment(s) from cell {idx}"
                )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python fix_notebook_duplicates.py <notebook1.ipynb> [notebook2.ipynb ...]"
        )
    ...
    for arg in sys.argv[1:]:
        ...
        if path.is_file() and path.suffix == ".ipynb":
            ...
            print(
                f"Warning: {arg} is not a valid notebook file, skipping",
                file=sys.stderr,
            )
