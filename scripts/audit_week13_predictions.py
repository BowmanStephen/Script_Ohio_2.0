    ...
    for home_col, away_col in prob_cols:
        if home_col in df.columns:
            if away_col and away_col in df.columns:
                ...
                if len(invalid) > 0:
                    print(
                        f"⚠️  {len(invalid)} games with {home_col}/{away_col} not summing to 1.0"
                    )
                    issues.append(f"Probability sum issue in {home_col}/{away_col}")
                    # Show first few examples
                    for idx in invalid.head(3).index:
                        print(
                            f"   Game {idx}: {df.loc[idx, home_col]:.3f} + {df.loc[idx, away_col]:.3f} = {prob_sum.loc[idx]:.3f}"
                        )
                else:
        ...
        if col in df.columns:
            ...
            if len(extreme) > 0:
                ...
                for idx in extreme.head(3).index:
                    print(
                        f"   {df.loc[idx, 'away_team']} @ {df.loc[idx, 'home_team']}: {df.loc[idx, col]:.1f}"
                    )
            else:
    ...
    for col in conf_cols:
        if col in df.columns:
            ...
            if len(invalid_conf) > 0:
                print(
                    f"⚠️  {len(invalid_conf)} games with invalid {col} (outside 0-100)"
                )
