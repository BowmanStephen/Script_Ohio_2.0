        ...
        try:
            ...
            try:
                ...
                if is_week12_agent:
                    ...
                    logger.warning(
                        f"Could not instantiate {class_name} directly, trying to extract from weekly agent: {type_err}"
                    )
                    # Extract from weekly agent (code below)
                    agent_instance = self._extract_from_weekly_agent(
                        file_path, class_name, agent_class
                    )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Could not instantiate {class_name} with fallback: {e}"
                        )
                        return None
            except Exception as e:
                if is_week12_agent:
                    ...
                    logger.warning(
                        f"Error instantiating {class_name}, attempting to extract from weekly agent: {e}"
                    )
                    agent_instance = self._extract_from_weekly_agent(
                        file_path, class_name, agent_class
                    )
            ...
            if hasattr(agent_instance, "capabilities"):
                ...
                for cap in agent_instance.capabilities:
                    ...
                    if cap_name and cap_name not in seen_capabilities:
                        ...
                        logger.warning(
                            f"Duplicate capability '{cap_name}' found in {class_name}, skipping duplicate."
                        )

            # Get permission level name
            perm_level = (
                agent_instance.permission_level.name
                if hasattr(agent_instance.permission_level, "name")
                else str(agent_instance.permission_level)
            )
            ...
            logger.error(
                f"Error extracting metadata for {class_name} in {file_path}: {e}"
            )
            return None

    def _extract_from_weekly_agent(
        self, file_path: Path, class_name: str, agent_class: Type
    ) -> Optional[Any]:
        ...
        try:
            ...
            with open(file_path, "r") as f:
                ...
                weekly_import_match = re.search(
                    r"from agents\.weekly_(\w+)_agent import (\w+)", source
                )
                if weekly_import_match:
                    weekly_module_name = (
                        f"agents.weekly_{weekly_import_match.group(1)}_agent"
                    )
                    ...
                    try:
                        ...
                        class MockAgentInstance:
                            def __init__(self, weekly_agent, agent_class):
                                ...
                                self.name = agent_class.__name__.replace(
                                    "Agent", " Agent"
                                )

                        agent_instance = MockAgentInstance(weekly_agent, agent_class)
                        logger.info(
                            f"Successfully extracted capabilities from weekly agent for {class_name}"
                        )
                        return agent_instance
                    except Exception as weekly_e:
                        logger.warning(
                            f"Could not extract from weekly agent for {class_name}: {weekly_e}"
                        )
                        return None
                else:
                    logger.warning(
                        f"Could not find weekly agent import for {class_name}"
                    )
                    return None
        except Exception as extract_e:
            logger.warning(
                f"Error extracting capabilities from weekly agent for {class_name}: {extract_e}"
            )
            return None
...
class ExampleExtractor:
        ...
        for path in search_paths:
            ...
            for py_file in path.rglob("*.py"):
                if (
                    ...
                    and agent_type in py_file.name.lower()
                ):
            ...
            try:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.FunctionDef) and node.name.startswith(
                        ...
                        example_code = self._extract_agent_usage_from_function(
                            node, lines, class_name_variants
                        )
                                    ...
                                    if isinstance(node.value.func, ast.Name):
                                        if node.value.func.id in class_name_variants:
                                            ...
                                            if example_lines:
                                                ...
                                                if (
                                                    example_code
                                                    and example_code not in examples
                                                ):
                                                    examples.append(example_code)
    ...
    def _extract_agent_usage_from_function(
        self,
        func_node: ast.FunctionDef,
        lines: List[str],
        class_name_variants: List[str],
    ) -> str:
        ...
        for stmt in func_node.body:
            ...
            if isinstance(stmt, (ast.Assign, ast.Expr, ast.Call)):
                ...
                stmt_end = (
                    stmt.end_lineno if hasattr(stmt, "end_lineno") else stmt.lineno
                )
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            if (
                ...
                and node.func.attr == "register_agent_class"
            ):
