...
def consolidate_week14_predictions():
    ...
    if enhanced_path.exists():
        ...
        if "matchup" not in enhanced_df.columns:
            enhanced_df["matchup"] = (
                enhanced_df["away_team"] + " @ " + enhanced_df["home_team"]
            )
        ...
        new_cols = [
            ...
            if c not in unified_df.columns and c != "matchup"
        ]
        if new_cols:
            unified_df = unified_df.merge(
                enhanced_df[["matchup"] + new_cols], on="matchup", how="left"
            )
            print(
                f"   Added {len(new_cols)} additional columns from enhanced predictions"
            )
    ...
    print(
        f"   Games with confidence grades: {unified_df['confidence_grade'].notna().sum()}"
    )

    # Check for any inconsistencies
    if len(unified_df) != len(model_preds):
        print(
            f"⚠️  Warning: Unified predictions ({len(unified_df)}) != Model predictions ({len(model_preds)})"
        )
