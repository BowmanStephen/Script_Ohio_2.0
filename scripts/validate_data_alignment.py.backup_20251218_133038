...
def validate_schema_alignment():
    ...
    success = True
    week_files = sorted(
        list(Path("data/weekly_training").glob("training_data_2025_week*.csv"))
    )

    if not week_files:
        print(
            "WARNING: No training_data_2025_week*.csv files found in data/weekly_training/"
        )
        return False
    ...
    for file_path in week_files:
        try:
            ...
            if len(target_columns) != len(ref_columns):
                print(
                    f"FAIL: {file_path.name} has {len(target_columns)} columns, expected {len(ref_columns)}"
                )
                success = False
    ...
    for file_path in week_files:
        try:
            ...
            if not completed.empty:
                calc_margin = (
                    completed["away_points"] - completed["home_points"]
