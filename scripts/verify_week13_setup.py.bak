...
def verify_week13_games() -> tuple[bool, list[str]]:
    ...
    try:
        ...
        if len(week13_games) > 0:
            ...
            teams = set(
                week13_games["home_team"].tolist() + week13_games["away_team"].tolist()
            )
...
def verify_week12_results() -> tuple[bool, list[str]]:
    ...
    try:
        ...
        if len(completed) == len(week12_games):
            ...
            messages.append(
                f"⚠️ {len(week12_games) - len(completed)} Week 12 games still pending"
            )
            return False, messages
    except Exception as e:
...
def verify_agents() -> tuple[bool, list[str]]:
    ...
    agent_files = [
        ...
        (
            "agents/weekly_prediction_generation_agent.py",
