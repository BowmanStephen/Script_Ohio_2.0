...
def parse_markdown_plan(md_path: Path) -> Dict[str, Any]:
            ...
            if line.startswith("##") and not line.startswith("###"):
                ...
                if field_match:
                    ...
                    if field_name == "id":
                        ...
                        current_task["steps"] = [
                            s.strip() for s in steps_str.split(",") if s.strip()
                        ]
                ...
                if config_match:
                    ...
                    if config_key == "parallel_execution":
                        workflow_config["parallel_execution"] = (
                            config_value.lower() == "true"
                        )
                    elif config_key == "error_recovery":
                        workflow_config["error_recovery"] = (
                            config_value.lower() == "true"
                        )
    ...
    plan_data = {
        "metadata": {"title": title, "objective": objective, "version": "1.0"},
        "tasks": tasks,
        "steps": steps,
...
def validate_plan_structure(plan_data: Dict[str, Any]) -> bool:
    ...
    if len(tasks) > 0:
        ...
        for i, task in enumerate(tasks):
            ...
            if task_keys != first_task_keys:
                raise ValueError(
                    f"Task {i} has inconsistent structure. Expected keys: {first_task_keys}, got: {task_keys}"
                )
    ...
    if len(steps) > 0:
        ...
        for i, step in enumerate(steps):
            ...
            if step_keys != first_step_keys:
                raise ValueError(
                    f"Step {i} has inconsistent structure. Expected keys: {first_step_keys}, got: {step_keys}"
                )
    ...
    for step in steps:
        if step["task_id"] not in task_ids:
            raise ValueError(
                f"Step {step['id']} references invalid task_id: {step['task_id']}"
            )
...
def convert_to_workflow_definition(plan_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        from agents.workflow_automator_agent import WorkflowStep, WorkflowStepType
    except SyntaxError:
...
Examples:
  ...
  python scripts/plan_to_workflow.py plan.toon --execute
    ...
    parser.add_argument("plan_path", help="Path to plan file (.toon, .json, or .md)")

    parser.add_argument(
        "--output", "-o", help="Output file path for workflow definition (JSON)"
    )
        ...
        if plan_path.suffix == ".toon":
            ...
            if args.toon:
                ...
                try:
                    ...
                    print(
                        f"Warning: Could not convert to TOON format: {e}",
                        file=sys.stderr,
                    )
            ...
            try:
                ...
                print(
                    f"Error: Unsupported file format: {plan_path.suffix}",
                    file=sys.stderr,
                )
                sys.exit(1)
    except Exception as e:
    ...
    if args.execute:
        print(
            "Execution via WorkflowAutomatorAgent not yet implemented in CLI",
            file=sys.stderr,
        )
        print(
            "Use the orchestrator or agent system to execute workflows", file=sys.stderr
        )
