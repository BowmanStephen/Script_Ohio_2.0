...
def run_syntax_validation(project_root: Path) -> Dict[str, Any]:
    ...
    for py_file in project_root.rglob("*.py"):
        if any(
            skip in str(py_file)
            for skip in ["__pycache__", ".venv", "venv", ".git", "node_modules"]
        ):
            continue
...
def compare_performance(
    baseline_file: Optional[Path], current_metrics: Dict[str, Any]
) -> Dict[str, Any]:
                    ...
                    comparison["improvements"].append(
                        {
                            ...
                            "current": current_time,
                            "decrease_percent": (
                                (baseline_time - current_time) / baseline_time
                            )
                            * 100,
                        }
                    )
...
def main():
    parser = argparse.ArgumentParser(description="Run consolidation validation")
    parser.add_argument(
        "--baseline", help="Path to baseline metrics JSON file for comparison"
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    if args.baseline:
        ...
        validation_results["performance_comparison"] = compare_performance(
            baseline_path, validation_results
        )
    ...
    validation_results["overall_status"] = (
        "pass" if (syntax_ok and tests_ok and imports_ok) else "fail"
    )
    ...
    print("Summary:")
    print(
        f"  - Syntax: {'✅ PASS' if syntax_ok else '❌ FAIL'} ({validation_results['syntax_validation']['valid_files']}/{validation_results['syntax_validation']['total_files']} files)"
    )
    print(
        f"  - Tests: {'✅ PASS' if tests_ok else '❌ FAIL'} ({validation_results['test_execution']['passed']} passed, {validation_results['test_execution']['failed']} failed)"
    )
    print(
        f"  - Imports: {'✅ PASS' if imports_ok else '❌ FAIL'} ({validation_results['import_validation']['successful_imports']}/{validation_results['import_validation']['total_imports']} modules)"
    )
    print(
        f"  - Overall: {'✅ PASS' if validation_results['overall_status'] == 'pass' else '❌ FAIL'}"
    )
