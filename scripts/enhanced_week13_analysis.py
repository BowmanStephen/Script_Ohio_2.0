...
class EnhancedWeek13Analyzer:
    ...
    def load_week13_data(self) -> Dict:
        ...
        try:
            ...
            if comp_file.exists():
                ...
                print(
                    f"  ✅ Loaded comprehensive predictions: {data['comprehensive'].shape}"
                )
    ...
    def analyze_conference_championships(self, data: Dict) -> Dict:
        ...
        for matchup in championship_matchups:
            ...
            game = df[
                ...
                | (
                    (df["home_team"] == matchup["away"])
                    & (df["away_team"] == matchup["home"])
                )
            ]
            ...
            if len(game) > 0:
                ...
                win_prob = (
                    game_data["home_win_probability"]
                    if matchup["home"] == game_data["home_team"]
                    else game_data["away_win_probability"]
                )
                ...
                championship_analysis["playoff_implications"].append(
                    playoff_implication
                )

                print(
                    f"  🏈 {matchup['conference']}: {matchup['home']} vs {matchup['away']}"
                )
                print(
                    f"     Prediction: {game_data['predicted_winner']} ({win_prob:.1%})"
                )
    ...
    def analyze_rivalry_games(self, data: Dict) -> Dict:
        ...
        for rivalry in rivalries:
            ...
            game = df[
                (
                    (df["home_team"].isin(rivalry["teams"]))
                    & (df["away_team"].isin(rivalry["teams"]))
                )
            ]
            ...
            if len(game) > 0:
                ...
                competitive_score = (
                    abs(home_win_prob - 0.5) * 2
                )  # 0 = perfectly balanced, 1 = one-sided
                ...
                game_data["rivalry_intensity"] = (
                    "EXTREME"
                    if rivalry["name"] in ["The Game", "Iron Bowl", "Bedlam"]
                    else "HIGH"
                )
                ...
                print(
                    f"  ⚔️  {rivalry['name']}: {game_data['home_team']} vs {game_data['away_team']}"
                )
                print(
                    f"     Prediction: {game_data['predicted_winner']} ({game_data['confidence_level']})"
                )
    ...
    def analyze_playoff_implications(self, data: Dict) -> Dict:
        ...
        for _, game in df.iterrows():
            ...
            contenders_in_game = [
                team for team in teams_involved if team in projected_contenders
            ]
            ...
            if len(contenders_in_game) > 0:
                ...
                loser = (
                    teams_involved[0]
                    if teams_involved[1] == winner
                    else teams_involved[1]
                )
    ...
    def _assess_playoff_impact(
        self, winner: str, loser: str, contenders: List[str]
    ) -> str:
    ...
    def generate_top_matchups(self, data: Dict) -> Dict:
        ...
        def calculate_matchup_score(row):
            ...
            if (
                abs(row["home_adjusted_epa"]) > 0.2
                and abs(row["away_adjusted_epa"]) > 0.2
            ):
                score += 2

            # Championship games
            if any(
                champ in f"{row['home_team']} {row['away_team']}"
        ...
        for _, game in top_matchups.iterrows():
            ...
            print(
                f"  {game_dict['matchup_rank']}. {game_dict['home_team']} vs {game_dict['away_team']}"
            )
            print(f"     Category: {game_dict['category']}")
            print(
                f"     Prediction: {game_dict['predicted_winner']} by {abs(game_dict['predicted_margin']):.1f}"
            )
            ...
            recommendations.append(
                {
                    ...
                    "games": [
                        game["conference"]
                        for game in self.results["conference_championships"]["games"]
                    ],
                }
            )
    ...
    def save_enhanced_analysis(self):
        ...
        analysis_file = (
            analysis_dir
            / f"enhanced_week13_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        summary_file = (
            analysis_dir
            / f"enhanced_week13_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
    ...
    def _save_markdown_summary(self, file_path: Path):
        ...
        with open(file_path, "w") as f:
            ...
            if (
                "conference_championships" in self.results
                and self.results["conference_championships"]
            ):
                ...
                for game in self.results["conference_championships"].get("games", []):
                    ...
                    f.write(
                        f"- **Matchup:** {game['home_team']} vs {game['away_team']}\n"
                    )
                    f.write(
                        f"- **Prediction:** {game['predicted_winner']} ({game['confidence_level']})\n"
                    )
                    f.write(
                        f"- **Margin:** {abs(game['predicted_margin']):.1f} points\n\n"
                    )
            ...
            if "top_matchups" in self.results and self.results["top_matchups"]:
                ...
                for game in self.results["top_matchups"].get("top_10", [])[:5]:
                    f.write(
                        f"### {game['matchup_rank']}. {game['home_team']} vs {game['away_team']}\n"
                    )
                    f.write(f"- **Category:** {game['category']}\n")
                    f.write(
                        f"- **Prediction:** {game['predicted_winner']} by {abs(game['predicted_margin']):.1f}\n"
                    )
