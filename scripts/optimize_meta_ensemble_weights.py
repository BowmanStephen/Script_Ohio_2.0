...
from scripts.load_prediction_tracker import load_multiple_weeks, load_prediction_tracker
...
def load_ohio_model_predictions(weeks: List[int], season: int = 2025) -> pd.DataFrame:
    ...
    for week in weeks:
        ...
        prediction_paths = [
            ...
            / "predictions"
            / f"week{week}"
            / f"week{week}_predictions_all_*.csv",
            project_root
            / "predictions"
            / f"week{week}"
            / f"week{week}_predictions.csv",
        ...
        for pattern in prediction_paths:
            ...
            if pattern.exists():
                try:
                    ...
                    if (
                        "ensemble_margin" in df.columns
                        or "predicted_margin" in df.columns
                    ):
                        df["week"] = week
                        df["season"] = season
...
def match_games(
    ohio_df: pd.DataFrame, tracker_df: pd.DataFrame, results_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    merged = merged.merge(
        results_df[["game_key", "actual_margin"]], on="game_key", how="inner"
    )
...
def optimize_meta_ensemble_weights(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    ohio_col = (
        "ensemble_margin" if "ensemble_margin" in df.columns else "predicted_margin"
    )
    ...
    system_average = (
        df["system_average"].values
        if "system_average" in df.columns
        else np.full(len(df), np.nan)
    )
    opening_line = (
        df["opening_line"].values
        if "opening_line" in df.columns
        else np.full(len(df), np.nan)
    )
    sagarin = (
        df["sagarin"].values if "sagarin" in df.columns else np.full(len(df), np.nan)
    )
    ...
    valid_mask = (
        ~np.isnan(ohio_preds)
        & ~np.isnan(actual)
        & (~np.isnan(system_average) | ~np.isnan(opening_line) | ~np.isnan(sagarin))
    )
...
def main():
    ...
    try:
        ...
        if not any(
            (project_root / f"training_data_2025_week{w:02d}.csv").exists()
        ...
        if results_df.empty:
            ...
            if ohio_df.empty:
                ...
                if tracker_df.empty:
                    logger.warning(
                        "⚠️  No Prediction_Tracker data loaded. Using defaults."
                    )
                    default_weights = {
                        "ohio_weight": 0.35,
                        "system_average_weight": 0.25,
        ...
        if "validation_mae" in default_weights and not np.isnan(
            default_weights["validation_mae"]
        ):
            logger.info(
                f"Validation MAE: {default_weights['validation_mae']:.2f} points"
            )
            logger.info(
                f"Baseline MAE: {default_weights.get('baseline_mae', np.nan):.2f} points"
            )
