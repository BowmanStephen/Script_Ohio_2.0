...
class Week13DataVerifier:
    ...
    def verify_starter_pack_data(self) -> Dict[str, Any]:
        ...
        week13_games = games_df[
            (games_df["season"] == 2025) & (games_df["week"] == 13)
        ].copy()
        ...
        games_with_scores = week13_games[
            week13_games["home_points"].notna() & week13_games["away_points"].notna()
        ]
        ...
        result = {
            ...
            "teams": {
                "home_teams": sorted(
                    week13_games["home_team"].dropna().unique().tolist()
                ),
                "away_teams": sorted(
                    week13_games["away_team"].dropna().unique().tolist()
                ),
            },
        }
    ...
    def verify_features_generation(self) -> Dict[str, Any]:
        ...
        required_files = {
            ...
            "features_model_compatible": self.week13_data_dir
        ...
        if features_86_path.exists():
            try:
                ...
                if has_86_features:
                    ...
                    print(
                        f"⚠️  Features file has {len(df.columns)} columns (expected 86)"
                    )
            except Exception as e:
    ...
    def verify_predictions(self) -> Dict[str, Any]:
        ...
        if not self.week13_predictions_dir.exists():
            print(
                f"⚠️  Week 13 predictions directory not found: {self.week13_predictions_dir}"
            )
            return {"status": "error", "message": "Directory not found"}
        ...
        prediction_files = {
            ...
            "predictions_csv": self.week13_predictions_dir
                ...
                try:
                    if file_path.suffix == ".json":
                        ...
                        with open(file_path, "r") as f:
                            data = json.load(f)
                        file_status[file_type]["data_keys"] = (
        ...
        for model_name, model_path in model_files.items():
            if model_path.exists():
                ...
                print(
                    f"✅ {model_name} model: Last modified {model_date.strftime('%Y-%m-%d')}"
                )
            else:
    ...
    def verify_analysis(self) -> Dict[str, Any]:
        ...
        if not self.week13_analysis_dir.exists():
            print(
                f"⚠️  Week 13 analysis directory not found: {self.week13_analysis_dir}"
            )
            return {"status": "error", "message": "Directory not found"}

        # Check for analysis files
        analysis_files = {
            "comprehensive": self.week13_analysis_dir
            / "week13_comprehensive_analysis.json",
            "power_rankings": self.week13_analysis_dir / "week13_power_rankings.csv",
            "strategic_recommendations": self.week13_analysis_dir
            ...
            if exists:
                try:
                    if file_path.suffix == ".json":
                        ...
                        with open(file_path, "r") as f:
                            data = json.load(f)
                        file_status[file_type]["has_data"] = (
                            len(data) > 0 if isinstance(data, (dict, list)) else False
                        )
                        print(f"✅ {file_type}: Found")
                    else:
                        df = pd.read_csv(file_path, low_memory=False)
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        all_checks_passed = all(
            check.get("status") in ["complete", "migrated"] for check in checks.values()
        )

        summary = {
            "overall_status": "complete" if all_checks_passed else "incomplete",
            "starter_pack_ready": checks.get("starter_pack", {}).get("status")
            == "complete",
            "migration_complete": checks.get("migration", {}).get("status")
            == "migrated",
            "features_generated": checks.get("features", {}).get("status")
            == "complete",
            "predictions_exist": checks.get("predictions", {}).get("status")
            == "complete",
