...
def generate_prediction_insights(unified_df):
    ...
    if insights["total_predictions"] > 0:
        insights["prediction_patterns"] = {
            "home_win_rate": insights["home_win_predictions"]
            ...
            "blowout_rate": insights["blowout_predictions"]
            / insights["total_predictions"],
        }
...
def generate_recommendations(unified_df):
    ...
    for _, row in unified_df.iterrows():
        ...
        if rec_type == "high_confidence":
            ...
            recommendation = (
                f"Moderate confidence bet ({confidence * 100:.0f}% confidence)"
            )
...
def generate_enhanced_analysis_report(unified_df):
    ...
    # Summary statistics
    if "ensemble_confidence" in unified_df.columns:
        report_json["summary_statistics"]["average_confidence"] = float(
            unified_df["ensemble_confidence"].mean()
        )
        report_json["summary_statistics"]["high_confidence_games"] = int(
            (unified_df["ensemble_confidence"] > 0.75).sum()
        )
        report_json["summary_statistics"]["moderate_confidence_games"] = int(
            (
                (unified_df["ensemble_confidence"] >= 0.5)
                & (unified_df["ensemble_confidence"] <= 0.75)
            ).sum()
        )
        report_json["summary_statistics"]["low_confidence_games"] = int(
            (unified_df["ensemble_confidence"] < 0.5).sum()
        )

    if "predicted_margin" in unified_df.columns:
        report_json["summary_statistics"]["average_predicted_margin"] = float(
            unified_df["predicted_margin"].mean()
        )
    ...
    if "home_win_probability" in unified_df.columns:
        ...
        report_json["key_insights"].append(
            f"Average home win probability: {avg_home_win:.1%}"
        )

    if "predicted_margin" in unified_df.columns:
        avg_margin = unified_df["predicted_margin"].mean()
        report_json["key_insights"].append(
            f"Average predicted margin: {avg_margin:.1f} points"
        )
...
def main():
    ...
    print(
        f"   ✅ Saved: {recommendations_path} ({len(recommendations_df)} recommendations)"
    )
