...
def load_meta_ensemble_weights() -> Dict[str, float]:
    ...
    if not weights_path.exists():
        ...
        logger.warning(
            "  Using default weights: Ohio=0.35, System Avg=0.25, Opening=0.20, Sagarin=0.10, SP+=0.10"
        )
        return {
            "ohio_weight": 0.35,
    ...
    try:
        ...
        logger.info(
            f"  Ohio={weights.get('ohio_weight', 0.35):.3f}, "
...
def load_ohio_predictions(week: int, season: int = 2025) -> pd.DataFrame:
    ...
    prediction_paths = [
        ...
        / "predictions"
        / f"week{week}"
        / f"week{week}_predictions_enhanced.csv",
...
def match_games(ohio_df: pd.DataFrame, tracker_df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(
        f"✅ Matched {merged['system_average'].notna().sum()}/{len(merged)} games with Prediction_Tracker data"
    )
...
def create_meta_ensemble_predictions(week: int, season: int = 2025) -> pd.DataFrame:
    ...
    if tracker_df.empty:
        logger.warning(
            "⚠️  No Prediction_Tracker data available. Using Ohio Model only."
        )
        # Still create output with Ohio Model predictions
        result_df = ohio_df.copy()
        result_df["meta_ensemble_margin"] = result_df.get(
            "ensemble_margin", result_df.get("predicted_margin", np.nan)
        )
    ...
    ohio_col = (
        "ensemble_margin"
        if "ensemble_margin" in merged_df.columns
        else "predicted_margin"
    )

    # Get SP+ margin if available (from enhanced predictions)
    sp_plus_col = (
        "sp_predicted_margin" if "sp_predicted_margin" in merged_df.columns else None
    )
    ...
    if ohio_col in merged_df.columns:
        merged_df["ohio_vs_system_avg_diff"] = merged_df[ohio_col] - merged_df.get(
            "system_average", np.nan
        )
        merged_df["ohio_vs_opening_diff"] = merged_df[ohio_col] - merged_df.get(
            "opening_line", np.nan
        )
        merged_df["meta_vs_ohio_diff"] = (
            merged_df["meta_ensemble_margin"] - merged_df[ohio_col]
        )

    # Coverage statistics
    meta_count = merged_df["meta_ensemble_margin"].notna().sum()
    logger.info(
        f"  ✅ Meta-ensemble predictions: {meta_count}/{len(merged_df)} ({meta_count / len(merged_df) * 100:.1f}%)"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create multi-system meta-ensemble predictions"
    )
    parser.add_argument(
    ...
    try:
        ...
        logger.info(
            f"Meta-ensemble predictions: {result_df['meta_ensemble_margin'].notna().sum()}"
        )
