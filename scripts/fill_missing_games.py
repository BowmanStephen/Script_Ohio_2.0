...
# CFBD imports will be moved to after argument parsing
# try:
#     import cfbd
...
try:
    ...
    print(
        "WARNING: Could not import AdvancedMetricsBuilder. Advanced metrics will be limited."
    )
    ADVANCED_METRIC_COLUMNS = ()
...
class MissingGamesFiller:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.error(f"❌ CFBD package not available: {e}")
            print("ERROR: cfbd package not installed. Install with: pip install cfbd")
            sys.exit(1)
    ...
    def load_missing_games(self) -> pd.DataFrame:
        ...
        if not self.missing_games_path.exists():
            ...
            logger.info(
                "Run scripts/identify_missing_games.py first to generate missing_games.csv"
            )
            return pd.DataFrame()
    ...
    def _fetch_games_for_week(self, season: int, week: int) -> List[Dict]:
        ...
        try:
            ...
            logger.error(
                f"API error fetching games for {season} Week {week}: {e.status} - {e.reason}"
            )
            return []
        except Exception as e:
    ...
    def fetch_game_data(
        ...
        season: int,
        week: int,
        games_cache: Dict[tuple, List[Dict]] = None,
    ) -> Optional[Dict]:
        ...
        if games_cache is None:
            logger.warning(
                f"fetch_game_data called without cache - this is inefficient!"
            )
        ...
        logger.warning(
            f"Game {game_id} not found in API response for {season} Week {week}"
        )
        return None
        ...
        try:
            ...
            if talent_data:
                for talent in talent_data:
                    talent_dict_obj = (
                        talent.to_dict() if hasattr(talent, "to_dict") else {}
                    )
                    team_name = (
                        talent_dict_obj.get("team")
                        or talent_dict_obj.get("school")
                        or talent_dict_obj.get("name")
                    )
                ...
                logger.info(
                    f"Fetched talent ratings for {len(talent_dict)} teams for {year}"
                )
        except Exception as e:
        ...
        try:
            ...
            for line in lines:
                ...
                if line_game_id == game_id:
                    spread = line_dict.get("spread") or line_dict.get(
                        "formatted_spread"
                    )
                    if spread:
                        try:
    ...
    def enrich_game_data(
        self, game_dict: Dict, elo_dict: Dict, talent_dict: Dict
    ) -> Dict:
    ...
    def generate_features(
        self, games_df: pd.DataFrame, plays_df: Optional[pd.DataFrame] = None
    ) -> pd.DataFrame:
        ...
        if AdvancedMetricsBuilder and self.api_client:
            try:
                ...
                advanced_metrics = builder.build_metrics_for_games(
                    result_df, plays_df if plays_df is not None else pd.DataFrame()
                )
        ...
        for col in expected_columns:
            if col not in result_df.columns:
                ...
                if fallback_config:
                    default_value = fallback_config.get_fallback_value(
                    ...
                    if default_value is not None:
                        ...
                        try:
                            hist_df = pd.read_csv(
                                self.training_data_path, low_memory=False
                            )
        ...
        result_df = result_df[
            [col for col in training_df.columns if col in result_df.columns]
        ]
    ...
    def fetch_all_missing_games(self, missing_games_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in missing_games_df.iterrows():
            ...
            games_by_week[cache_key].append({"game_id": game_id, "row": row.to_dict()})

        logger.info(
            f"Grouped {len(missing_games_df)} games into {len(games_by_week)} unique (season, week) combinations"
        )
        logger.info(
            f"This will require {len(games_by_week)} API calls instead of {len(missing_games_df)} calls"
        )
        ...
        logger.info(
            f"API calls made: {actual_calls} (reduced from {original_calls} - {reduction_pct:.1f}% reduction)"
        )
        if failed_games:
    ...
    def integrate_games(self, new_games_df: pd.DataFrame) -> bool:
        ...
        if duplicates:
            logger.warning(
                f"Found {len(duplicates)} duplicate game IDs, removing from new games"
            )
            new_games_df = new_games_df[
                ~new_games_df["id"].astype(str).isin(duplicates)
            ]
        ...
        combined_df = pd.concat(
            [existing_df, new_games_df], ignore_index=True, sort=False
        )
        ...
        logger.info(
            f"Saved updated training data: {len(combined_df)} total games ({len(existing_df)} existing + {len(new_games_df)} new)"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fill missing games in training data from CFBD API"
    )
    parser.add_argument(
