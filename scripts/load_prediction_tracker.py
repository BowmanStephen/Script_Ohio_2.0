...
def load_prediction_tracker_file(
    file_path: Path, week: Optional[int] = None, season: int = 2025
) -> pd.DataFrame:
    """
        ...
        if week is None:
            ...
            if "week" in filename.lower():
                try:
                    ...
                    logger.warning(
                        f"⚠️  Could not extract week from filename: {filename}"
                    )
                    week = None
...
def load_prediction_tracker(
    week: Optional[int] = None, season: int = 2025, file_path: Optional[Path] = None
) -> pd.DataFrame:
    """
    ...
    logger.warning(
        f"⚠️  No Prediction_Tracker file found for week {week}, season {season}"
    )
    return pd.DataFrame()
...
def main():
    ...
    for system in systems:
        if system in df.columns:
            ...
            logger.info(
                f"  {system}: {coverage}/{len(df)} ({coverage / len(df) * 100:.1f}%)"
            )
