...
def load_week13_data():
    ...
    week13_features_path = (
        PROJECT_ROOT / "data" / "week13" / "enhanced" / "week13_features_86.csv"
    )
    week13_games_path = (
        PROJECT_ROOT / "data" / "week13" / "enhanced" / "week13_enhanced_games.csv"
    )
    ...
    if week13_games_path.exists():
        ...
        logger.info(
            f"✅ Loaded Week 13 enhanced games: {len(week13_data['enhanced_games'])} games"
        )

    # Load from starter pack games.csv
    if games_path.exists():
        all_games = pd.read_csv(games_path, low_memory=False)
        week13_games = all_games[
            (all_games["season"] == 2025) & (all_games["week"] == 13)
        ]
        if len(week13_games) > 0:
            week13_data["games"] = week13_games
            logger.info(
                f"✅ Loaded Week 13 games from starter pack: {len(week13_games)} games"
            )
...
def transform_week13_for_models(
    week13_data: Dict[str, pd.DataFrame],
        ...
        if "game_id" in features_df.columns and "id" in games_df.columns:
            ...
            for col in ["home_team", "away_team"]:
                if f"{col}_games" in features_df.columns:
                    features_df[col] = features_df[f"{col}_games"].fillna(
                        features_df[col]
                    )
...
def generate_predictions(
    ...
    # Get game metadata
    if "features" in week13_data:
        games_meta = week13_data["features"][
            ["game_id", "home_team", "away_team"]
        ].copy()
    elif "enhanced_games" in week13_data:
        games_meta = week13_data["enhanced_games"][
    ...
    for idx, (_, game) in enumerate(games_meta.iterrows()):
        ...
        if pd.notna(pred.get("predicted_margin")):
            pred["predicted_winner"] = (
                pred["home_team"] if pred["predicted_margin"] > 0 else pred["away_team"]
            )
...
def main():
    ...
    try:
        ...
        transformed_data = transform_week13_for_models(
            week13_data, model_features, defaults
        )
