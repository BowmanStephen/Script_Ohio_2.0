...
def to_markdown_table(df):
    ...
    data_rows = []
    for row in rows:
        data_rows.append("| " + " | ".join(str(val).ljust(w) for val, w in zip(row, widths)) + " |")
...
def main():
    ...
    def format_row(row):
        ...
        date_str = date_val.strftime("%Y-%m-%d %H:%M") if pd.notnull(date_val) else "TBD"
        ...
        # Calculations
        talent_diff = (home_talent - away_talent) if (home_talent and away_talent) else 0.0
    ...
    md_lines.append("- **Talent Edge**: Team with significantly higher 247Sports composite ratings.")
    md_lines.append("- **Value**: Where our model differs from the market line by > 3 points.")
