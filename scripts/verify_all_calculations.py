...
class CalculationVerifier:
    ...
    def load_data(self) -> bool:
        ...
        self.historical_data = self.training_data[
            self.training_data["season"] < 2025
        ].copy()
        self.current_season_data = self.training_data[
            self.training_data["season"] == 2025
        ].copy()
    ...
    def verify_epa_calculations(self) -> Dict[str, Any]:
        ...
        missing_columns = [
            col for col in epa_columns if col not in self.training_data.columns
        ]
        ...
        for col in epa_columns:
            if col in self.training_data.columns:
                ...
                print(
                    f"{col}: mean={mean_val:.4f}, std={std_val:.4f}, range=[{min_val:.4f}, {max_val:.4f}]"
                )
        ...
        for col in epa_columns:
            if (
                col in self.historical_data.columns
                and col in self.current_season_data.columns
            ):
                ...
                deviation = (
                    abs(current_mean - hist_mean) / hist_std if hist_std > 0 else 0
                )
                ...
                print(
                    f"{col}: Hist={hist_mean:.4f}±{hist_std:.4f}, 2025={current_mean:.4f} [{status}]"
                )
    ...
    def verify_opponent_adjustments(self) -> Dict[str, Any]:
        ...
        adjusted_features = [
            col for col in self.training_data.columns if "adjusted" in col.lower()
        ]
    ...
    def verify_feature_engineering(self) -> Dict[str, Any]:
        ...
        non_numeric_features = [
            col
            for col in required_feature_groups.values()
            for col in col
            if col in self.training_data.columns and col not in numeric_columns
        ]
    ...
    def verify_model_compatibility(self) -> Dict[str, Any]:
        ...
        missing_metadata = [
            col for col in required_metadata if col not in self.training_data.columns
        ]
    ...
    def verify_temporal_consistency(self) -> Dict[str, Any]:
        ...
        if len(self.current_season_data) > 0:
            week_distribution = (
                self.current_season_data["week"].value_counts().sort_index()
            )
            games_before_week5 = self.current_season_data[
                self.current_season_data["week"] < 5
            ]
        ...
        historical_weeks = (
            self.historical_data["week"].value_counts().sort_index()
    ...
    def verify_data_leakage(self) -> Dict[str, Any]:
        ...
        if (
            len(self.current_season_data) > 0
            and "home_adjusted_epa" in self.current_season_data.columns
        ):
            ...
            if "margin" in self.current_season_data.columns:
                ...
                print(
                    f"Correlation between home_adjusted_epa and margin: {correlation:.4f}"
                )

                # Correlation should be positive but not too high (< 0.9)
                if correlation > 0.9:
                    leakage_warning = (
                        "High correlation detected - possible data leakage"
                    )
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "epa_calculations": checks.get("epa_calculations", {}).get(
                "status", "unknown"
            ),
            "opponent_adjustments": checks.get("opponent_adjustments", {}).get(
                "status", "unknown"
            ),
            "feature_engineering": checks.get("feature_engineering", {}).get(
                "status", "unknown"
            ),
            "model_compatibility": checks.get("model_compatibility", {}).get(
                "status", "unknown"
            ),
            "temporal_consistency": checks.get("temporal_consistency", {}).get(
                "status", "unknown"
            ),
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if checks.get("opponent_adjustments", {}).get("status") != "pass":
            recommendations.append(
                "Verify opponent adjustment methodology (subtraction-based)"
            )
        ...
        if checks.get("model_compatibility", {}).get("status") != "pass":
            recommendations.append(
                "Ensure all models exist and data types match requirements"
            )

        if checks.get("temporal_consistency", {}).get("status") != "pass":
            recommendations.append(
                "Consider filtering to Week 5+ for opponent adjustments"
            )

        if checks.get("data_leakage", {}).get("status") != "pass":
            recommendations.append(
                "Review data leakage concerns - verify no future data used"
            )
