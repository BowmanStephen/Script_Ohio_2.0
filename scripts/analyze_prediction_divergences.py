...
def categorize_divergences(
    df: pd.DataFrame, divergence_threshold: float = 10.0
) -> pd.DataFrame:
    """
    ...
    if "phwin" in df.columns and "phcover" in df.columns:
        ...
        result_df["consensus_aligned"] = (
            result_df["abs_divergence"] <= 5.0
        )  # Low divergence = aligned
...
def generate_divergence_report(df: pd.DataFrame, week: int, season: int = 2025) -> str:
    ...
    if "high_divergence" in df.columns:
        ...
        if high_div_count > 0:
            ...
            report_lines.append("")
            report_lines.append(
                "| Home Team | Away Team | Ohio Model | System Avg | Divergence |"
            )
            report_lines.append(
                "|-----------|----------|------------|------------|------------|"
            )
            ...
            for _, row in top_div.iterrows():
                ...
                report_lines.append(
                    f"| {home} | {away} | {ohio:.1f} | {sys_avg:.1f} | {div:.1f} |"
                )
            report_lines.append("")
    ...
    if "ohio_correct" in df.columns:
        ...
        if "ohio_wrong_consensus_right" in df.columns:
            ...
            report_lines.append(
                f"- **Learning Opportunities** (Ohio wrong, System Avg right): {learning_opps} games"
            )

        if "ohio_right_consensus_wrong" in df.columns:
            unique_value = df["ohio_right_consensus_wrong"].sum()
            report_lines.append(
                f"- **Unique Value Cases** (Ohio right, System Avg wrong): {unique_value} games"
            )

        if "consensus_aligned" in df.columns:
            aligned = df["consensus_aligned"].sum()
            report_lines.append(
                f"- **Consensus Aligned** (Both correct): {aligned} games"
            )
...
def analyze_divergences(
    week: int, season: int = 2025, divergence_threshold: float = 10.0
) -> pd.DataFrame:
    """
    ...
    if "ohio_wrong_consensus_right" in result_df.columns:
        logger.info(
            f"Learning opportunities: {result_df['ohio_wrong_consensus_right'].sum()}"
        )
    if "ohio_right_consensus_wrong" in result_df.columns:
        logger.info(
            f"Unique value cases: {result_df['ohio_right_consensus_wrong'].sum()}"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=10.0,
        help="Divergence threshold (default: 10.0)",
    )
