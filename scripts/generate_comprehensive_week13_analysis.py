...
try:
    ...
    logging.warning(
        "Could not import RandomForestScorePredictor from src.models.random_forest"
    )
...
class PredictionGeneratorAgent(AgentBase):
    ...
    def generate_predictions(self, models: Dict, features: Dict) -> Dict[str, Any]:
        ...
        if "ridge" in models and "ridge" in features:
            try:
                preds["ridge_margin"] = float(
                    models["ridge"].predict(features["ridge"])[0]
                )
            except Exception as e:
...
class FeatureImportanceAgent(AgentBase):
    ...
    def calculate_importance(
        self, models: Dict, feature_extractor: FeatureExtractorAgent
    ) -> Dict[str, Any]:
        importance = {}
            ...
            try:
                ...
                if hasattr(model, "feature_importances_"):
                    ...
                    if len(feats) == len(imps):
                        # Zip and sort
                        sorted_imp = sorted(
                            zip(feats, imps), key=lambda x: x[1], reverse=True
                        )
            ...
            try:
                ...
                if hasattr(model, "coef_"):
                    ...
                    if len(feats) == len(coefs):
                        sorted_coef = sorted(
                            zip(feats, np.abs(coefs)), key=lambda x: x[1], reverse=True
                        )
...
class NarrativeGeneratorAgent(AgentBase):
    ...
    def generate_narrative(
        self, game_info: pd.Series, ensemble: Dict, importance: Dict
    ) -> str:
        ...
        prob = (
            ensemble["win_prob"]
            if ensemble["win_prob"] > 0.5
            else 1.0 - ensemble["win_prob"]
        )
...
class ArtifactGeneratorAgent(AgentBase):
    ...
    def save_artifacts(self, analysis_results: List[Dict], week: int, output_dir: Path):
        ...
        with open(md_path, "w") as f:
            ...
            for item in analysis_results:
                f.write(
                    f"## {item['game_info'].get('home_team')} vs {item['game_info'].get('away_team')}\n"
                )
