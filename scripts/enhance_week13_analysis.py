...
class Week13AnalysisEnhancer:
    ...
    def analyze_conference_championship_implications(
        ...
        for conf in conference_games["home_conference"].unique():
            ...
            conference_analysis[conf] = {
                ...
                "teams": sorted(
                    set(
                        conf_games["home_team"].tolist()
                        + conf_games["away_team"].tolist()
                    )
                ),
                "games": conf_games[["home_team", "away_team", "id"]].to_dict(
                    "records"
                ),
            }
    ...
    def analyze_playoff_implications(self, games_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        existing_analysis_path = (
            self.analysis_dir / "week13_comprehensive_analysis.json"
        )
        playoff_games = []
        ...
        if existing_analysis_path.exists():
            with open(existing_analysis_path, "r") as f:
                ...
                top_teams = {
                    team["team"] for team in team_rankings[:20]
                }  # Top 20 teams
        ...
        for _, game in games_df.iterrows():
            ...
            if (
                home_elo > 1800 or away_elo > 1800
            ):  # Top teams typically have Elo > 1800
    ...
    def analyze_situational_factors(self, games_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        situational_analysis = {
            ...
            "neutral_site_games": len(
                games_df[games_df.get("neutral_site", False) == True]
            ),
            "conference_games": len(
                games_df[
                    (games_df.get("home_conference", "").notna())
                    & (games_df.get("away_conference", "").notna())
                    & (
                        games_df.get("home_conference")
                        == games_df.get("away_conference")
                    )
                ]
            ),
        }
        ...
        if "start_date" in games_df.columns:
            games_df["start_date"] = pd.to_datetime(
                games_df["start_date"], errors="coerce"
            )
    ...
    def generate_advanced_statistical_breakdowns(
        self, games_df: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        # Robust FBS check
        if (
            "home_classification" in games_df.columns
            and "away_classification" in games_df.columns
        ):
            fbs_games_count = len(
                games_df[
        ...
        statistical_breakdowns = {
            ...
            "teams_involved": len(
                set(
                    games_df["home_team"].dropna().tolist()
                    + games_df["away_team"].dropna().tolist()
                )
            ),
            "conferences_represented": len(
                set(
        ...
        if "home_elo" in games_df.columns or "home_start_elo" in games_df.columns:
            home_elo_col = (
                "home_elo" if "home_elo" in games_df.columns else "home_start_elo"
            )
            away_elo_col = (
                "away_elo" if "away_elo" in games_df.columns else "away_start_elo"
            )

            all_elos = pd.concat(
                [games_df[home_elo_col].dropna(), games_df[away_elo_col].dropna()]
            )
