...
def clean_data():
    file_path = Path("data/processed/training/master_training_data_v2.csv")
    ...
    mask_zero_scores = (
        (df["season"] == 2025) & (df["home_points"] == 0) & (df["away_points"] == 0)
    )
    ...
    if len(bad_games) > 0:
        logger.warning(
            f"Found {len(bad_games)} games with 0-0 scores in 2025. These are likely unplayed games."
        )
        ...
        logger.info(
            f"✅ Removed {len(bad_games)} bad games. Saved cleaned data to {file_path}"
        )
