...
from src.data_sources import CFBDClientConfig, CFBDRESTDataSource
...
def main():
    ...
    if not api_key:
        logger.warning(
            "⚠️ CFBD_API_KEY not found. Will not be able to fetch missing scores from API."
        )
        rest_ds = None
    else:
        try:
            ...
            logger.warning(
                f"⚠️ Failed to initialize CFBD client: {e}. Will not fetch missing scores."
            )
            rest_ds = None
    ...
    try:
        ...
        if game_stats_path.exists():
            ...
            for idx, row in starter_df.iterrows():
                if row["season"] == 2025 and (
                    pd.isna(row.get("home_points")) or pd.isna(row.get("away_points"))
                ):
                    gid = row["id"]
                    if gid in score_map:
            ...
            logger.info(
                f"✅ Repaired scores for {updated_count} games using game_stats"
            )
        else:
            logger.warning(
                f"⚠️ Game stats file not found at {game_stats_path}. Cannot repair missing scores."
            )
    ...
    if 12 in starter_weeks and 12 in training_weeks:
        ...
        logger.info(
            f"  Week 12 - Starter: {starter_week12_count}, Training: {training_week12_count}"
        )

        if starter_week12_count > training_week12_count:
            missing_week12 = starter_week12_count - training_week12_count
            logger.info(
                f"  ⚠️  Missing {missing_week12} week 12 games to complete the week"
            )
    ...
    for _, game in missing_games_df.iterrows():
        ...
        if pd.isna(home_points) or pd.isna(away_points):
            ...
            if rest_ds is not None:
                ...
                logger.info(
                    f"🔍 Fetching scores from API for {home_team} vs {away_team} (Week {week}, ID: {game_id})"
                )
                ...
                if api_home_points is not None and api_away_points is not None:
                    ...
                    api_fetched_count += 1
                    logger.info(
                        f"✅ Fetched scores from API: {home_team} {home_points} - {away_points} {away_team}"
                    )
                    time.sleep(0.2)  # Rate limiting
                else:
                    # Game not found or not completed yet - skip it
                    logger.warning(
                        f"⚠️ Skipping game {home_team} vs {away_team} (Week {week}) - no scores available (likely future game)"
                    )
                ...
                logger.warning(
                    f"⚠️ Skipping game {game.get('home_team')} vs {game.get('away_team')} (Week {game.get('week')}) due to missing scores (no API client)."
                )
                skipped_future_games += 1
                continue
        ...
        for col in training_columns:
            if col in game.index:
                ...
                if col == "id":
                    processed_game[col] = game.get(
                        "id", f"missing_{len(missing_games_processed)}"
                    )
    ...
    if not missing_games_processed:
        logger.warning(
            "⚠️ All missing games were skipped due to missing scores. No new games to integrate."
        )
        logger.info(
            f"📊 API Fetch Summary: {api_fetched_count} scores fetched, {skipped_future_games} future games skipped"
        )
    ...
    logger.info(
        f"📊 API Fetch Summary: {api_fetched_count} scores fetched from API, {skipped_future_games} future games skipped"
    )

    # Show week breakdown
    for week in sorted(missing_games_df_processed["week"].unique()):
        week_count = len(
            missing_games_df_processed[missing_games_df_processed["week"] == week]
        )
    ...
    combined_df = pd.concat(
        [training_df, missing_games_df_processed], ignore_index=True
    )
    ...
    logger.info(
        f"  ✅ Schema consistency maintained: {len(combined_df.columns)} columns"
    )
