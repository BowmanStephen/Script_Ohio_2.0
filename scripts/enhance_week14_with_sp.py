    ...
    try:
        ...
        for col in df.columns:
            ...
            if col_lower == "team":
                ...
                col_map["sp_def"] = col
            elif "st sp+" in col_lower or (
                "special" in col_lower and "sp+" in col_lower
            ):
                col_map["sp_st"] = col
        ...
        for _, row in df.iterrows():
            ...
            sp_rating = (
                row[col_map["sp"]]
                if "sp" in col_map and pd.notna(row[col_map["sp"]])
                else None
            )
            sp_off_rating = (
                row[col_map["sp_off"]]
                if "sp_off" in col_map and pd.notna(row[col_map["sp_off"]])
                else None
            )
            sp_def_rating = (
                row[col_map["sp_def"]]
                if "sp_def" in col_map and pd.notna(row[col_map["sp_def"]])
                else None
            )
            sp_st_rating = (
                row[col_map["sp_st"]]
                if "sp_st" in col_map and pd.notna(row[col_map["sp_st"]])
                else None
            )
            sp_rank = (
                row[col_map["sp_rank"]]
                if "sp_rank" in col_map and pd.notna(row[col_map["sp_rank"]])
                else None
            )
            sp_off_rank = (
                row[col_map["sp_off_rank"]]
                if "sp_off_rank" in col_map and pd.notna(row[col_map["sp_off_rank"]])
                else None
            )
            sp_def_rank = (
                row[col_map["sp_def_rank"]]
                if "sp_def_rank" in col_map and pd.notna(row[col_map["sp_def_rank"]])
                else None
            )
            sp_st_rank = (
                row[col_map["sp_st_rank"]]
                if "sp_st_rank" in col_map and pd.notna(row[col_map["sp_st_rank"]])
                else None
            )
            ...
            if sp_rating is not None:
                try:
                    ...
                    if (
                        normalized_name != team_str
                        and normalized_name not in team_ratings
                    ):
...
def load_calibration_weights() -> Dict[str, float]:
    ...
    if not weights_path.exists():
        ...
        return {"ohio_model_weight": 0.65, "sp_weight": 0.35}
...
def calculate_calibrated_margin(
    ensemble_margin: float, sp_margin: Optional[float], weights: Dict[str, float]
) -> Optional[float]:
...
def enhance_predictions(
    df: pd.DataFrame, ratings: Dict[str, Dict[str, float]], weights: Dict[str, float]
) -> pd.DataFrame:
    ...
    for idx, row in enhanced_df.iterrows():
        ...
        if home_key and away_key:
            ...
            if (
                home_rating.get("sp_off") is not None
                and away_rating.get("sp_def") is not None
            ):
                enhanced_df.at[idx, "offense_matchup"] = home_rating.get(
                    "sp_off"
                ) - away_rating.get("sp_def")

            if (
                home_rating.get("sp_def") is not None
                and away_rating.get("sp_off") is not None
            ):
                enhanced_df.at[idx, "defense_matchup"] = home_rating.get(
                    "sp_def"
                ) - away_rating.get("sp_off")

            if home_rating.get("sp") is not None and away_rating.get("sp") is not None:
                enhanced_df.at[idx, "sp_matchup_advantage"] = home_rating.get(
                    "sp"
                ) - away_rating.get("sp")
...
def main():
    ...
    try:
        ...
        predictions_path = (
            project_root / "predictions" / "week14" / "week14_model_predictions.csv"
        )
