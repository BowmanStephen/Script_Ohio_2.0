...
def update_agents_md(agent_metadata: Dict[str, AgentMetadata]):
    ...
    for agent in sorted_agents:
        capabilities_list = ", ".join(
            [f"`{cap['name']}`" for cap in agent.capabilities]
        )
    ...
    logger.info(
        f"AGENTS.md updated. Removed {len(sections_to_remove)} duplicate section(s)."
    )
            ...
            all_caps.append(
                {
                    ...
                    "permission": cap.get(
                        "permission_required", agent.permission_level
                    ),
                }
            )
...
def generate_registry(agent_metadata: Dict[str, AgentMetadata]):
    ...
    registry: Dict[str, Any] = {"agent_types": {}, "capability_index": {}}
...
def generate_readme(agent_metadata: Dict[str, AgentMetadata]):
    ...
    content += (
        "- [Agent Cheat Sheet](../../AGENTS.md#agent-cheat-sheet) - Quick reference\n\n"
    )
...
def main():
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Validate documentation consistency"
    )
    parser.add_argument(
        "--output", type=str, help="Output directory", default=str(DOCS_DIR)
    )
    ...
    for class_name, metadata in agent_metadata.items():
        if class_name in mapping:
            ...
            if class_name not in EXCLUDED_AGENTS:
                logger.warning(
                    f"Agent {class_name} not found in orchestrator mappings and not excluded."
                )
    ...
    if args.update:
        ...
        if not args.validate:
            print(
                "Use --update to generate documentation or --validate to check consistency."
            )
