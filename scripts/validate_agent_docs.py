...
class DocValidator:
    ...
    def validate(self) -> List[str]:
        ...
        if not registry_path.exists():
            self.issues.append(
                "capability_registry.json not found. Run --update first."
            )
            return self.issues
            ...
            try:
                ...
                for cap_name in capabilities.keys():
                    ...
                    for method_name in method_names:
                        ...
                        if len(cap_parts) > 1:
                            ...
                            for part in cap_parts:
                                if (
                                    len(part) > 3
                                    and part.lower() in method_name.lower()
                                ):
                    ...
                    if not found_in_method:
                        ...
                        if (
                            "orchestrator" in class_name.lower()
                            or "orchestrator" in agent_type.lower()
                        ):
                            # For orchestrators, check if capability name appears in phase method names
                            phase_methods = [
                                m
                                for m in method_names
                                if "phase" in m.lower() or "execute" in m.lower()
                            ]
                            for phase_method in phase_methods:
                                ...
                                for part in cap_parts:
                                    if (
                                        len(part) > 3
                                        and part.lower() in phase_method.lower()
                                    ):
                                        found_in_method = True
                                        break
                                if found_in_method:
    ...
    def _validate_doc_completeness(self, registry: Dict[str, Any]):
        ...
        for agent_type in registry.get("agent_types", {}).keys():
            ...
            for cap_name, cap_info in agent_info.get("capabilities", {}).items():
                if (
                    not cap_info.get("description")
                    or cap_info["description"].strip() == ""
                ):
                    self.issues.append(
