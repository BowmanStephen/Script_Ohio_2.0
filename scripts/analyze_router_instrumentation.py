...
class RouterInstrumentationAnalyzer:
    ...
    def __init__(self, log_file: str = "logs/app.log"):
        ...
        self.patterns = {
            ...
            "submit": re.compile(
                r"SUBMIT:\s+request_id=(\S+)\s+queue_size=(\d+)\s+priority=(\d+)"
            ),
            "process_start": re.compile(r"PROCESS_START:\s+queue_size=(\d+)"),
            "permission_denied": re.compile(
                r"PERMISSION_DENIED:\s+request_id=(\S+)\s+agent_type=(\S+)"
            ),
            "permission_granted": re.compile(
                r"PERMISSION_GRANTED:\s+request_id=(\S+)\s+agent_type=(\S+)"
            ),
            "agent_not_found": re.compile(
                r"AGENT_NOT_FOUND:\s+request_id=(\S+)\s+agent_type=(\S+)"
            ),
            "priority_sort": re.compile(r"PRIORITY_SORT:\s+queue_size=(\d+)"),
            "error_caught": re.compile(
                r"ERROR_CAUGHT:\s+request_id=(\S+)\s+error=(.+)"
            ),
            "status_retrieved": re.compile(r"STATUS_RETRIEVED:\s+request_id=(\S+)"),
            "router_call": re.compile(
                r"ROUTER_CALL:\s+request_id=(\S+)(?:\s+agent_count=(\d+))?(?:\s+action=(\S+))?(?:\s+router_overhead_ms=([\d.]+))?(?:\s+total_time_ms=([\d.]+))?(?:\s+successful_agents=(\d+))?(?:\s+failed_agents=(\d+))?"
            ),
    ...
    def evaluate_thresholds(self) -> Dict[str, Any]:
        ...
        results["permission_denials"] = {
            ...
            "pass": self.metrics["permission_denials"]
            <= self.thresholds["permission_denials_threshold"],
        ...
        results["priority_sorts"] = {
            ...
            "pass": self.metrics["priority_sorts"]
            <= self.thresholds["priority_sorts_threshold"],
        ...
        avg_overhead = (
            sum(self.metrics["router_overhead_ms"])
            / len(self.metrics["router_overhead_ms"])
        ...
        results["agent_not_found"] = {
            ...
            "pass": self.metrics["agent_not_found_count"]
            <= self.thresholds["agent_not_found_threshold"],
        ...
        results["errors_caught"] = {
            ...
            "pass": self.metrics["errors_caught"]
            <= self.thresholds["errors_caught_threshold"],
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        avg_queue_submit = (
            sum(self.metrics["queue_sizes_at_submit"])
            / len(self.metrics["queue_sizes_at_submit"])
            if self.metrics["queue_sizes_at_submit"]
            else 0.0
        )
        avg_queue_process = (
            sum(self.metrics["queue_sizes_at_process"])
            / len(self.metrics["queue_sizes_at_process"])
            if self.metrics["queue_sizes_at_process"]
            else 0.0
        )
        avg_overhead = (
    ...
    def print_human_readable_summary(self, report: Dict[str, Any]) -> None:
        ...
        print("-" * 80)
        print(
            f"All Thresholds Passed: {'✅ YES' if report['summary']['all_thresholds_passed'] else '❌ NO'}"
        )
        ...
        print(
            f"Average Queue Size (Process): {metrics['average_queue_size_at_process']}"
        )
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Analyze router instrumentation logs")
    parser.add_argument(
        "--log-file",
    ...
    parser.add_argument(
        "--output", help="Output JSON report file (default: print to stdout)"
    )
    parser.add_argument(
        "--human-readable", action="store_true", help="Print human-readable summary"
    )
