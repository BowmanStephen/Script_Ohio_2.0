...
def get_team_metrics(team_name, training_df):
    ...
    if len(home_games) > 0:
        ...
        epa_allowed_values.extend(
            home_games["home_adjusted_epa_allowed"].dropna().tolist()
        )
        success_values.extend(home_games["home_adjusted_success"].dropna().tolist())
        success_allowed_values.extend(
            home_games["home_adjusted_success_allowed"].dropna().tolist()
        )
    ...
    if len(away_games) > 0:
        ...
        epa_allowed_values.extend(
            away_games["away_adjusted_epa_allowed"].dropna().tolist()
        )
        success_values.extend(away_games["away_adjusted_success"].dropna().tolist())
        success_allowed_values.extend(
            away_games["away_adjusted_success_allowed"].dropna().tolist()
        )
