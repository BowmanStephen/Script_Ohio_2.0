...
def validate_notebook(nb_path: Path) -> tuple[bool, list[str]]:
    ...
    if config_import_count > MAX_CONFIG_IMPORTS:
        issues.append(
            f"Too many config imports ({config_import_count}), possible duplicates"
        )
    ...
    if has_stale_outputs:
        issues.append(
            "Has stale outputs with template strings (config.current_year not evaluated)"
        )
    ...
    for idx, cell in enumerate(cells):
        if cell.get("cell_type") == "code":
            ...
            if data_dir_count > 1:
                issues.append(
                    f"Cell {idx} has {data_dir_count} DATA_DIR assignments (duplicate)"
                )
    ...
    for idx, cell in enumerate(cells):
        if cell.get("cell_type") == "code":
            ...
            if current_year_count > 1:
                issues.append(
                    f"Cell {idx} has {current_year_count} current_year assignments (duplicate)"
                )
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        ...
        if starter_pack_dir.exists():
            ...
            print(
                "Usage: python validate_starter_pack_notebooks.py [notebook1.ipynb ...]"
            )
            print(
                "   or: python validate_starter_pack_notebooks.py (validates all in starter_pack/)"
            )
            sys.exit(1)
    else:
