...
def get_model_features(models: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    if "fastai" in models:
        try:
            ...
            fastai_features = getattr(
                getattr(models["fastai"], "dls", None), "x_names", None
            )
            if fastai_features:
                ...
                logger.info(
                    f"FastAI features (fallback to XGBoost schema): {len(features['fastai'])} features"
                )
        except Exception as e:
            logger.warning(
    ...
    if "logistic" in models:
        if hasattr(models["logistic"], "feature_names_in_"):
            ...
            logger.info(
                f"Logistic Regression features: {len(features['logistic'])} features"
            )
        elif get_model_config_func:
            ...
            if config:
                ...
                logger.info(
                    f"Logistic Regression features (from config): {len(features['logistic'])} features"
                )
        else:
            ...
            logger.info(
                f"Logistic Regression features (fallback): {len(features['logistic'])} features"
            )
    ...
    if "random_forest" in models:
        try:
            ...
            if hasattr(models["random_forest"], "features"):
                ...
                logger.info(
                    f"Random Forest features: {len(features['random_forest'])} features"
                )
            elif get_model_config_func:
                ...
                if config:
                    ...
                    logger.info(
                        f"Random Forest features (from config): {len(features['random_forest'])} features"
                    )
            else:
                # Fallback feature list
                ...
                logger.info(
                    f"Random Forest features (fallback): {len(features['random_forest'])} features"
                )
        except Exception as e:
            logger.warning(
...
def load_week14_data():
    ...
    week14_training_path = (
        PROJECT_ROOT / "data" / "training" / "weekly" / "training_data_2025_week14.csv"
    )
    ...
    if week14_training_path.exists():
        ...
        week14_features_path = (
            PROJECT_ROOT / "data" / "week14" / "enhanced" / "week14_features_86.csv"
        )
        if week14_features_path.exists():
            week14_data["features"] = pd.read_csv(week14_features_path)
            logger.info(
                f"✅ Loaded Week 14 features from enhanced dir: {len(week14_data['features'])} games"
            )
...
def get_feature_defaults_from_training() -> Dict[str, float]:
    ...
    if training_path.exists():
        try:
            ...
            logger.info(
                f"✅ Loaded {len(defaults)} feature defaults from training data"
            )
        except Exception as e:
            logger.warning(
...
def tune_ridge_hyperparameters(
    training_data: pd.DataFrame, feature_cols: List[str], target_col: str = "margin"
) -> Dict[str, Any]:
    """
...
def tune_xgboost_hyperparameters(
    training_data: pd.DataFrame, feature_cols: List[str], target_col: str = "home_win"
) -> Dict[str, Any]:
    """
    ...
    base_model = xgb.XGBClassifier(
        eval_metric="logloss", random_state=77, use_label_encoder=False
    )

    # Grid search
    grid = GridSearchCV(
        base_model, param_grid, cv=tscv, scoring="accuracy", n_jobs=-1, verbose=1
    )
...
def margin_to_probability(
    margin: Optional[float], scale: float = 14.0
) -> Optional[float]:
...
def load_sp_ratings_from_csv(
    ...
    try:
        ...
        for col in df.columns:
            ...
            if (
                "sp+" in col_lower
                and "off" not in col_lower
                and "def" not in col_lower
                and "st" not in col_lower
            ):
                sp_col = col
                break
        ...
        if sp_col is None:
            logger.warning(
                f"⚠️  Could not find SP+ column in CSV. Available columns: {list(df.columns)}"
            )
            return {}
...
def fetch_sp_fpi_ratings(
    season: int = 2025, use_csv: bool = True
) -> Dict[str, Dict[str, float]]:
    """
    ...
    ratings = {
        ...
        if v.get("sp") is not None or v.get("fpi") is not None
    }
...
def calculate_sp_predicted_margin(
    home_sp: Optional[float], away_sp: Optional[float], neutral_site: bool = False
) -> Optional[float]:
...
def transform_week14_for_models(
    ...
    for model_name, required_features in model_features.items():
        ...
        for feature in required_features:
            if feature in features_df.columns:
                aligned_df[feature] = pd.to_numeric(
        ...
        for feature in required_features:
            if feature in aligned_df.columns:
                aligned_df[feature] = aligned_df[feature].fillna(
                    defaults.get(feature, 0.0)
                )

        transformed[model_name] = aligned_df
        logger.info(
            f"✅ Transformed data for {model_name}: {len(aligned_df)} games, {len(aligned_df.columns)} features"
        )
...
def create_ensemble_prediction(
    ...
    if probs:
        ...
        ensemble["ensemble_away_win_prob"] = 1.0 - ensemble["ensemble_home_win_prob"]
...
def generate_predictions(
    ...
    if "ridge" in models and "ridge" in transformed_data:
        try:
            ...
            ridge_probs = np.array([margin_to_probability(val) for val in ridge_preds])
    ...
    if "fastai" in models and "fastai" in transformed_data:
        try:
            ...
            for idx in range(len(X_fastai)):
                pred_class, pred_idx, probs = models["fastai"].predict(
                    X_fastai.iloc[idx]
                )
    ...
    if "logistic" in models and "logistic" in transformed_data:
        try:
            ...
            logistic_proba = models["logistic"].predict_proba(X_logistic)[
                :, 1
            ]  # Home win probability

            logger.info(
                f"✅ Generated Logistic Regression predictions: {len(logistic_proba)} games"
            )
        except Exception as e:
    ...
    if "random_forest" in models and "random_forest" in transformed_data:
        try:
            ...
            if hasattr(rf_model, "predict"):
                ...
                if (
                    isinstance(rf_model, dict)
                    and "model_home" in rf_model
                    and "model_away" in rf_model
                ):
            ...
            if rf_preds is not None:
                logger.info(
                    f"✅ Generated Random Forest predictions: {len(rf_preds)} games"
                )
        except Exception as e:
    ...
    if sp_ratings:
        try:
            ...
            for idx, (_, game) in enumerate(games_meta.iterrows()):
                ...
                neutral_site = (
                    bool(game.get("neutral_site", False))
            ...
            logger.info(
                f"✅ Generated SP+ predictions: {sp_count}/{len(sp_preds)} games"
            )
        except Exception as e:
    ...
    for idx, (_, game) in enumerate(games_meta.iterrows()):
        ...
        if rf_preds is not None and idx < len(rf_preds):
            ...
            pred["random_forest_home_win_probability"] = (
                float(rf_prob) if rf_prob is not None else np.nan
            )
        else:
        ...
        if sp_preds is not None and idx < len(sp_preds):
            ...
            if not np.isnan(sp_margin_val):
                ...
                pred["sp_home_win_probability"] = (
                    float(sp_prob) if sp_prob is not None else np.nan
                )
            else:
        ...
        sp_margin = (
            float(sp_preds[idx])
            if sp_preds is not None
            and idx < len(sp_preds)
            and not np.isnan(sp_preds[idx])
            else None
        )
        ...
        pred["ensemble_home_win_probability"] = ensemble["ensemble_home_win_prob"]
        pred["ensemble_away_win_probability"] = ensemble["ensemble_away_win_prob"]
...
def run_prediction_pipeline(tune_hyperparameters: bool = False) -> int:
    ...
    try:
            ...
            if training_path.exists():
                ...
                if "ridge" in models and "ridge" in model_features:
                    ridge_params = tune_ridge_hyperparameters(
                        training_df, model_features["ridge"], "margin"
                    )
                    if ridge_params:
                        logger.info(
                            f"💾 Best Ridge alpha: "
                ...
                if "xgb" in models and "xgb" in model_features:
                    xgb_params = tune_xgboost_hyperparameters(
                        training_df, model_features["xgb"], "home_win"
                    )
                    if xgb_params:
                        logger.info(
        ...
        transformed_data = transform_week14_for_models(
            week14_data, model_features, defaults
        )
        ...
        for idx, row in predictions_df.head(10).iterrows():
            ...
            prob = row.get(
                "ensemble_home_win_probability", row.get("home_win_probability", np.nan)
            )
            ...
            if pd.notna(margin):
                conf_str = (
                    f" (confidence: {confidence:.1%})" if pd.notna(confidence) else ""
                )
                logger.info(
                    f"  {away} @ {home}: {winner} by {margin:.1f} points{conf_str}"
                )
            elif pd.notna(prob):
                conf_str = (
                    f" (confidence: {confidence:.1%})" if pd.notna(confidence) else ""
                ...
                logger.info(
                    f"  {away} @ {home}: {winner} ({prob:.1%} home win prob){conf_str}"
                )
            else:
...
if __name__ == "__main__":
    ...
    sys.exit(
        run_prediction_pipeline(tune_hyperparameters=cli_args.tune_hyperparameters)
    )
