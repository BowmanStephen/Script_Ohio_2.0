    ...
    for week in weeks:
        try:
            file_path = get_weekly_training_file(
                week=week, season=season, base_path=PROJECT_ROOT
            )
            if file_path.exists():
...
class LocalFirstGameRecovery:
    ...
    def phase1_extract_from_local(self) -> Dict[str, pd.DataFrame]:
        ...
        if STARTER_PACK_GAMES.exists():
            ...
            try:
                ...
                logger.info(
                    f"✅ Found {len(found_in_starter)} missing games in starter pack"
                )
        ...
        if BACKUP_TRAINING.exists():
            ...
            try:
                ...
                logger.info(
                    f"Loaded {len(backup_training_df)} games from backup training data"
                )
                ...
                logger.info(
                    f"✅ Found {len(found_in_backup)} missing games in backup training data (with 86 features!)"
                )
            except Exception as e:
            ...
            if weekly_file.exists():
                try:
                    ...
                    if len(found_in_this_week) > 0:
                        ...
                        logger.info(
                            f"    ✅ Found {count} missing games in {weekly_file.name} (with 86 features!)"
                        )
                except Exception as e:
        ...
        if found_in_weekly:
            ...
            logger.info(
                f"✅ Total found in weekly training data (weeks 5-13): {total_found_in_weekly} games (all have 86 features!)"
            )
        ...
        if len(still_missing) > 0:
            ...
            logger.info(
                f"📡 API calls needed: {self.stats['api_calls_needed']} (one per unique week/year)"
            )
            logger.info(
                f"   Reduction: {((len(missing_games_df) - self.stats['api_calls_needed']) / len(missing_games_df) * 100):.1f}% fewer calls than naive approach"
            )
        else:
            logger.info(
                "🎉 All missing games found in local sources! ZERO API calls needed!"
            )
    ...
    def phase2_build_features_local(
        self, found_games: Dict[str, pd.DataFrame]
    ) -> pd.DataFrame:
        ...
        if len(found_games.get("found_in_backup", pd.DataFrame())) > 0:
            ...
            logger.info(
                f"✅ Using {len(backup_games)} games from backup (already have 86 features)"
            )
        ...
        if len(found_games.get("found_in_weekly_training", pd.DataFrame())) > 0:
            ...
            logger.info(
                f"✅ Using {len(weekly_games)} games from weekly training data (weeks 5-13, already have 86 features)"
            )
        ...
        if len(found_games.get("found_in_starter", pd.DataFrame())) > 0:
            ...
            logger.info(
                f"🔧 Building features for {len(starter_games)} games from starter pack"
            )
        ...
        if all_games_with_features:
            ...
            logger.info(
                f"✅ Total games with features from local data: {len(combined)}"
            )
            return combined
        else:
    ...
    def _build_basic_features(self, games_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "home_points" in result_df.columns and "away_points" in result_df.columns:
            result_df["margin"] = (
                result_df["home_points"] - result_df["away_points"]
            ).abs()

        logger.info(f"Built basic features for {len(result_df)} games")
        logger.info(
            "NOTE: Full 86-feature engineering should be implemented using local data files"
        )
    ...
    def phase3_api_fetch_if_needed(self, still_missing: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "fill_missing_games", fill_missing_path
            )
            if spec is None or spec.loader is None:
                raise ValueError(
                    f"Unable to load fill_missing_games module spec: {fill_missing_path}"
                )
    ...
    def phase4_integrate_all(
        self, local_games: pd.DataFrame, api_games: pd.DataFrame
    ) -> bool:
    ...
    def run(self, analyze_only: bool = False) -> Dict:
        ...
        api_games_with_features = self.phase3_api_fetch_if_needed(
            found_games.get("still_missing", pd.DataFrame())
        )

        # Phase 4: Integrate all
        success = self.phase4_integrate_all(
            local_games_with_features, api_games_with_features
        )
        ...
        logger.info(
            f"Found in weekly training (weeks 5-13): {self.stats['found_in_weekly_training']}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Recover missing games using local data first"
    )
    parser.add_argument(
        "--analyze-only",
        action="store_true",
