...
def verify_2025_data_completeness() -> Dict[str, Any]:
    ...
    for week in expected_weeks:
        ...
        if week in weeks_present:
            ...
            if (
                "home_points" in week_data.columns
                and "away_points" in week_data.columns
            ):
                games_with_outcomes = week_data[
                    week_data["home_points"].notna() & week_data["away_points"].notna()
                ]
    ...
    report["summary"]["game_counts_by_week"] = {
        week: len(df_2025[df_2025["week"] == week]) for week in expected_weeks
    }
...
def print_report_summary(report: Dict[str, Any]):
    ...
    print(
        f"  Weeks present: {report['summary']['total_weeks_present']}/{report['summary']['total_weeks_expected']}"
    )
    ...
    for week in range(1, 14):
        ...
        if week_key in report["weeks_status"]:
            ...
            print(
                f"  Week {week:2d}: {present} | Games: {games:3d} | File: {file_exists} | Outcomes: {outcomes}"
            )
