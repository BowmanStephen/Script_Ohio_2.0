...
def validate_week14_data():
    ...
    if training_path.exists():
        ...
        if len(df.columns) >= 86:
            validation_results["checks"]["training_features"] = {
                "status": "passed",
            ...
            validation_results["issues"].append(
                f"Training data has only {len(df.columns)} features, expected 86+"
            )
            print(f"   ❌ Feature count: {len(df.columns)} (expected: 86+)")
        ...
        if missing == 0:
            validation_results["checks"]["training_missing"] = {
                "status": "passed",
                "missing_count": 0,
            }
            ...
            validation_results["warnings"].append(
                f"Training data has {missing} missing values across {len(missing_cols)} columns"
            )
        ...
        if not missing_req:
            validation_results["checks"]["training_required_cols"] = {
                "status": "passed"
            }
        ...
        validation_results["checks"]["training_data"] = {
            "status": "failed",
            "path": str(training_path),
        }
        validation_results["issues"].append(
            f"Training data file not found: {training_path}"
        )
    ...
    for model_name, model_path in model_files.items():
        if model_path.exists():
            ...
            validation_results["checks"][f"model_{model_name}"] = {
                "status": "passed",
                "path": str(model_path),
            }
    ...
    for file_name, file_path in pred_files.items():
        if file_path.exists():
            try:
                ...
                validation_results["checks"][f"predictions_{file_name}"] = {
                    "status": "passed",
                    "count": count,
                }
            ...
            validation_results["warnings"].append(
                f"Predictions file not found: {file_path}"
            )
    ...
    for file_name, file_path in web_app_files.items():
        if file_path.exists():
            validation_results["checks"][f"webapp_{file_name}"] = {
                "status": "passed",
                "path": str(file_path),
            }
            ...
            validation_results["warnings"].append(
                f"Web app file not found: {file_path}"
            )
            print(f"   ⚠️  {file_name}: Not found")
