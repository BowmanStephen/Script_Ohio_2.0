...
class PerformanceValidator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.frontend_dist = (
            self.project_root / config["project_structure"]["frontend_dist"]
        )
        self.bundle_size_limit = (
            config.get("bundle_size_limit_kb", 244) * 1024
        )  # Convert to bytes
        self.timeout = config["validation_timeouts"].get("performance", 60)

    def validate(self, scope: str = "full") -> Dict[str, Any]:
        """Run performance validation"""
        results = {"success": True, "frontend": {}, "errors": []}
    ...
    def _validate_frontend_performance(self) -> Dict[str, Any]:
        ...
        exceeded = [
            name
            for name, size in bundle_analysis["bundles"].items()
