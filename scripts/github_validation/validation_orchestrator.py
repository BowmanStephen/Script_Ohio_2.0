...
class ValidationOrchestrator:
    ...
    def run_validation(
        self, validation_type: str = "all", validation_scope: str = "full"
    ) -> Dict[str, Any]:
        """Run validation suite"""
        logger.info(
            f"Running validation: type={validation_type}, scope={validation_scope}"
        )
        ...
        if "security" in validations_to_run:
            results["security"] = self.security_validator.validate(
        ...
        if "performance" in validations_to_run:
            results["performance"] = self.performance_validator.validate(
                scope=validation_scope
            )
    ...
    def run_parallel_validations(
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = {
                executor.submit(task): name for name, task in parallel_tasks.items()
            }
