...
def transform_week12_data(
    features_path: Path, games_path: Path, training_data_path: Path, output_path: Path
) -> bool:
    """
    ...
    try:
        ...
        logger.info(
            f"Loaded {len(features_df)} games with {len(features_df.columns)} feature columns"
        )
        ...
        if overlap:
            logger.info(
                f"Columns in both datasets (will use games_df values): {overlap}"
            )
            ...
            features_df_clean = features_df.drop(
                columns=[col for col in overlap if col in features_df.columns]
            )
        else:
        ...
        logger.info(
            f"Merged columns: {list(merged_df.columns)[:10]}... (showing first 10)"
        )
        ...
        week12_training["start_date"] = pd.to_datetime(
            merged_df["game_date"]
        ...
        # Add points
        week12_training["home_points"] = (
            merged_df["home_points"].fillna(0).astype(float)
        )
        week12_training["away_points"] = (
            merged_df["away_points"].fillna(0).astype(float)
        )

        # Calculate margin
        week12_training["margin"] = (
            week12_training["home_points"] - week12_training["away_points"]
        )
        ...
        feature_cols_to_add = [
            ...
            if col
            not in ["game_id", "home_team", "away_team", "home_talent", "away_talent"]
        ]
        ...
        training_feature_cols = [
            col for col in training_columns if col not in metadata_cols
        ]
        ...
        for col in training_feature_cols:
            if col in merged_df.columns:
                ...
                logger.warning(
                    f"Feature column {col} not found in week12 data, filling with NaN"
                )

        # Add game_key
        week12_training["game_key"] = (
            week12_training["season"].astype(str)
            + "_"
            + week12_training["week"].astype(str)
            + "_"
            + week12_training["home_team"]
            + "_"
            ...
            if col in training_df.columns:
                try:
                    ...
                    if target_dtype == "object":
                        ...
                        week12_training[col] = pd.to_numeric(
                            week12_training[col], errors="coerce"
                        )
                        week12_training[col] = week12_training[col].astype(target_dtype)
                except Exception as e:
                    logger.warning(
                        f"Could not convert column {col} to {training_df[col].dtype}: {e}"
                    )
        ...
        logger.info(
            f"✅ Week12 transformation complete: {len(week12_training)} games, {len(week12_training.columns)} columns"
        )
    ...
    try:
        ...
        if output_path is None:
            output_path = (
                PROJECT_ROOT
                / "model_pack"
                / "2025_starter_pack_migrated_weeks_1_11.csv"
            )

        weeks_1_11.to_csv(output_path, index=False)
        logger.info(
            f"✅ Weeks 1-11 migration complete: {len(weeks_1_11)} games saved to {output_path}"
        )
...
def main():
    ...
    try:
        ...
        week12_features_path = (
            PROJECT_ROOT / "data" / "week12" / "enhanced" / "week12_features_86.csv"
        )
        week12_games_path = (
            PROJECT_ROOT / "data" / "week12" / "enhanced" / "week12_enhanced_games.csv"
        )
        ...
        weeks_1_11_output_path = (
            PROJECT_ROOT / "model_pack" / "2025_starter_pack_migrated_weeks_1_11.csv"
        )
        ...
        if not week12_features_path.exists():
            raise FileNotFoundError(
                f"Week12 features not found: {week12_features_path}"
            )
        ...
        week_distribution = (
            final_df[final_df["season"] == 2025]["week"].value_counts().sort_index()
        )
        ...
        logger.info(
            f"Games without scores (future games): {len(final_df) - len(games_with_scores)}"
        )
        ...
        logger.info(
            f"Games with scores: {len(final_df_with_scores)} (out of {len(final_df)} total)"
        )
        ...
        try:
            ...
            if len(df_2025_with_scores) == 0:
                logger.warning(
                    "⚠️  No 2025 games with scores found. Using 2024 as test set instead."
                )
                ...
                trainer.train_df["home_win"] = (
                    trainer.train_df["home_points"] > trainer.train_df["away_points"]
                ).astype(int)
                trainer.test_df["home_win"] = (
                    trainer.test_df["home_points"] > trainer.test_df["away_points"]
                ).astype(int)

                # Train models manually
                trainer.performance_metrics["ridge"] = trainer.train_ridge_regression()
                trainer.performance_metrics["xgboost"] = (
                    trainer.train_xgboost_classifier()
                )
                trainer.performance_metrics["fastai"] = (
                    trainer.train_fastai_neural_network()
                )
        ...
        for model_name, model_path in model_files.items():
            if model_path.exists():
                ...
                logger.info(
                    f"✅ {model_name} model exists: {model_path.name} ({size_mb:.2f} MB)"
                )
            else:
