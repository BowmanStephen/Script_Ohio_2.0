...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    try:
        results = orchestrator.run_validation(
            validation_type=validation_type, validation_scope=validation_scope
        )
...
Examples:
  ...
  python3 scripts/github_cmd.py rollback --commit abc123
    ...
    push_parser.add_argument(
        "--branch", type=str, help="Target branch (default: current)"
    )
    push_parser.add_argument(
        "--remote", type=str, default="origin", help="Remote name (default: origin)"
    )
    push_parser.add_argument(
        "--skip-tests", action="store_true", help="Skip validation (dangerous)"
    )
    push_parser.add_argument(
    ...
    push_parser.add_argument(
        "--python-only", action="store_true", help="Validate Python only"
    )
    push_parser.add_argument(
        "--frontend-only", action="store_true", help="Validate frontend only"
    )

    # Validate command
    validate_parser = subparsers.add_parser("validate", help="Run validation checks")
    validate_parser.add_argument(
        "--build-only", action="store_true", help="Build validation only"
    )
    validate_parser.add_argument(
        "--tests-only", action="store_true", help="Test validation only"
    )
    validate_parser.add_argument(
        "--security-only", action="store_true", help="Security validation only"
    )
    validate_parser.add_argument(
        "--performance-only", action="store_true", help="Performance validation only"
    )
    validate_parser.add_argument(
        "--python-only", action="store_true", help="Python validation only"
    )
    validate_parser.add_argument(
        "--frontend-only", action="store_true", help="Frontend validation only"
    )

    # Rollback command
    rollback_parser = subparsers.add_parser("rollback", help="Rollback last push")
    rollback_parser.add_argument(
        "--commit", type=str, help="Rollback to specific commit"
    )
    rollback_parser.add_argument(
        "--emergency", action="store_true", help="Emergency rollback mode"
    )
    rollback_parser.add_argument(
        "--no-backup", action="store_true", help="Skip backup branch creation"
    )
