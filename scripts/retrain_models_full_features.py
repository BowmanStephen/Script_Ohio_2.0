...
def load_data(data_path: Path):
    ...
    if use_2025_test:
        ...
        logger.warning(
            "2025 season has no valid scores. Falling back to 2024 as test set."
        )
...
def train_ridge(train_df, test_df, output_dir):
    ...
    grid = GridSearchCV(
        Ridge(), param_grid, cv=tscv, scoring="neg_mean_absolute_error", n_jobs=-1
    )
...
def load_optimized_fastai_params(output_dir):
    ...
    if params_path.exists():
        ...
        logger.info(
            "   Run 'python3 scripts/tune_fastai.py' to optimize hyperparameters."
        )
        return None
...
def train_fastai(train_df, test_df, output_dir):
    ...
    try:
        ...
        logger.info(
            f"Training with: lr={learning_rate:.4f}, layers={layers}, drop={dropout:.2f}, bs={batch_size}"
        )
