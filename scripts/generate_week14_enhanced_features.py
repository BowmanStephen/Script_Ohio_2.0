...
def generate_week14_enhanced_features():
    ...
    if week13_features_path.exists():
        ...
        if missing_columns:
            print(
                f"📝 Adding {len(missing_columns)} missing columns to match Week 13 structure..."
            )
            for col in missing_columns:
                if col == "home_points":
                    ...
                    if "id" in df.columns:
                        ...
                        df["game_id"] = df.apply(
                            lambda row: hash(
                                f"{row['home_team']}_{row['away_team']}_{row['start_date']}"
                            )
                            % 1000000000,
                            axis=1,
                        )
    ...
    metadata = {
        ...
        "files_generated": {"features": str(features_path), "games": str(games_path)},
    }
