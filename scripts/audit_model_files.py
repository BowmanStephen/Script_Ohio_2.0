...
class ModelAuditor:
    ...
    def categorize_models(
        self, model_files: Dict[str, Dict], references: Dict[str, List]
    ) -> Dict[str, Any]:
        ...
        for rel_path, file_info in model_files.items():
            ...
            is_referenced = any(
                filename in ref or filename.endswith(ref) for ref in references.keys()
            )
            ...
            for search_path in search_paths:
                if search_path.exists():
                    found_locations.append(
                        str(search_path.relative_to(self.project_root))
                    )

            if not found_locations:
                existence_report["missing"].append(
                    {"reference": model_ref, "referenced_in": ref_locations}
                )
    ...
    def _generate_markdown_report(self, report: Dict[str, Any]) -> str:
        ...
        for model_path in categories["production"]:
            ...
            md_lines.append(
                f"- `{model_path}` ({file_info['size_mb']} MB, modified: {file_info['modified_time']})"
            )
        ...
        if existence["missing"]:
            for missing in existence["missing"]:
                ...
                md_lines.append(
                    f"  - Referenced in: {', '.join([r['file'] for r in missing['referenced_in']])}"
                )
        else:
        ...
        if fixed_refs:
            md_lines.append(
                "⚠️ The following references use `_fixed` suffix (should be removed):"
            )
