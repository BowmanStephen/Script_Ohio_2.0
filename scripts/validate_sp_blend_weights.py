...
def load_historical_data(weeks: List[int], season: int = 2025) -> pd.DataFrame:
    ...
    for week in weeks:
        ...
        if not week_file.exists():
            ...
            week_file = (
                ...
                / "data"
                / "weekly_training"
                / f"training_data_{season}_week{week:02d}.csv"
            )
    ...
    with_results = combined[
        combined["home_points"].notna() & combined["away_points"].notna()
    ].copy()

    if "margin" not in with_results.columns:
        with_results["margin"] = (
            with_results["home_points"] - with_results["away_points"]
        )
...
def generate_ohio_model_predictions(
...
def fetch_sp_fpi_ratings(season: int = 2025) -> Dict[str, Dict[str, float]]:
    ...
    if not api_key:
        logger.error(
            "❌ CFBD_API_KEY or CFBD_API_TOKEN not set. Cannot fetch SP+/FPI ratings."
        )
        return {}
...
def calculate_sp_fpi_predictions(
    df: pd.DataFrame, ratings: Dict[str, Dict[str, float]]
) -> pd.DataFrame:
...
def optimize_blend_weights(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    def objective(weights: np.ndarray) -> float:
        """Minimize MAE."""
        blended = (
            weights[0] * ohio_valid + weights[1] * sp_valid + weights[2] * fpi_valid
        )
    ...
    logger.info(
        f"  📊 Optimal weights: Ohio={optimal_weights[0]:.3f}, SP+={optimal_weights[1]:.3f}, FPI={optimal_weights[2]:.3f}"
    )
    logger.info(
        f"  📊 Validation MAE: {metrics['mae']:.2f} (baseline: {baseline_metrics['mae']:.2f})"
    )
    logger.info(
        f"  📊 Validation Accuracy: {metrics['accuracy']:.1%} (baseline: {baseline_metrics['accuracy']:.1%})"
    )
...
def main():
    ...
    try:
        ...
        if not ratings:
            logger.warning(
                "⚠️  No ratings fetched. Proceeding with Ohio Model only weights."
            )
            optimal_weights = {
        ...
        logger.info("=" * 80)
        logger.info(
            f"Optimal Weights: Ohio={optimal_weights['ohio_model_weight']:.3f}, "
