...
def fetch_game_id_from_cfbd(
    ...
    try:
        ...
        with cfbd_session() as session:
            ...
            games = games_api.get_games(year=season, week=week, team=home_team)

            if games:
                for game in games:
                    if (
                        game.home_team == home_team and game.away_team == away_team
                    ) or (game.home_team == away_team and game.away_team == home_team):
...
def fix_missing_game_ids():
    ...
    for idx, row in nan_id_games.iterrows():
        ...
        game_id = fetch_game_id_from_cfbd(
            row["home_team"], row["away_team"], int(row["season"]), int(row["week"])
        )
        ...
        if game_id:
            ...
            print(
                f"⚠️ Cannot generate ID for {row['home_team']} vs {row['away_team']} (week {row['week']})"
            )
