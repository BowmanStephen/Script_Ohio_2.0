...
class DataLoaderService:
    ...
    def load_comprehensive_analysis(self) -> Dict[int, Dict]:
        ...
        file_path = (
            self.week13_dir / "week13_comprehensive_analysis_20251119_221939.json"
        )
    ...
    def merge_data_sources(self) -> Dict[str, Any]:
            ...
            for _, row in strategic_df.iterrows():
                ...
                if game_id in games:
                    games[game_id]["strategic"] = {
                        ...
                        "score_prediction": self._parse_dict_string(
                            row.get("score_prediction", "{}")
                        ),
                        "spread_analysis": self._parse_dict_string(
                            row.get("spread_analysis", "{}")
                        ),
                        "over_under_analysis": self._parse_dict_string(
                            row.get("over_under_analysis", "{}")
                        ),
                        "betting_recommendations": self._parse_list_string(
                            row.get("betting_recommendations", "[]")
                        ),
                        "upset_probability": row.get("upset_probability", None),
                        "key_factors": self._parse_list_string(
                            row.get("key_factors", "[]")
                        ),
                        "strategic_advantages": self._parse_dict_string(
                            row.get("strategic_advantages", "{}")
                        ),
                        "game_script_prediction": row.get(
                            "game_script_prediction", None
                        ),
                    }
        ...
        playoff_implications = {
            "elimination_games": enhanced.get("playoff_implications", {}).get(
                "elimination_games", []
            ),
            "path_to_playoff": enhanced.get("playoff_implications", {}).get(
                "path_to_playoff", []
            ),
            "bubble_teams": enhanced.get("playoff_implications", {}).get(
                "bubble_teams", []
            ),
            "lock_scenarios": enhanced.get("playoff_implications", {}).get(
                "lock_scenarios", []
            ),
        }
        ...
        for game_id, game_data in games.items():
            ...
            for title, narrative in narratives.items():
                if home_team in title and away_team in title:
                    if not game_data.get("narrative") or len(
                        game_data["narrative"]
                    ) < len(narrative):
        ...
        confidences = [
            g.get("ensemble", {}).get("confidence", 0)
            for g in games.values()
            if g.get("ensemble", {}).get("confidence")
        ]
        avg_confidence = sum(confidences) / len(confidences) if confidences else 0
        high_confidence_games = sum(1 for c in confidences if c > 0.7)
        playoff_impact_games = len(playoff_implications["elimination_games"]) + len(
            playoff_implications["path_to_playoff"]
        )
        ...
        logger.info(
            f"Merge complete: {total_games} games, {len(confidences)} with confidence scores"
        )
        return unified
    ...
    def validate_data_integrity(self, unified_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Validation: {validation['games_with_predictions']}/{validation['total_games']} games have predictions"
        )
        return validation
...
"""
        for game in games:
            ...
            betting_str = (
                ", ".join(betting_rec)
                if isinstance(betting_rec, list)
                else str(betting_rec)
            )
...
class VisualizationDashboardBuilder:
    ...
    def generate_section(
        self, games: List[Dict], enhanced_data: Dict, power_rankings: List[Dict]
    ) -> str:
        """Generate visualization dashboard"""
        # Extract data
        confidences = [
            g.get("ensemble", {}).get("confidence", 0.5)
            for g in games
            if g.get("ensemble", {}).get("confidence")
        ]
        margins = [
            g.get("ensemble", {}).get("margin", 0) for g in games if g.get("ensemble")
        ]
        upset_probs = [
            g.get("upset_alert", {}).get("upset_probability", 0)
            for g in games
            if g.get("upset_alert")
        ]
        if not upset_probs:
            upset_probs = [0.5] * len(games)

        # V001: Confidence Histogram
        conf_hist_data = self._clean_json(
            [
                {
                    "x": confidences,
                    "type": "histogram",
        ...
        conf_hist_layout = self._clean_json(
            {
                "title": "Confidence Distribution",
                "xaxis": {"title": "Confidence (0.5-1.0)"},
                "yaxis": {"title": "Count"},
            }
        )
            ...
            [
                {
                    ...
                    "text": [
                        f"{g.get('game_info', {}).get('away_team', '')} @ {g.get('game_info', {}).get('home_team', '')}"
                        for g in games[: len(margins)]
                    ],
                }
            ]
        )
        spread_layout = self._clean_json(
            {
                "title": "Spread vs Confidence",
        ...
        top_bar_data = self._clean_json(
            [
                {
                    "x": [
                        g.get("ensemble", {}).get("confidence", 0) * 100
                        for g in top_games
                    ],
                    "y": [
                        f"{g.get('game_info', {}).get('away_team', '')} @ {g.get('game_info', {}).get('home_team', '')}"
        ...
        top_bar_layout = self._clean_json(
            {
                "title": "Top 10 Matchups by Confidence",
...
class PlayoffPictureVisualizer:
    ...
    def generate_section(self, enhanced_data: Dict, power_rankings: List[Dict]) -> str:
        ...
        playoff_implications = (
            enhanced_data.get("playoff_implications", {}) if enhanced_data else {}
        )
...
class BettingIntelligenceHub:
    ...
    def generate_section(
        self,
        games: List[Dict],
        strategic_data: Optional[pd.DataFrame],
        upset_alerts: Optional[pd.DataFrame],
    ) -> str:
        ...
        upset_list = sorted(
            upset_list,
            key=lambda x: x.get("upset_alert", {}).get("upset_probability", 0),
            reverse=True,
        )[:5]
...
class MasterReportAssembler:
    ...
    def generate_report(self) -> str:
        ...
        html_parts.append(
            self.summary_builder.generate_section(
                unified_data["metadata"], unified_data["games"]
            )
        )
        html_parts.append(self.game_explorer.generate_section(unified_data["games"]))
        html_parts.append(
            self.viz_builder.generate_section(
                unified_data["games"],
                enhanced_data,
        ...
        html_parts.append(
            self.playoff_viz.generate_section(
                enhanced_data, unified_data.get("power_rankings", [])
            )
        )
        html_parts.append(
            self.betting_hub.generate_section(
                unified_data["games"], strategic_data, upset_alerts
            )
        )
        html_parts.append(
            self.rankings_showcase.generate_section(
                unified_data.get("power_rankings", [])
            )
        )
...
""")
    ...
    def save_report(
        self, html_content: str, filename: str = "week13_master_report.html"
    ):
