...
def fix_notebook(filepath):
    ...
    for cell in cells:
        ...
        if (
            'pd.read_csv("training_data.csv")' in source_str
            and 'query("season == 2023' in source_str
        ):
            ...
            new_source_lines.append(
                "# Standardized Data Loading with Dynamic Season/Week Selection\n"
            )
            ...
            new_source_lines.append("\n")
            new_source_lines.append(
                "# Dynamically select the latest available season and week to prevent time-travel/stale data issues\n"
            )
            ...
            new_source_lines.append(
                'print(f"Testing with data from Season {latest_season}, Week {latest_week}")\n'
            )
            new_source_lines.append("\n")
            new_source_lines.append(
                'df = df.query(f"season == {latest_season} and week == {latest_week}").reset_index(drop=True)\n'
            )
            ...
            for line in source:
                if (
                    'pd.read_csv("training_data.csv")' in line
                    and 'query("season == 2023' in line
                ):
