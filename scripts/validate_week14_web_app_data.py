...
def validate_web_app_data():
    ...
    if preds_json_path.exists():
        ...
        if preds:
            ...
            if not missing_fields:
                validation_results["checks"]["predictions_json"] = {
                    "status": "passed",
                    "count": len(preds),
                }
                ...
                validation_results["warnings"].append(
                    f"Missing fields in predictions JSON: {missing_fields}"
                )
        ...
        validation_results["checks"]["predictions_json"] = {
            "status": "failed",
            "path": str(preds_json_path),
        }
        validation_results["issues"].append(
            f"Predictions JSON not found: {preds_json_path}"
        )
    ...
    if preds_csv_path.exists():
        ...
        validation_results["checks"]["predictions_csv"] = {
            "status": "passed",
            "count": len(df),
        }
    else:
        validation_results["checks"]["predictions_csv"] = {
            "status": "missing",
            "path": str(preds_csv_path),
        }
        validation_results["warnings"].append(
            f"Predictions CSV not found: {preds_csv_path}"
        )
    ...
    if ats_json_path.exists():
        ...
        if ats:
            ...
            if not missing_fields:
                validation_results["checks"]["ats_json"] = {
                    "status": "passed",
                    "count": len(ats),
                }
                ...
                validation_results["warnings"].append(
                    f"Missing fields in ATS JSON: {missing_fields}"
                )
                print(f"   ⚠️  Missing fields: {missing_fields}")
        else:
    ...
    if ats_csv_path.exists():
        ...
        validation_results["checks"]["ats_csv"] = {
            "status": "missing",
            "path": str(ats_csv_path),
        }
        validation_results["warnings"].append(
            f"ATS table CSV not found: {ats_csv_path}"
        )
    ...
    if preds_json_path.exists() and ats_json_path.exists():
        ...
        if len(preds) == len(ats):
            validation_results["checks"]["data_consistency"] = {
                "status": "passed",
                "count": len(preds),
            }
            ...
            validation_results["warnings"].append(
                f"Data count mismatch: predictions={len(preds)}, ats={len(ats)}"
            )
            print(
                f"   ⚠️  Data count mismatch: predictions={len(preds)}, ats={len(ats)}"
            )
