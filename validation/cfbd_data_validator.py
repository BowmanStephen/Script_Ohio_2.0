...
class CFBDDataValidator:
    ...
    def validate_dataframe(self, df: pd.DataFrame) -> ValidationResult:
        ...
        if (
            "home_points" in df_columns
            and "away_points" in df_columns
            and "margin" in df_columns
        ):
    ...
    def fix_data_issues(
        ...
        if all(
            col in df_fixed.columns for col in ["home_points", "away_points", "margin"]
        ):
            ...
            if mask.any():
                ...
                df_fixed.loc[mask & df_fixed["margin"].isna(), "margin"] = (
                    calculated_margin[mask & df_fixed["margin"].isna()]
                )
                ...
                if inconsistent.any():
                    logger.info(
                        f"Fixed margin calculation for {inconsistent.sum()} rows"
                    )
