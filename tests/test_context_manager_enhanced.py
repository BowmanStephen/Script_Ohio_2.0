...
class TestContextManagerEnhanced:
    ...
    def test_role_detection_edge_cases(self, context_manager):
        ...
        mixed_context = {
            "notebooks": [
                "model_pack/advanced_notebook.ipynb"
    ...
    def test_context_loading_comprehensive(
    ...
    def test_token_optimization_algorithms(self, context_manager):
        ...
        large_context = {
            ...
            "notebooks": [
                {"path": f"notebook_{i}", "content": "content " * 1000}
                for i in range(100)
            ],
            "models": [
                {"name": f"model_{i}", "metadata": "data " * 500} for i in range(50)
            ],
        }
    ...
    def test_context_personalization(
    ...
    def test_context_validation_and_error_handling(self, context_manager):
        ...
        context = context_manager.load_context_for_role(
            UserRole.ANALYST, malformed_context
        )
    ...
    def test_concurrent_context_loading(
        ...
        def load_context(thread_id):
            try:
                context = context_manager.load_context_for_role(
                    UserRole.DATA_SCIENTIST, sample_data_scientist_context
                )
    ...
    def test_memory_usage_optimization(self, context_manager):
        ...
        for i in range(10):
            ...
            context = context_manager.load_context_for_role(
                UserRole.ANALYST, large_context
            )
            assert context is not None
    ...
    def test_context_persistence_and_recovery(
        ...
        context2 = new_context_manager.load_context_for_role(
            role, sample_analyst_context
        )
    ...
    def test_context_quality_metrics(self, context_manager):
        ...
        assert (
            len(quality_scores) >= len(test_contexts) - 1
    ...
    def test_context_adaptation_learning(
        self, context_manager, sample_data_scientist_context
    ):
        ...
        context = context_manager.load_context_for_role(
            UserRole.DATA_SCIENTIST, adaptive_context
        )
    ...
    def test_context_security_and_privacy(self, context_manager):
        ...
        context = context_manager.load_context_for_role(
            UserRole.ANALYST, sensitive_context
        )

        # Sensitive data should be filtered or masked
        assert "user_data" not in context or all(
            key not in str(context) for key in ["email", "api_key", "personal_info"]
        )
