...
class TestMetricsCalculationAgent:
    ...
    def mock_agent(self, temp_dir, sample_historical_data, sample_2025_data):
        ...
        with patch(
            "model_pack.metrics_calculation_agent.MetricsCalculationAgent.__init__",
            lambda self: None,
        ):
            ...
            agent.processed_2025_path = os.path.join(
                temp_dir, "2025_processed_features.csv"
            )
            agent.updated_training_path = os.path.join(
                temp_dir, "updated_training_data.csv"
            )
...
class TestDataAcquisitionAgent:
    ...
    def mock_agent(self, mock_api_key):
        ...
        file_path = os.path.join(
            os.path.dirname(__file__), "..", "model_pack", "2025_data_acquisition_v2.py"
        )
...
class TestStarterPackDataMigrator:
    ...
    def mock_config(self):
        ...
        mock_config.get_starter_pack_data_path.return_value = Path(
            "/tmp/test_games.csv"
        )
    ...
    def test_apply_week_filter_with_max(self, mock_migrator):
        ...
        assert all(
            (mock_migrator.processed_current_season["week"] >= 5)
...
class TestEdgeCases:
    ...
    def test_api_failure_handling(self, mock_api_key):
        ...
        data_acquisition_v2 = importlib.import_module(
            "model_pack.2025_data_acquisition_v2"
        )
        with patch.object(data_acquisition_v2, "API_KEY", mock_api_key):
            with patch.object(data_acquisition_v2, "GQL_AVAILABLE", False):
                with patch.object(data_acquisition_v2, "CFBDGraphQLClient", None):
