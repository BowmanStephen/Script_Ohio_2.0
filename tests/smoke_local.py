...
def test_end_to_end_predict():
    ...
    try:
        ...
        parameters = {"team1": "Ohio State", "team2": "Michigan"}

        # Execute prediction
        result = agent._execute_action(
            "predict_game_outcome", parameters, {"user_id": "smoke_test"}
        )
        ...
        print(
            f"✅ End-to-end predict call completed: {result.get('status', 'unknown')}"
        )
