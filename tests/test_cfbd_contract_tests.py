...
class TestGraphQLFallbackContracts:
    ...
    def agent_with_fallback_enabled(self):
        ...
        with patch('src.cfbd_client.unified_client.cfbd.ApiClient'):
            ...
            mock_graphql_error = Exception("Authentication hook unauthorized this request (403)")
    ...
    def test_graphql_fallback_on_403_when_enabled(self, agent_with_fallback_enabled):
        ...
        mock_game = Mock()
        mock_game.to_dict.return_value = {'id': 1, 'home_team': 'Ohio State'}
        agent_with_fallback_enabled.client.games_api.get_games.return_value = [mock_game]
    ...
    def test_graphql_no_fallback_on_403_when_disabled(self, agent_with_fallback_disabled):
    ...
    def test_graphql_fallback_respects_env_var(self):
        ...
        with patch.dict(os.environ, {"CFBD_GRAPHQL_FALLBACK_TO_REST": "false"}, clear=False):
            config = CFBDConfig(
                api_key="test_key",
            ...
            with patch('src.cfbd_client.unified_client.cfbd.ApiClient'):
                ...
                assert result["status"] == "error"
                assert "requires_tier" in result or "Patreon Tier 3+" in result.get("error", "")
