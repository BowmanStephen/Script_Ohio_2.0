...
def graphql_malformed_fixture():
    ...
    return {
        ...
        "invalid_types": {
            "game": [{"id": "not_a_number", "season": 2025}]
        },  # Invalid types
        "null_values": {
            "game": [{"id": None, "homeTeam": None, "awayTeam": None}]
        },  # Null values
...
def test_graphql_dict_with_nested_structure(
    feature_engineer, graphql_scoreboard_fixture
):
    """Test GraphQL dict processing when games are in nested structure."""
    # Test with full nested structure {"game": [...]}
    games_df = feature_engineer.prepare_games_frame(
        graphql_scoreboard_fixture, source="graphql"
    )
...
def test_mixed_graphql_rest_scenario(
    feature_engineer, graphql_scoreboard_fixture, rest_games_fixture
):
...
def test_malformed_rest_missing_attributes(feature_engineer, rest_malformed_fixture):
    ...
    games_df = feature_engineer.prepare_games_frame(
        rest_malformed_fixture, source="rest"
    )
...
def test_feature_engineering_integration_graphql(
    feature_engineer, graphql_scoreboard_fixture
):
...
def test_feature_engineering_with_86_column_schema(
    feature_engineer, graphql_scoreboard_fixture
):
