...
def create_team_efficiency_dashboard(
    teams_data: pd.DataFrame,
    x_col: str = "offense_ppa",
    y_col: str = "defense_ppa",
    title: str = "Team Efficiency",
    ...
    if hover_data is None:
        hover_data = (
            ...
            if "record" in teams_data.columns
    ...
    fig.add_hline(
        y=y_mean, line_dash="dash", line_color="gray", annotation_text="Avg Defense"
    )
    fig.add_vline(
        x=x_mean, line_dash="dash", line_color="gray", annotation_text="Avg Offense"
    )

    return fig


def create_metric_distribution_explorer(
    metrics_data: pd.DataFrame, metrics: List[str]
) -> go.Figure:
    """
    ...
    for metric in metrics:
        if metric in metrics_data.columns:
            fig.add_trace(
                go.Histogram(
                    x=metrics_data[metric], name=metric, visible=False, opacity=0.75
                )
            )
    ...
    for i, metric in enumerate(metrics):
        ...
        buttons.append(
            dict(
                label=metric,
                method="update",
                args=[{"visible": visible}, {"title": f"Distribution of {metric}"}],
            )
        )
