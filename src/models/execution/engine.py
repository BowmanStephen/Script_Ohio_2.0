...
class ModelInterface(ABC):
    ...
    def calculate_confidence(
        self, model: Any, prediction: Any, input_data: Dict[str, Any]
    ) -> float:
...
class ScikitLearnInterface(ModelInterface):
    ...
    def align_features_for_model(
        self, input_features: Dict[str, Any], expected_features: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        for feature in expected_features:
            if feature in input_features:
                ...
                aligned_features[feature] = (
                    0.0  # Default to 0.0 for missing numeric features
                )
        ...
        if extra_features:
            logger.info(
                f"Removing extra features not expected by model: {list(extra_features)}"
            )
    ...
    def align_features_for_ridge(self, model: Any, input_features: Any) -> np.ndarray:
        ...
        if not isinstance(input_features, dict):
            raise ValueError(
                f"Input must be Dict or DataFrame, got {type(input_features)}"
            )
        ...
        aligned_features = self.align_features_for_model(
    ...
    def prepare_features_for_xgb(self, input_data: Any) -> Dict[str, Any]:
        ...
        for feature_name, feature_value in input_data.items():
            ...
            try:
                ...
                logger.info(
                    f"Removing object column for XGBoost: {feature_name} (type: {type(feature_value).__name__})"
                )
                continue

        logger.info(
            f"✅ XGBoost feature preparation: {len(cleaned_features)} numeric features retained"
        )
    ...
    def predict(self, model: Any, input_data: Any) -> Any:
        ...
        try:
            ...
            if not isinstance(input_data, dict):
                raise ValueError(
                    f"Input data must be Dict or DataFrame, got {type(input_data)}"
                )
            ...
            is_xgboost_model = (
                hasattr(model, "__class__") and "xgboost" in str(type(model)).lower()
            )
            ...
            if not np.issubdtype(feature_array.dtype, np.number):
                raise ValueError(
                    "XGBoost requires all features to be numeric. Found non-numeric data."
                )
...
class FastAIInterface(ModelInterface):
        ...
        if self._fastai_available and self._load_learner is not None:
            try:
                ...
                if hasattr(learner, "predict"):
                    ...
                    logger.warning(
                        "FastAI learner loaded but invalid - trying fallback"
                    )
            except Exception as e:
                logger.warning(
                    f"FastAI load_learner failed: {e} - trying pickle fallback"
                )
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                if hasattr(model, "predict"):
                    logger.info(
                        f"Successfully loaded FastAI model via pickle fallback from {file_path}"
                    )
                    return model
                else:
    ...
    def _build_prediction_row(
        self, model: Any, input_data: Dict[str, Any], feature_names: List[str]
    ) -> Dict[str, Any]:
...
class ModelExecutionEngine(BaseAgent):
    ...
    def __init__(self, agent_id: str, tool_loader=None, base_path=None):
        ...
        logger.info(
            f"Model Execution Engine initialized with {len(self.models)} models and betting analytics"
        )
                ...
                if model_def["file_path"].exists():
                    ...
                    if interface:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to load model {model_def['name']}: {str(load_error)}"
                            )
                            logger.warning(
                                f"Skipping model {model_def['name']} - unable to load"
                            )
                            continue
                        ...
                        logger.warning(
                            f"No interface available for model type: {file_ext}"
                        )
                else:
        ...
        try:
            ...
            if not model_path.exists():
                raise FileNotFoundError(
                    f"Model file not found: {model_metadata.file_path}"
                )
            ...
            if include_explanation:
                explanation = self._generate_prediction_explanation(
                    model_metadata, input_features, formatted_prediction
                )
    ...
    def prepare_training_features(
        ...
        numeric_columns = features_df.select_dtypes(
            include=[np.int64, np.float64, np.int32, np.float32]
        ).columns
        ...
        for col in features_df.columns:
            if features_df[col].dtype == "object":
                ...
                non_numeric = features_df[col].apply(
                    lambda x: not isinstance(x, (int, float)) and pd.notna(x)
                )
                if non_numeric.any():
                    logger.warning(
                        f"Column {col} contains non-numeric values, converting to numeric"
                    )
        ...
        if "features" in parameters and parameters["features"]:
            ...
            if home_team and away_team:
                try:
                    ...
                    logger.info(
                        f"✅ Loaded real game data for {home_team} vs {away_team}"
                    )
                except Exception as e:
                    logger.warning(
                        f"⚠️ Could not load real game data: {e}. Using intelligent imputation."
                    )
                    ...
                    if interface:
                        ...
                        if hasattr(model, "feature_names_in_"):
                            ...
                            if sklearn_interface:
                                aligned_features = (
                                    sklearn_interface.align_features_for_model(
                                        features, expected_features
                                    )
                                )
                                logger.info(
                                    f"✅ Aligned {len(aligned_features)} features for {model_name}"
                                )
                                return aligned_features

            logger.info(
                f"⚠️ Could not align features for {model_name} - using original features"
            )
    ...
    def _get_intelligent_defaults(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            training_data_path = (
                self.base_path / "model_pack" / "updated_training_data.csv"
            )
            ...
            if training_data_path.exists():
                ...
                logger.info(
                    f"✅ Using intelligent defaults from training data medians/modes"
                )
                return defaults
            else:
    ...
    def _calculate_regression_confidence(
        self, prediction: float, input_features: Dict[str, Any]
    ) -> float:
    ...
    def _batch_predictions(
        ...
        for i, game in enumerate(games):
            try:
                ...
                predictions.append({"game_index": i, "error": str(e), "success": False})
    ...
    def _model_comparison(
        ...
        if len(successful_models) > 1:
            ...
            if self.models[models_to_compare[0]].model_type == "regression":
                margins = [
                    v["prediction"].get("predicted_margin", 0)
                ...
                home_win_probs = [
                    v["prediction"].get("home_win_probability", 0.5)
                    for v in successful_models.values()
                ]
    ...
    def _model_performance_analysis(
        self, parameters: Dict[str, Any], user_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        model_predictions = [
            p for p in self.prediction_history if p["model_name"] == model_name
        ]
            ...
            if self.tool_loader:
                feature_analysis = self.execute_tool(
                    "analyze_feature_importance",
                    {
                        ...
                        "sample_data": [
                            p["input_features"] for p in model_predictions[-5:]
                        ],
                    },
                    user_context,
                )
    ...
    def _generate_model_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if recent_predictions == 0:
            recommendations.append(
                "Model needs more usage to generate reliable recommendations"
            )
        elif recent_predictions < 10:
            recommendations.append(
                "Consider using model more frequently to improve confidence metrics"
            )

        if avg_confidence < 0.6:
            recommendations.append(
                "Model confidence is low - consider feature engineering or retraining"
            )

        feature_count = analysis.get("feature_count", 0)
        if feature_count > 50:
            recommendations.append(
                "Model uses many features - consider feature selection for efficiency"
            )

        performance_metrics = analysis.get("performance_metrics", {})
        if performance_metrics.get("mae", 0) > 20:
            recommendations.append(
                "Model MAE is high - consider additional training data or feature engineering"
            )
        elif performance_metrics.get("accuracy", 0) < 0.4:
            recommendations.append(
                "Model accuracy is below threshold - review training methodology"
            )
    ...
    def _generate_system_health_recommendations(
        ...
        if critical_count > 0:
            recommendations.append(
                f"URGENT: {critical_count} of {total_models} models have critical issues requiring immediate attention"
            )
            recommendations.append(
                "Critical models cannot be used for predictions - system functionality impaired"
            )

        if warning_models:
            recommendations.append(
                f"{len(warning_models)} models have warnings that may impact performance"
            )
            recommendations.append(
                "Review warning models for potential optimization opportunities"
            )

        if len(healthy_models) == total_models:
            recommendations.append(
                "All models are healthy - system is operating normally"
            )
        elif len(healthy_models) > 0:
            recommendations.append(
                f"{len(healthy_models)} models are healthy and ready for predictions"
            )
        ...
        if any(
            "mae" in issue.lower() or "accuracy" in issue.lower()
            for issue in all_issues
        ):
            recommendations.append(
                "Consider model retraining with updated data or feature engineering"
            )

        # Usage-related issues
        if any("recent predictions" in issue.lower() for issue in all_issues):
            recommendations.append(
                "Test models with sample predictions to verify functionality"
            )
    ...
    def list_available_models(self) -> Dict[str, Any]:
        ...
        try:
            ...
            working_models = sum(
                1
                for m in models_info.values()
                if m["file_exists"] and m["interface_available"]
            )
            regression_models = sum(
                1 for m in models_info.values() if m["model_type"] == "regression"
            )
            classification_models = sum(
                1 for m in models_info.values() if m["model_type"] == "classification"
            )
            ...
            return {
                ...
                "summary": {
                    ...
                    "total_file_size_mb": round(
                        sum(m["file_size_mb"] for m in models_info.values()), 2
                    ),
                },
            }
        ...
        try:
            ...
            detailed_metadata = {
                ...
                "health_status": {
                    ...
                    "interface_available": file_path.suffix.lower().lstrip(".")
                    in self.model_interfaces,
    ...
    def batch_predict(
        self, prediction_requests: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
            ...
            for i, request in enumerate(prediction_requests):
                try:
                    ...
                    prediction_params = {
                        ...
                        "include_explanation": request.get(
                        ...
                        results.append(
                            {
                                ...
                                "prediction": prediction_result["prediction"],
                                "model_used": model_name,
                                "execution_time": prediction_result.get(
                                    "execution_time", 0
                                ),
                            }
                        )
                        ...
                        results.append(
                            {
                                ...
                                "error": prediction_result.get(
                                    "error_message", "Unknown prediction error"
                                ),
                            }
                        )
                        failed_predictions += 1
        ...
        try:
            ...
            for check_model_name, metadata in models_to_check.items():
                ...
                if not file_accessible:
                    ...
                    model_health["issues"].append(
                        f"Model file not found or inaccessible: {metadata.file_path}"
                    )
                    model_health["recommendations"].append(
                        "Restore missing model file or update file path"
                    )
                    critical_models += 1
                    continue
                ...
                try:
                    ...
                    if file_size == 0:
                        ...
                        model_health["recommendations"].append(
                            "Model file appears to be corrupted, restore from backup"
                        )
                        critical_models += 1
                        continue
                        ...
                        model_health["issues"].append(
                            f"Model file is very large: {file_size_mb}MB"
                        )
                        model_health["recommendations"].append(
                            "Consider model optimization if performance is impacted"
                        )
                        warning_models += 1
                    ...
                    model_health["status"] = "critical"
                    model_health["issues"].append(
                        f"Cannot read file properties: {str(e)}"
                    )
                    model_health["recommendations"].append(
                ...
                if not loadable:
                    model_health["status"] = "critical"
                    model_health["issues"].append(
                        "Model cannot be loaded - may be corrupted"
                    )
                    model_health["recommendations"].append(
                        "Retrain model or restore from clean backup"
                    )
                    critical_models += 1
                    continue
                ...
                if metadata.features_required:
                    ...
                    model_health["checks"]["features_count"] = len(
                        metadata.features_required
                    )
                else:
                    model_health["status"] = "warning"
                    ...
                    model_health["recommendations"].append(
                        "Define required features for better model documentation"
                    )
                    warning_models += 1
                ...
                recent_predictions = len(
                    [
                        ...
                        if p["model_name"] == check_model_name
                    ]
                )
                ...
                if recent_predictions == 0:
                    model_health["status"] = "warning"
                    model_health["issues"].append(
                        "No recent predictions - model may be unused"
                    )
                    model_health["recommendations"].append(
                        "Test model with sample predictions to verify functionality"
                    )
                    warning_models += 1
                ...
                if metadata.performance_metrics:
                    ...
                    if mae and mae > 25:  # High MAE for sports predictions
                        model_health["status"] = "warning"
                        model_health["issues"].append(
                            f"High MAE indicates poor model performance: {mae}"
                        )
                        model_health["recommendations"].append(
                        ...
                        model_health["status"] = "warning"
                        model_health["issues"].append(
                            f"Low accuracy indicates poor model performance: {accuracy:.1%}"
                        )
                        model_health["recommendations"].append(
                            "Review training methodology and feature selection"
                        )
                        warning_models += 1
                else:
                    model_health["status"] = "warning"
                    ...
                    model_health["recommendations"].append(
                        "Calculate and store performance metrics for model monitoring"
                    )
                    warning_models += 1
    ...
    def _kelly_criterion_analysis(
        ...
        try:
            ...
            kelly_result = (
                self.betting_analytics.kelly_calculator.calculate_kelly_fraction(
            ...
            if fraction < 1.0:
                kelly_result = (
                    self.betting_analytics.kelly_calculator.calculate_fractional_kelly(
    ...
    def _value_betting_detection(
        ...
        try:
            ...
            # Format results
            value_bets = {
                k: asdict(v) for k, v in opportunities.items() if v.is_value_bet
            }
    ...
    def _betting_opportunity_analysis(
        self, parameters: Dict[str, Any], user_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def get_model_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "model_types": list(
                set(self.models[name].model_type for name in self.models.keys())
            ),
            "total_predictions_made": total_predictions,
            "model_usage_counts": model_usage,
            "prediction_history_size": len(self.prediction_history),
            "average_features_per_model": np.mean(
                [
                    len(self.models[name].features_required)
