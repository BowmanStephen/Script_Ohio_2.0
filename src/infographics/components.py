...
class AgentArchitectureVisualizer(BaseComponent):
    ...
    def __init__(
        self,
        title: str = "Agent System Architecture",
        description: Optional[str] = None,
    ...
    def generate_html(
        ...
        try:
            ...
            if not data or "agents" not in data or not data.get("agents"):
                ...
                # Validate data
                validate_component_data(
                    data, required_fields=["agents"], field_types={"agents": list}
                )
                agents = data["agents"]
            ...
            html_path = generate_standalone_html(
                ...
                or "Interactive visualization of Script Ohio 2.0 agent system architecture",
            )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    marker=dict(
                        size=[
                            20 + cap * 2
                            for cap in [capability_counts[i] for i in indices]
                        ],
    ...
    def _create_controls(self, agents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        permission_levels = sorted(
            set(agent.get("permission_level", "READ_EXECUTE") for agent in agents)
        )
...
class ModelComparisonDashboard(BaseComponent):
    ...
    def generate_html(
        ...
        try:
            ...
            if not data or "models" not in data or not data.get("models"):
                ...
                # Validate data
                validate_component_data(
                    data, required_fields=["models"], field_types={"models": list}
                )
                models = data["models"]
            ...
            html_path = generate_standalone_html(
                ...
                title=self.title,
                description=self.description
                or "Interactive comparison of machine learning model performance",
            )
    ...
    def _create_comparison_dashboard(self, models: List[Dict[str, Any]]) -> go.Figure:
        ...
        fig.update_layout(title=self.title, height=800, showlegend=False)
...
class PredictionConfidenceAnalyzer(BaseComponent):
    ...
    def __init__(
        self,
        title: str = "Prediction Confidence Analyzer",
        description: Optional[str] = None,
        interactive: bool = True,
    ):
        super().__init__(title, description, interactive)
    ...
    def generate_html(
        ...
        try:
            ...
            html_path = generate_standalone_html(
                ...
                title=self.title,
                description=self.description
                or "Interactive analysis of prediction confidence vs spread",
            )
    ...
    def _create_scatter_plot(self, predictions: List[Dict[str, Any]]) -> go.Figure:
        ...
        games = [
            f"{p.get('away_team', 'Away')} @ {p.get('home_team', 'Home')}"
    ...
    def _create_controls(
        ...
        """Create interactive controls"""
        conferences = sorted(
            set(p.get("conference", "") for p in predictions if p.get("conference"))
        )
...
class DataFlowExplorer(BaseComponent):
    ...
    def generate_html(
        ...
        try:
            ...
            if not data or "pipeline" not in data or not data.get("pipeline"):
                ...
                # Validate data
                validate_component_data(
                    data, required_fields=["pipeline"], field_types={"pipeline": list}
                )
            ...
            html_path = generate_standalone_html(
                ...
                title=self.title,
                description=self.description
                or "Interactive visualization of data processing pipeline",
            )
    ...
    def _create_pipeline_diagram(self, pipeline: List[Dict[str, Any]]) -> go.Figure:
        ...
        x_positions = [
            i / (len(pipeline) - 1) if len(pipeline) > 1 else 0.5
        ...
        fig.update_layout(
            title=self.title,
            xaxis=dict(
                showgrid=False, showticklabels=False, zeroline=False, range=[-0.1, 1.1]
            ),
            yaxis=dict(
...
class LearningPathNavigator(BaseComponent):
    ...
    def __init__(
        self,
        title: str = "Learning Path Navigator",
        description: Optional[str] = None,
        interactive: bool = True,
    ):
        super().__init__(title, description, interactive)
    ...
    def generate_html(
        ...
        try:
            ...
            if not data or "notebooks" not in data or not data.get("notebooks"):
                ...
                # Validate data
                validate_component_data(
                    data, required_fields=["notebooks"], field_types={"notebooks": list}
                )
            ...
            html_path = generate_standalone_html(
                ...
                title=self.title,
                description=self.description
                or "Interactive guide through Script Ohio 2.0 learning path",
            )
    ...
    def _create_learning_path(self, notebooks: List[Dict[str, Any]]) -> go.Figure:
        ...
        notebook_names = [
            nb.get("name", f"Notebook {i + 1}") for i, nb in enumerate(notebooks)
        ]
        ...
        fig.update_layout(
            ...
            xaxis=dict(showgrid=False, showticklabels=False, zeroline=False),
            yaxis=dict(
                showgrid=False, showticklabels=False, zeroline=False, range=[-0.5, 0.5]
            ),
            height=400,
            showlegend=False,
        )
    ...
    def _create_controls(self, notebooks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Create interactive controls"""
        skill_levels = sorted(
            set(nb.get("skill_level", "beginner") for nb in notebooks)
        )
