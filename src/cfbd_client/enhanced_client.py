...
try:
    ...
    CFBD_VERSION = getattr(
        cfbd, "__version__", "4.3.1"
    )  # Handle version detection gracefully
...
class EnhancedCFBDClient:
    ...
    def __init__(
        ...
        config: Optional[CFBDRateLimitConfig] = None,
        ...
        if cfbd is None:
            raise ImportError(
                "❌ CFBD client not available. Install with: pip install cfbd==5.13.2"
            )
        ...
        self.api_host = os.environ.get(
            "CFBD_API_HOST", "https://api.collegefootballdata.com"
        )
        ...
        logger.info(
            f"   Rate Limit: {self.rate_config.max_requests_per_second} req/sec"
        )
        logger.info(
            f"   Caching: {'Enabled' if self.cache_config.enable_cache else 'Disabled'}"
        )
        ...
        for attempt in range(self.rate_config.max_retries):
            try:
                ...
                if e.status == 429:  # Rate limit exceeded
                    ...
                    if (
                        cfbd_error
                        and hasattr(cfbd_error, "retry_after")
                        and cfbd_error.retry_after
                    ):
                        wait_time = cfbd_error.retry_after
                    else:
                        wait_time = 2**attempt + 1  # Exponential backoff
                    logger.warning(
                        f"⏱️ Rate limit hit, waiting {wait_time}s (attempt {attempt + 1})"
                    )
                    ...
                    if attempt < self.rate_config.max_retries - 1:
                        ...
                        logger.error(
                            f"❌ Server error after {attempt + 1} attempts: {error_msg}"
                        )
                        self._track_performance(start_time, success=False)
                        if cfbd_error:
                            raise cfbd_error
        ...
        if games_data:
            ...
            if use_cache and self.cache_config.enable_cache:
                self.cache_manager.cache_data(
                    "games", cache_params, games_list, "games"
                )
    ...
    def get_team_stats(
        self, year: int, team: Optional[str] = None, use_cache: bool = True
    ) -> List[Dict[str, Any]]:
        ...
        stats_data = self._safe_api_call(
            self.stats_api.get_team_season_stats, year=year, team=team
        )
        ...
        if stats_data:
            ...
            if use_cache and self.cache_config.enable_cache:
                self.cache_manager.cache_data(
                    "team_stats", cache_params, stats_list, "stats"
                )
    ...
    def get_teams(
        self, conference: Optional[str] = None, use_cache: bool = True
    ) -> List[Dict[str, Any]]:
        ...
        teams_data = self._safe_api_call(
            self.teams_api.get_teams, conference=conference
        )
        ...
        if teams_data:
            ...
            if use_cache and self.cache_config.enable_cache:
                self.cache_manager.cache_data(
                    "teams", cache_params, teams_list, "teams"
                )
...
def create_enhanced_cfbd_client(
    ...
    return EnhancedCFBDClient(
        api_key=api_key, config=rate_config, cache_config=cache_config
    )
...
def test_enhanced_cfbd_client():
    ...
    try:
        ...
        from src.utils.data import calculate_current_week, get_current_season
