...
class CFBDForbiddenError(CFBDClientError):
    ...
    def __init__(
        self,
        message: str = "Access forbidden - check API key permissions",
        response_body: str = None,
    ):
...
def convert_api_exception(exception) -> CFBDClientError:
    ...
    if status == 401:
        return CFBDAuthenticationError(
            message, response_body=getattr(exception, "body", None)
        )
    elif status == 403:
        return CFBDForbiddenError(
            message, response_body=getattr(exception, "body", None)
        )
    elif status == 404:
        return CFBDNotFoundError(
            message, response_body=getattr(exception, "body", None)
        )
    elif status == 429:
        return CFBDRateLimitError(
            message,
            retry_after=retry_after,
            response_body=getattr(exception, "body", None),
        )
    elif 500 <= status < 600:
        return CFBDServerError(
            message, status_code=status, response_body=getattr(exception, "body", None)
        )
    else:
        # Other 4xx errors
        return CFBDClientError(
            message, status_code=status, response_body=getattr(exception, "body", None)
        )
