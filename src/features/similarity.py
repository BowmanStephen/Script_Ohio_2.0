...
class TeamSimilarityModel:
    ...
    def __init__(self, stats_df: pd.DataFrame, metrics: Optional[List[str]] = None):
        ...
        self.teams_data = self.stats_df.dropna(subset=self.metrics).reset_index(
            drop=True
        )
        ...
        self.knn = NearestNeighbors(
            n_neighbors=min(len(self.teams_data), 20), metric="euclidean"
        )
        self.knn.fit(self.X_scaled)

    def find_similar_teams(
        self, team_name: str, top_n: int = 5
        ...
        distances, indices = self.knn.kneighbors(
            [self.X_scaled[idx]], n_neighbors=top_n + 1
        )
...
def find_similar_teams(
    stats_df: pd.DataFrame, team_name: str, top_n: int = 5
) -> List[Tuple[str, float]]:
