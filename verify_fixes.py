...
def test_logging_config():
    ...
    try:
        ...
        if len(root_logger.handlers) == 0:
            ...
            print(f"WARNING: Root logger has {len(root_logger.handlers)} handlers (Expected 0).")
...
def test_merge_robustness():
    ...
    try:
        ...
        games = pd.DataFrame({'home_team': ['Ohio State', 'Michigan'], 'away_team': ['Penn State', 'Wisconsin'], 'home_points': [10, 20]})
        talent = pd.DataFrame({'school': ['Ohio State', 'Michigan', 'Penn State', 'Wisconsin'], 'talent': [900, 850, 800, 750]})
        ...
        if 'home_talent' not in res.columns: # Should return original df
            print("PASS: Handled invalid talent DataFrame correctly (returned original).")
        else:
