...
## GraphQL Data Acquisition

### Overview

GraphQL provides an alternative to REST for fetching CFBD data with several key advantages:
- **Efficient Queries**: Request only the fields you need, reducing payload size and bandwidth
- **Real-time Subscriptions**: WebSocket support for live game updates via `wss://graphql.collegefootballdata.com/v1/graphql`
- **Flexible Querying**: Single endpoint with customizable queries
- **Talent/Recruiting Data**: Enhanced access to recruiting information
- **Reduced Over-fetching**: Avoid downloading unnecessary data compared to REST endpoints

**Requirements**: **Patreon Tier 3+ subscription** and `gql[requests]` library

GraphQL is used as a **complement to REST**, not a replacement. REST remains the **default** for bulk historical pulls, while GraphQL is recommended for live or highly targeted queries.

### GraphQL Usage Examples

#### Basic Scoreboard Query
```python
from agents.cfbd_integration_agent import CFBDIntegrationAgent

agent = CFBDIntegrationAgent(agent_id="cfbd_001")

# Fetch games via GraphQL
...
# Games are automatically converted to snake_case for ML compatibility
```

#### Recruiting Data Query
```python
# Fetch recruiting data to enhance model features
result = agent._execute_action(
    "graphql_recruiting",
...
# Use recruiting data to enhance team strength features
```
...
# GraphQL data integrates seamlessly with feature pipeline
...
# Now ready for ML model predictions
feature_frame = engineer.build_feature_frame(games_df)
```

GraphQL results are normalized into the same feature store used by the model, using the same team/game keys as the REST pipeline for consistency.

### Advantages Over REST

1. **Reduced Over-fetching**: Request only needed fields, reducing bandwidth and processing time
2. **Multi-entity Queries**: Easier to compose queries that fetch games, teams, and stats in a single request
3. **Real-time Updates**: Better fit for live dashboards via WebSocket subscriptions (`wss://graphql.collegefootballdata.com/v1/graphql`)
4. **Typed Schema**: Clear, typed schema that maps directly to model features
5. **Simpler Evolution**: Easier to adapt to changing data needs without modifying REST contracts
6. **Efficient Field Selection**: Query exactly what you need, when you need it

### Data Acquisition Patterns

#### Pattern 1: GraphQL for Live Data
Use GraphQL subscriptions or frequently polled queries to feed near real-time features:
- **Use Case**: Current week's games, live scoreboard updates, real-time predictions
- **Tradeoffs**: Lower latency, but requires active WebSocket connection or frequent polling
- **Example**: Live dashboard showing current week predictions with real-time score updates

#### Pattern 2: REST for Historical Data
Use existing REST/Next pipelines or bulk jobs for backfills and large historical pulls:
- **Use Case**: Training data collection, historical analysis, bulk data exports
- **Tradeoffs**: Higher latency per request, but better for batch processing and large datasets
- **Example**: Weekly batch job to fetch all games from previous seasons for model retraining

#### Pattern 3: Hybrid Approach
Use REST to seed the feature store, then GraphQL to keep it warm and current:
- **Use Case**: Initial data load via REST, then switch to GraphQL for ongoing updates
- **Tradeoffs**: Best of both worlds, but requires coordination between data sources
- **Example**: Seed historical data via REST, then use GraphQL subscriptions to update current week's games

### Integration with Model Execution Engine

The Model Execution Engine sees a unified feature schema regardless of whether data comes from REST or GraphQL:
...
# GraphQL query automatically routed
request = AnalyticsRequest(
    user_id="user_001",
    query="Get scoreboard via GraphQL for week 12 and predict outcomes",
...
# Automatically fetches via GraphQL and generates predictions
# Feature schema is unified regardless of source
```

The execution engine automatically:
- Chooses between REST and GraphQL based on query type and availability
- Normalizes field names (camelCase → snake_case) for GraphQL data
- Applies the same feature engineering pipeline regardless of source
- Falls back to REST-derived historical features when GraphQL is unavailable

### Documentation Reference

For complete setup instructions, schema details, advanced query patterns, REST–GraphQL field mappings, WebSocket subscription examples, and agent integration patterns, see [`docs/CFBD_GRAPHQL_GUIDE.md`](../docs/CFBD_GRAPHQL_GUIDE.md).
